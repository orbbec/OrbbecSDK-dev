...
struct Text {
     ...
     */
    Text(const std::string &text_, const cv::Point &org_, int ff_, double fs_, const cv::Scalar &color_, int thick_ = 1, int lt_ = cv::LINE_8,
         bool bottom_left_origin_ = false)
        : text(text_), org(org_), ff(ff_), fs(fs_), color(color_), thick(thick_), lt(lt_), bottom_left_origin(bottom_left_origin_) {}
...
struct FText {
     ...
     * @param color_ The text color
     */
    FText(const std::wstring &text_, const cv::Point &org_, int fh_, const cv::Scalar &color_) : text(text_), org(org_), fh(fh_), color(color_) {}
...
struct Rect {
     ...
     * @param thick_  The thickness of lines that make up the rectangle. Negative values, like #FILLED, mean that the function has to draw a filled
     * rectangle
    ...
    int        thick;  //!< The thickness of lines that make up the rectangle. Negative values, like #FILLED, mean that the function has to draw a filled
                       //!< rectangle
...
struct Image {
     ...
     */
    Image(const cv::Point &org_, const cv::Mat &img_, const cv::Mat &alpha_) : org(org_), img(img_), alpha(alpha_) {}
...
@param args graph compile time parameters
*/
void GAPI_EXPORTS render(cv::Mat &y_plane, cv::Mat &uv_plane, const Prims &prims, cv::GCompileArgs &&args = {});

G_TYPED_KERNEL_M(GRenderNV12, <GMat2(cv::GMat, cv::GMat, cv::GArray<wip::draw::Prim>)>, "org.opencv.render.nv12"){
    static GMatDesc2 outMeta(GMatDesc y_plane, GMatDesc uv_plane, GArrayDesc){ return std::make_tuple(y_plane, uv_plane);
}  // namespace draw
};  // namespace wip
