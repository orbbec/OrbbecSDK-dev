...
template <typename T> Quat<T>::Quat(const Vec<T, 4> &coeff) : w(coeff[0]), x(coeff[1]), y(coeff[2]), z(coeff[3]) {}

template <typename T> Quat<T>::Quat(const T qw, const T qx, const T qy, const T qz) : w(qw), x(qx), y(qy), z(qz) {}

template <typename T> Quat<T> Quat<T>::createFromAngleAxis(const T angle, const Vec<T, 3> &axis) {
    T w, x, y, z;
...
template <typename T> inline Quat<T> Quat<T>::operator+(const Quat<T> &q1) const {
...
template <typename T> inline Quat<T> Quat<T>::operator/(const Quat<T> &q) const {
    return *this * q.inv();
}
...
template <typename T> inline Quat<T> power(const Quat<T> &q1, const T alpha, QuatAssumeType assumeUnit) {
    return q1.power(alpha, assumeUnit);
}

template <typename T> inline Quat<T> Quat<T>::power(const T alpha, QuatAssumeType assumeUnit) const {
...
template <typename T> inline Quat<T> sqrt(const Quat<T> &q, QuatAssumeType assumeUnit) {
    return q.sqrt(assumeUnit);
}

template <typename T> inline Quat<T> Quat<T>::sqrt(QuatAssumeType assumeUnit) const {
    return power(0.5, assumeUnit);
}

template <typename T> inline Quat<T> power(const Quat<T> &p, const Quat<T> &q, QuatAssumeType assumeUnit) {
    return p.power(q, assumeUnit);
}

template <typename T> inline Quat<T> Quat<T>::power(const Quat<T> &q, QuatAssumeType assumeUnit) const {
...
template <typename T> inline Quat<T> crossProduct(const Quat<T> &p, const Quat<T> &q) {
    return p.crossProduct(q);
}

template <typename T> inline Quat<T> Quat<T>::crossProduct(const Quat<T> &q) const {
...
template <typename T> inline Quat<T> inv(const Quat<T> &q, QuatAssumeType assumeUnit) {
    return q.inv(assumeUnit);
}

template <typename T> inline Quat<T> Quat<T>::inv(QuatAssumeType assumeUnit) const {
    if(assumeUnit) {
        return conjugate();
    }
    T norm2 = dot(*this);
...
template <typename T> inline Quat<T> sinh(const Quat<T> &q) {
    return q.sinh();
}
...
template <typename T> inline Quat<T> cosh(const Quat<T> &q) {
    return q.cosh();
}
...
template <typename T> inline Quat<T> tanh(const Quat<T> &q) {
    return q.tanh();
}
...
template <typename T> inline Quat<T> sin(const Quat<T> &q) {
    return q.sin();
}
...
template <typename T> inline Quat<T> cos(const Quat<T> &q) {
    return q.cos();
}
...
template <typename T> inline Quat<T> tan(const Quat<T> &q) {
    return q.tan();
}
...
template <typename T> inline Quat<T> asin(const Quat<T> &q) {
    return q.asin();
}
...
template <typename T> inline Quat<T> acos(const Quat<T> &q) {
    return q.acos();
}
...
template <typename T> inline Quat<T> atan(const Quat<T> &q) {
    return q.atan();
}
...
template <typename T> inline T Quat<T>::getAngle(QuatAssumeType assumeUnit) const {
    if(assumeUnit) {
        return 2 * std::acos(w);
    }
    if(norm() < CV_QUAT_EPS) {
...
template <typename T> inline Vec<T, 3> Quat<T>::getAxis(QuatAssumeType assumeUnit) const {
    T       angle = getAngle(assumeUnit);
...
template <typename T> Matx<T, 4, 4> Quat<T>::toRotMat4x4(QuatAssumeType assumeUnit) const {
    T a = w, b = x, c = y, d = z;
    if(!assumeUnit) {
...
template <typename T> Matx<T, 3, 3> Quat<T>::toRotMat3x3(QuatAssumeType assumeUnit) const {
    T a = w, b = x, c = y, d = z;
    if(!assumeUnit) {
        Quat<T> qTemp = normalize();
        a             = qTemp.w;
    ...
    Matx<T, 3, 3> R{ 1 - 2 * (c * c + d * d), 2 * (b * c - a * d), 2 * (b * d + a * c), 2 * (b * c + a * d),    1 - 2 * (b * b + d * d),
                     2 * (c * d - a * b),     2 * (b * d - a * c), 2 * (c * d + a * b), 1 - 2 * (b * b + c * c) };
    return R;
}

template <typename T> Vec<T, 3> Quat<T>::toRotVec(QuatAssumeType assumeUnit) const {
    T         angle = getAngle(assumeUnit);
    Vec<T, 3> axis  = getAxis(assumeUnit);
    return angle * axis;
}
...
template <typename T> Quat<T> Quat<T>::lerp(const Quat<T> &q0, const Quat<T> &q1, const T t) {
    return (1 - t) * q0 + t * q1;
}

template <typename T> Quat<T> Quat<T>::slerp(const Quat<T> &q0, const Quat<T> &q1, const T t, QuatAssumeType assumeUnit, bool directChange) {
    Quatd v0(q0);
    Quatd v1(q1);
    if(!assumeUnit) {
        v0 = v0.normalize();
...
template <typename T> inline Quat<T> Quat<T>::nlerp(const Quat<T> &q0, const Quat<T> &q1, const T t, QuatAssumeType assumeUnit) {
    Quat<T> v0(q0), v1(q1);
    if(v1.dot(v0) < 0) {
        v0 = -v0;
    }
...
template <typename T>
inline Quat<T> Quat<T>::squad(const Quat<T> &q0, const Quat<T> &q1, const Quat<T> &q2, const Quat<T> &q3, const T t, QuatAssumeType assumeUnit,
                              bool directChange) {
...
template <typename T> Quat<T> Quat<T>::interPoint(const Quat<T> &q0, const Quat<T> &q1, const Quat<T> &q2, QuatAssumeType assumeUnit) {
...
template <typename T> static Quat<T> createFromAxisRot(int axis, const T theta) {
    if(axis == 0)
...
template <typename T> Quat<T> Quat<T>::createFromEulerAngles(const Vec<T, 3> &angles, QuatEnum::EulerAnglesType eulerAnglesType) {
...
template <typename T> Vec<T, 3> Quat<T>::toEulerAngles(QuatEnum::EulerAnglesType eulerAnglesType) {
    CV_Assert(eulerAnglesType < QuatEnum::EulerAnglesType::EULER_ANGLES_MAX_VALUE);
    Matx33d R = toRotMat3x3();
    enum { C_ZERO, C_PI, C_PI_2, N_CONSTANTS, R_0_0 = N_CONSTANTS, R_0_1, R_0_2, R_1_0, R_1_1, R_1_2, R_2_0, R_2_1, R_2_2 };
