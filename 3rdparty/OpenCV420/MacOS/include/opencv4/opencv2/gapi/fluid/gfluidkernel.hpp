...
template <typename Impl, typename... Ins> struct get_border_helper<true, Impl, Ins...> {
    template <int... IIs> static gapi::fluid::BorderOpt get_border_impl(const GMetaArgs &metas, const cv::GArgs &in_args, cv::detail::Seq<IIs...>) {
...
template <typename Impl, typename... Ins> struct get_window_helper<true, Impl, Ins...> {
    template <int... IIs> static int get_window_impl(const GMetaArgs &metas, const cv::GArgs &in_args, cv::detail::Seq<IIs...>) {
...
template <typename Impl, typename... Ins, typename... Outs, bool UseScratch> struct FluidCallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...>, UseScratch> {
    ...
    static void call(const cv::GArgs &in_args, const std::vector<gapi::fluid::Buffer *> &out_bufs) {
        ...
        call_impl(in_args, out_bufs, typename detail::MkSeq<sizeof...(Ins)>::type(), typename detail::MkSeq<numOuts>::type());
    }
...
public:
    ...
    static GFluidKernel kernel() {
        ...
        return GFluidKernel(Impl::Kind, Impl::LPI, UseScratch, &P::call, &P::init_scratch, &P::reset_scratch, &P::getBorder, &P::getWindow);
    }
