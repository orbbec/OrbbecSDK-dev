...
template <> struct Yield<GFrame> {
    static inline cv::GFrame yield(cv::GCall &call, int i) {
        return call.yieldFrame(i);
    }
};
...
template <> struct MetaType<cv::GFrame> {
...
template <typename U> struct MetaType<cv::GOpaque<U>> {
...
template <typename T>
typename std::enable_if<!is_nongapi_type<T>::value, typename MetaType<T>::type>::type get_in_meta(const GMetaArgs &in_meta, const GArgs &, int idx) {
    return util::get<typename MetaType<T>::type>(in_meta.at(idx));
}

template <typename T> typename std::enable_if<is_nongapi_type<T>::value, T>::type get_in_meta(const GMetaArgs &, const GArgs &in_args, int idx) {
...
template <typename K, typename... Ins, typename... Outs> struct MetaHelper<K, std::tuple<Ins...>, std::tuple<Outs...>> {
    ...
    static GMetaArgs getOutMeta(const GMetaArgs &in_meta, const GArgs &in_args) {
        return getOutMeta_impl(in_meta, in_args, typename detail::MkSeq<sizeof...(Ins)>::type(), typename detail::MkSeq<sizeof...(Outs)>::type());
    }
};
...
template <typename K, typename... Ins, typename Out> struct MetaHelper<K, std::tuple<Ins...>, Out> {
    template <int... IIs> static GMetaArgs getOutMeta_impl(const GMetaArgs &in_meta, const GArgs &in_args, detail::Seq<IIs...>) {
        // FIXME: decay?
...
class GKernelTypeM<K, std::function<std::tuple<R...>(Args...)>> : public detail::MetaHelper<K, std::tuple<Args...>, std::tuple<R...>>, public detail::NoTag {
...
template <typename K, typename R, typename... Args>
class GKernelType<K, std::function<R(Args...)>> : public detail::MetaHelper<K, std::tuple<Args...>, R>, public detail::NoTag {
public:
    using InArgs  = std::tuple<Args...>;
    using OutArgs = std::tuple<R>;
...
template <typename K, typename... R, typename... Args>
class KernelTypeMedium<K, std::function<std::tuple<R...>(Args...)>> : public cv::GKernelTypeM<K, std::function<std::tuple<R...>(Args...)>> {};

template <typename K, typename R, typename... Args>
class KernelTypeMedium<K, std::function<R(Args...)>> : public cv::GKernelType<K, std::function<R(Args...)>> {};
}  // namespace detail
...
#define G_TYPED_KERNEL_HELPER(Class, API, Id) \
    ...
    struct Class final : public cv::detail::KernelTypeMedium<Class, std::function API>, public G_ID_HELPER_CLASS(Class)
...
#define G_TYPED_KERNEL(Class, ...)                                                                                                                     \
    __WRAP_VAARGS(GET_G_TYPED_KERNEL(__VA_ARGS__, G_TYPED_KERNEL_HELPER_10, G_TYPED_KERNEL_HELPER_9, G_TYPED_KERNEL_HELPER_8, G_TYPED_KERNEL_HELPER_7, \
                                     G_TYPED_KERNEL_HELPER_6, G_TYPED_KERNEL_HELPER_5, G_TYPED_KERNEL_HELPER_4, G_TYPED_KERNEL_HELPER_3,               \
                                     G_TYPED_KERNEL_HELPER_2, G_TYPED_KERNEL_HELPER)(Class, __VA_ARGS__))
...
namespace gapi {
GAPI_EXPORTS cv::GKernelPackage combine(const cv::GKernelPackage &lhs, const cv::GKernelPackage &rhs);
...
protected:
    ...
    template <typename KImpl> typename std::enable_if<(std::is_base_of<cv::detail::KernelTag, KImpl>::value), void>::type includeHelper() {
    ...
    template <typename TImpl> typename std::enable_if<(std::is_base_of<cv::detail::TransformTag, TImpl>::value), void>::type includeHelper() {
...
public:
    ...
    template <typename KImpl> bool includes() const {
        static_assert(std::is_base_of<cv::detail::KernelTag, KImpl>::value, "includes() can be applied to kernels only");
     ...
     *
     */
    template <typename KAPI> cv::gapi::GBackend lookup() const {
     ...
     */
    friend GAPI_EXPORTS GKernelPackage cv::gapi::combine(const GKernelPackage &lhs, const GKernelPackage &rhs);
};
/** @} */
...
template <typename... FF> GKernelPackage kernels(FF &...functors) {
    GKernelPackage pkg;
 ...
 */
template <typename... Ps> cv::GKernelPackage combine(const cv::GKernelPackage &a, const cv::GKernelPackage &b, Ps &&...rest) {
