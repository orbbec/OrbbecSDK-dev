...
struct GMeta {
    ...
    template <typename... R, int... IIs> static std::tuple<R...> yield(cv::GCall &call, cv::detail::Seq<IIs...>) {
...
template <typename T, typename G> cv::GOpaque<T> meta(G g, const std::string &tag) {
...
template <typename G> cv::GOpaque<int64_t> timestamp(G g) {
    return meta<int64_t>(g, meta_tag::timestamp);
}

template <typename G> cv::GOpaque<int64_t> seq_id(G g) {
    return meta<int64_t>(g, meta_tag::seq_id);
}

template <typename G> cv::GOpaque<int64_t> seqNo(G g) {
