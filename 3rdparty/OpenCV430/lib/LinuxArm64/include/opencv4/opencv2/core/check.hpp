...
enum TestOp { TEST_CUSTOM = 0, TEST_EQ = 1, TEST_NE = 2, TEST_LE = 3, TEST_LT = 4, TEST_GE = 5, TEST_GT = 6, CV__LAST_TEST_OP };
...
#define CV__DEFINE_CHECK_CONTEXT(id, message, testOp, p1_str, p2_str) \
    static const cv::detail::CheckContext CV__CHECK_LOCATION_VARNAME(id) = { CV__CHECK_FUNCTION, CV__CHECK_FILENAME, __LINE__, testOp, message, p1_str, p2_str }
...
/// Some complex conditions: CV_Check(src2, src2.empty() || (src2.type() == src1.type() && src2.size() == src1.size()), "src2 should have same size/type as
/// src1")
