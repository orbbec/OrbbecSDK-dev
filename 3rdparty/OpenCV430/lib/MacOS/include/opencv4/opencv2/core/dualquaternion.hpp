 ...
 * - ["A beginners guide to dual-quaternions: what they are, how they work, and how to use them for 3D character hierarchies", Ben Kenwright,
 ...
 *
 * Alternatively, dual quaternions can also be interpreted as four components which are all [dual
 * numbers](https://www.cs.utah.edu/~ladislav/kavan08geometric/kavan08geometric): \f[ \sigma = \hat{q}_w + \hat{q}_xi + \hat{q}_yj + \hat{q}_zk \f] If we set
 * \f$\hat{q}_x, \hat{q}_y\f$ and \f$\hat{q}_z\f$ equal to 0, a dual quaternion is transformed to a dual number. see normalize().
 *
...
public:
     ...
     * and this function will save some computations.
     */
    template <typename T> friend DualQuat<T> inv(const DualQuat<T> &dq, QuatAssumeType assumeUnit);
     ...
     * and this function will save some computations.
     */
    template <typename T> friend DualQuat<T> power(const DualQuat<T> &dq, const T t, QuatAssumeType assumeUnit);
     ...
     * and this function will save some computations.
     */
    template <typename T> friend DualQuat<T> power(const DualQuat<T> &p, const DualQuat<T> &q, QuatAssumeType assumeUnit);
     ...
     * and this function will save some computations.
     */
    template <typename T> friend DualQuat<T> log(const DualQuat<T> &dq, QuatAssumeType assumeUnit);
     ...
     * ```
     */
    static DualQuat<_Tp> sclerp(const DualQuat<_Tp> &q1, const DualQuat<_Tp> &q2, const _Tp t, bool directChange = true,
     ...
     * @note the type of scalar should be equal to the dual quaternion.
     */
    template <typename T> friend DualQuat<T> cv::operator+(const T s, const DualQuat<T> &);
     ...
     * @note the type of scalar should be equal to the dual quaternion.
     */
    template <typename T> friend DualQuat<T> cv::operator+(const DualQuat<T> &, const T s);
    ...
    template <typename S> friend std::ostream &cv::operator<<(std::ostream &, const DualQuat<S> &);
};
