...
template <typename T> DualQuat<T>::DualQuat() : w(0), x(0), y(0), z(0), w_(0), x_(0), y_(0), z_(0){};
...
template <typename T> DualQuat<T>::DualQuat(const Vec<T, 8> &q) : w(q[0]), x(q[1]), y(q[2]), z(q[3]), w_(q[4]), x_(q[5]), y_(q[6]), z_(q[7]){};

template <typename T> DualQuat<T> DualQuat<T>::createFromQuat(const Quat<T> &realPart, const Quat<T> &dualPart) {
    T w  = realPart.w;
    T x  = realPart.x;
    T y  = realPart.y;
    T z  = realPart.z;
...
template <typename T> DualQuat<T> DualQuat<T>::createFromAngleAxisTrans(const T angle, const Vec<T, 3> &axis, const Vec<T, 3> &trans) {
...
template <typename T> DualQuat<T> DualQuat<T>::createFromAffine3(const Affine3<T> &R) {
    return createFromMat(R.matrix);
}

template <typename T> DualQuat<T> DualQuat<T>::createFromPitch(const T angle, const T d, const Vec<T, 3> &axis, const Vec<T, 3> &moment) {
    ...
    Quat<T> dual = -half_d * std::sin(half_angle) + std::sin(half_angle) * qmoment + half_d * std::cos(half_angle) * qaxis;
    return createFromQuat(Quat<T>::createFromAngleAxis(angle, axis), dual);
}

template <typename T> inline bool DualQuat<T>::operator==(const DualQuat<T> &q) const {
    return (abs(w - q.w) < CV_DUAL_QUAT_EPS && abs(x - q.x) < CV_DUAL_QUAT_EPS && abs(y - q.y) < CV_DUAL_QUAT_EPS && abs(z - q.z) < CV_DUAL_QUAT_EPS
            && abs(w_ - q.w_) < CV_DUAL_QUAT_EPS && abs(x_ - q.x_) < CV_DUAL_QUAT_EPS && abs(y_ - q.y_) < CV_DUAL_QUAT_EPS
...
template <typename T> inline Quat<T> DualQuat<T>::getRotation(QuatAssumeType assumeUnit) const {
    if(assumeUnit) {
        return getRealPart();
    }
...
template <typename T> inline Vec<T, 3> DualQuat<T>::getTranslation(QuatAssumeType assumeUnit) const {
...
template <typename T> inline DualQuat<T> inv(const DualQuat<T> &dq, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    return dq.inv(assumeUnit);
}

template <typename T> inline DualQuat<T> DualQuat<T>::inv(QuatAssumeType assumeUnit) const {
    Quat<T> real = getRealPart();
    Quat<T> dual = getDualPart();
    return createFromQuat(real.inv(assumeUnit), -real.inv(assumeUnit) * dual * real.inv(assumeUnit));
}

template <typename T> inline DualQuat<T> DualQuat<T>::operator-(const DualQuat<T> &q) const {
...
template <typename T> inline DualQuat<T> &DualQuat<T>::operator+=(const DualQuat<T> &q) {
    *this = *this + q;
    return *this;
}
...
template <typename T> inline DualQuat<T> operator+(const T a, const DualQuat<T> &q) {
    return DualQuat<T>(a + q.w, q.x, q.y, q.z, q.w_, q.x_, q.y_, q.z_);
}

template <typename T> inline DualQuat<T> operator+(const DualQuat<T> &q, const T a) {
    return DualQuat<T>(a + q.w, q.x, q.y, q.z, q.w_, q.x_, q.y_, q.z_);
}
...
template <typename T> inline DualQuat<T> DualQuat<T>::operator/(const T a) const {
...
template <typename T> std::ostream &operator<<(std::ostream &os, const DualQuat<T> &q) {
...
template <typename T> DualQuat<T> log(const DualQuat<T> &dq, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    return dq.log(assumeUnit);
}

template <typename T> DualQuat<T> DualQuat<T>::log(QuatAssumeType assumeUnit) const {
...
template <typename T> inline DualQuat<T> power(const DualQuat<T> &dq, const T t, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    return dq.power(t, assumeUnit);
}

template <typename T> inline DualQuat<T> DualQuat<T>::power(const T t, QuatAssumeType assumeUnit) const {
    return (t * log(assumeUnit)).exp();
}

template <typename T> inline DualQuat<T> power(const DualQuat<T> &p, const DualQuat<T> &q, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    return p.power(q, assumeUnit);
}

template <typename T> inline DualQuat<T> DualQuat<T>::power(const DualQuat<T> &q, QuatAssumeType assumeUnit) const {
...
template <typename T> Affine3<T> DualQuat<T>::toAffine3(QuatAssumeType assumeUnit) const {
    return Affine3<T>(toMat(assumeUnit));
}

template <typename T> Matx<T, 4, 4> DualQuat<T>::toMat(QuatAssumeType assumeUnit) const {
...
template <typename T> DualQuat<T> DualQuat<T>::sclerp(const DualQuat<T> &q0, const DualQuat<T> &q1, const T t, bool directChange, QuatAssumeType assumeUnit) {
...
template <typename T> DualQuat<T> DualQuat<T>::dqblend(const DualQuat<T> &q1, const DualQuat<T> &q2, const T t, QuatAssumeType assumeUnit) {
...
template <typename T> DualQuat<T> DualQuat<T>::gdqblend(InputArray _dualquat, InputArray _weight, QuatAssumeType assumeUnit) {
