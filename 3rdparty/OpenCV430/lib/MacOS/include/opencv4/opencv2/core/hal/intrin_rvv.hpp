...
#define OPENCV_HAL_IMPL_RVV_SELF_REINTERPRET(_Tpvec, suffix)           \
    inline v_##_Tpvec v_reinterpret_as_##suffix(const v_##_Tpvec &v) { \
        return v;                                                      \
    }
...
inline v_int8x16 v_load_halves(const schar *ptr0, const schar *ptr1) {
    schar elems[16] = { ptr0[0], ptr0[1], ptr0[2], ptr0[3], ptr0[4], ptr0[5], ptr0[6], ptr0[7],
...
inline v_int16x8 v_load_halves(const short *ptr0, const short *ptr1) {
    short elems[8] = { ptr0[0], ptr0[1], ptr0[2], ptr0[3], ptr1[0], ptr1[1], ptr1[2], ptr1[3] };
...
inline v_int8x16 v_lut(const schar *tab, const int *idx) {
    schar elems[16] = { tab[idx[0]], tab[idx[1]], tab[idx[2]],  tab[idx[3]],  tab[idx[4]],  tab[idx[5]],  tab[idx[6]],  tab[idx[7]],
                        tab[idx[8]], tab[idx[9]], tab[idx[10]], tab[idx[11]], tab[idx[12]], tab[idx[13]], tab[idx[14]], tab[idx[15]] };
    return v_int8x16(vle8_v_i8m1(elems, 16));
}
inline v_int8x16 v_lut_pairs(const schar *tab, const int *idx) {
    schar elems[16] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[1]], tab[idx[1] + 1], tab[idx[2]], tab[idx[2] + 1], tab[idx[3]], tab[idx[3] + 1],
                        tab[idx[4]], tab[idx[4] + 1], tab[idx[5]], tab[idx[5] + 1], tab[idx[6]], tab[idx[6] + 1], tab[idx[7]], tab[idx[7] + 1] };
    return v_int8x16(vle8_v_i8m1(elems, 16));
}
inline v_int8x16 v_lut_quads(const schar *tab, const int *idx) {
    schar elems[16] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[0] + 2], tab[idx[0] + 3], tab[idx[1]], tab[idx[1] + 1], tab[idx[1] + 2], tab[idx[1] + 3],
...
inline v_int16x8 v_lut(const short *tab, const int *idx) {
    short elems[8] = { tab[idx[0]], tab[idx[1]], tab[idx[2]], tab[idx[3]], tab[idx[4]], tab[idx[5]], tab[idx[6]], tab[idx[7]] };
    return v_int16x8(vle16_v_i16m1(elems, 8));
}
inline v_int16x8 v_lut_pairs(const short *tab, const int *idx) {
    short elems[8] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[1]], tab[idx[1] + 1], tab[idx[2]], tab[idx[2] + 1], tab[idx[3]], tab[idx[3] + 1] };
    return v_int16x8(vle16_v_i16m1(elems, 8));
}
inline v_int16x8 v_lut_quads(const short *tab, const int *idx) {
    short elems[8] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[0] + 2], tab[idx[0] + 3], tab[idx[1]], tab[idx[1] + 1], tab[idx[1] + 2], tab[idx[1] + 3] };
    return v_int16x8(vle16_v_i16m1(elems, 8));
}
...
inline v_int32x4 v_lut(const int *tab, const int *idx) {
    int elems[4] = { tab[idx[0]], tab[idx[1]], tab[idx[2]], tab[idx[3]] };
    return v_int32x4(vle32_v_i32m1(elems, 4));
}
inline v_int32x4 v_lut_pairs(const int *tab, const int *idx) {
    int elems[4] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[1]], tab[idx[1] + 1] };
    return v_int32x4(vle32_v_i32m1(elems, 4));
}
...
inline v_int64x2 v_lut(const int64_t *tab, const int *idx) {
    int64_t elems[2] = { tab[idx[0]], tab[idx[1]] };
    return v_int64x2(vle64_v_i64m1(elems, 2));
}
...
inline v_float32x4 v_lut(const float *tab, const int *idx) {
    float elems[4] = { tab[idx[0]], tab[idx[1]], tab[idx[2]], tab[idx[3]] };
    return v_float32x4(vle32_v_f32m1(elems, 4));
}
inline v_float32x4 v_lut_pairs(const float *tab, const int *idx) {
    float elems[4] = { tab[idx[0]], tab[idx[0] + 1], tab[idx[1]], tab[idx[1] + 1] };
    return v_float32x4(vle32_v_f32m1(elems, 4));
}
...
inline v_int32x4 v_lut(const int *tab, const v_int32x4 &idxvec) {
    int elems[4] = { tab[v_extract_n<0>(idxvec)], tab[v_extract_n<1>(idxvec)], tab[v_extract_n<2>(idxvec)], tab[v_extract_n<3>(idxvec)] };
    return v_int32x4(vle32_v_i32m1(elems, 4));
}

inline v_uint32x4 v_lut(const unsigned *tab, const v_int32x4 &idxvec) {
    unsigned elems[4] = { tab[v_extract_n<0>(idxvec)], tab[v_extract_n<1>(idxvec)], tab[v_extract_n<2>(idxvec)], tab[v_extract_n<3>(idxvec)] };
    return v_uint32x4(vle32_v_u32m1(elems, 4));
}

inline v_float32x4 v_lut(const float *tab, const v_int32x4 &idxvec) {
    float elems[4] = { tab[v_extract_n<0>(idxvec)], tab[v_extract_n<1>(idxvec)], tab[v_extract_n<2>(idxvec)], tab[v_extract_n<3>(idxvec)] };
    return v_float32x4(vle32_v_f32m1(elems, 4));
}
...
inline v_float64x2 v_lut(const double *tab, const int *idx) {
    double elems[2] = { tab[idx[0]], tab[idx[1]] };
    return v_float64x2(vle64_v_f64m1(elems, 2));
}
...
inline v_float64x2 v_lut(const double *tab, const v_int32x4 &idxvec) {
    double elems[2] = { tab[v_extract_n<0>(idxvec)], tab[v_extract_n<1>(idxvec)] };
    return v_float64x2(vle64_v_f64m1(elems, 2));
}
...
inline v_float32x4 v_reduce_sum4(const v_float32x4 &a, const v_float32x4 &b, const v_float32x4 &c, const v_float32x4 &d) {
    float elems[4] = { v_reduce_sum(a), v_reduce_sum(b), v_reduce_sum(c), v_reduce_sum(d) };
    return v_float32x4(vle32_v_f32m1(elems, 4));
}
...
#define OPENCV_HAL_IMPL_RVV_TRANSPOSE4x4(_Tpvec, _Tp, suffix)                                                                                          \
    inline void v_transpose4x4(const v_##_Tpvec &a0, const v_##_Tpvec &a1, const v_##_Tpvec &a2, const v_##_Tpvec &a3, v_##_Tpvec &b0, v_##_Tpvec &b1, \
        ...
        _Tp elems0[4] = { v_extract_n<0>(a0), v_extract_n<0>(a1), v_extract_n<0>(a2), v_extract_n<0>(a3) };                                            \
        b0            = v_load(elems0);                                                                                                                \
        _Tp elems1[4] = { v_extract_n<1>(a0), v_extract_n<1>(a1), v_extract_n<1>(a2), v_extract_n<1>(a3) };                                            \
        b1            = v_load(elems1);                                                                                                                \
        _Tp elems2[4] = { v_extract_n<2>(a0), v_extract_n<2>(a1), v_extract_n<2>(a2), v_extract_n<2>(a3) };                                            \
        b2            = v_load(elems2);                                                                                                                \
        _Tp elems3[4] = { v_extract_n<3>(a0), v_extract_n<3>(a1), v_extract_n<3>(a2), v_extract_n<3>(a3) };                                            \
        b3            = v_load(elems3);                                                                                                                \
    }

OPENCV_HAL_IMPL_RVV_TRANSPOSE4x4(uint32x4, unsigned, u32) OPENCV_HAL_IMPL_RVV_TRANSPOSE4x4(int32x4, int, i32)
...
#define OPENCV_HAL_IMPL_RVV_REVERSE(_Tpvec, _Tp, suffix) \
    inline _Tpvec v_reverse(const _Tpvec &a) {           \
        ...
        OPENCV_HAL_IMPL_RVV_REVERSE(v_uint8x16, uchar, u8) OPENCV_HAL_IMPL_RVV_REVERSE(v_int8x16, schar, i8)
            OPENCV_HAL_IMPL_RVV_REVERSE(v_uint16x8, ushort, u16) OPENCV_HAL_IMPL_RVV_REVERSE(v_int16x8, short, i16)
                OPENCV_HAL_IMPL_RVV_REVERSE(v_uint32x4, unsigned, u32) OPENCV_HAL_IMPL_RVV_REVERSE(v_int32x4, int, i32)
                    OPENCV_HAL_IMPL_RVV_REVERSE(v_float32x4, float, f32) OPENCV_HAL_IMPL_RVV_REVERSE(v_uint64x2, uint64, u64)
...
#define OPENCV_HAL_IMPL_RVV_PACK(_Tpvec, _Tp, _wTpvec, _wTp, hwidth, width, hsuffix, suffix, rshr, shr, hvl, vl) \
    ...
    template <int n> inline _Tpvec v_rshr_pack(const _wTpvec &a, const _wTpvec &b) {                             \
        _wTp arr[_Tpvec::nlanes] = { 0 };                                                                        \
        v_store(arr, a);                                                                                         \
        v_store(arr + _wTpvec::nlanes, b);                                                                       \
        return _Tpvec(rshr(vle##width##_v_##suffix##m2(arr, vl), n, vl));                                        \
    }                                                                                                            \
    template <int n> inline void v_rshr_pack_store(_Tp *ptr, const _wTpvec &a) {                                 \
        _wTp arr[_Tpvec::nlanes] = { 0 };                                                                        \
        v_store(arr, a);                                                                                         \
        v_store(arr + _wTpvec::nlanes, _wTpvec(vmv_v_x_##suffix##m1(0, hvl)));                                   \
...
#define OPENCV_HAL_IMPL_RVV_PACK_U(_Tpvec, _Tp, _wTpvec, _wTp, hwidth, width, hsuffix, suffix, rshr, cast, hvl, vl)                    \
    ...
    template <int n> inline _Tpvec v_rshr_pack_u(const _wTpvec &a, const _wTpvec &b) {                                                 \
        _wTp arr[_Tpvec::nlanes] = { 0 };                                                                                              \
        v_store(arr, a);                                                                                                               \
        v_store(arr + _wTpvec::nlanes, b);                                                                                             \
        return _Tpvec(rshr(cast(vmax_vx_##suffix##m2(vle##width##_v_##suffix##m2(arr, vl), 0, vl)), n, vl));                           \
    }                                                                                                                                  \
    template <int n> inline void v_rshr_pack_u_store(_Tp *ptr, const _wTpvec &a) {                                                     \
        _wTp arr[_Tpvec::nlanes] = { 0 };                                                                                              \
        v_store(arr, a);                                                                                                               \
        v_store(arr + _wTpvec::nlanes, _wTpvec(vmv_v_x_##suffix##m1(0, hvl)));                                                         \
