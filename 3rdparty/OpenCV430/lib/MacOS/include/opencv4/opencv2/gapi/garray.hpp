...
GAPI_EXPORTS_W inline GArrayDesc empty_array_desc() {
    return {};
}
/** @} */
...
template <typename T> void GArrayU::storeKind() {
...
public:
    VectorRef() = default;
    template <typename T> explicit VectorRef(const std::vector<T> &vec) : m_ref(new VectorRefT<T>(vec)), m_kind(GOpaqueTraits<T>::kind) {}
    template <typename T> explicit VectorRef(std::vector<T> &vec) : m_ref(new VectorRefT<T>(vec)), m_kind(GOpaqueTraits<T>::kind) {}
    template <typename T> explicit VectorRef(std::vector<T> &&vec) : m_ref(new VectorRefT<T>(std::move(vec))), m_kind(GOpaqueTraits<T>::kind) {}
...
template <class T> struct flatten_g<GArray<T>> {
...
public:
    ...
    explicit GArray(const std::vector<HT> &v)  // Constant value constructor
        : m_ref(detail::GArrayU(detail::VectorRef(v))) {
        putDetails();
    }
    ...
    explicit GArray(std::vector<HT> &&v)  // Move-constructor
        : m_ref(detail::GArrayU(detail::VectorRef(std::move(v)))) {
        putDetails();
    }
