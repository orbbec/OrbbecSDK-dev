...
template <typename U> GOpaqueDesc descr_of(const U &) {
    return {};
}
GAPI_EXPORTS_W inline GOpaqueDesc empty_gopaque_desc() {
    return {};
}
/** @} */
...
public:
    ...
    template <typename T> bool holds() const;  // Check if was created from GOpaque<T>
...
protected:
    ...
    template <typename T> void specifyType();  // Store type of initial GOpaque<T>

    template <typename T> void storeKind();
...
template <typename T> bool GOpaqueU::holds() const {
    GAPI_Assert(m_hint != nullptr);
    using U = util::decay_t<T>;
    return dynamic_cast<TypeHint<U> *>(m_hint.get()) != nullptr;
};

template <typename T> void GOpaqueU::specifyType() {
    m_hint.reset(new TypeHint<util::decay_t<T>>);
};

template <typename T> void GOpaqueU::storeKind() {
...
public:
    ...
    explicit OpaqueRefT(const T &obj) : m_ref(&obj) {
        init(&obj);
    }
    explicit OpaqueRefT(T &obj) : m_ref(&obj) {
        init(&obj);
    }
    explicit OpaqueRefT(T &&obj) : m_ref(std::move(obj)) {
        init(&obj);
    }
    ...
    void reset() {
        if(isEmpty()) {
            ...
            GAPI_Error("InternalError");  // shouldn't be called in *EXT modes
    }
...
public:
    ...
    explicit OpaqueRef(T &&obj) : m_ref(new OpaqueRefT<util::decay_t<T>>(std::forward<T>(obj))), m_kind(GOpaqueTraits<util::decay_t<T>>::kind) {}
