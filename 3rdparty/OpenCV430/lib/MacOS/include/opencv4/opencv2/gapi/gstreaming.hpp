...
template <typename RefHolder> struct OptRef {
    struct OptHolder {
    ...
    template <class T> explicit OptRef(cv::optional<T> &t) : m_opt{ new Holder<T>(t) } {}
...
using GOptRunArgP  = util::variant<optional<cv::Mat> *, optional<cv::RMat> *, optional<cv::MediaFrame> *, optional<cv::Scalar> *, cv::detail::OptionalVectorRef,
                                  cv::detail::OptionalOpaqueRef>;
using GOptRunArgsP = std::vector<GOptRunArgP>;

using GOptRunArg  = util::variant<optional<cv::Mat>, optional<cv::RMat>, optional<cv::MediaFrame>, optional<cv::Scalar>, optional<cv::detail::VectorRef>,
...
template <typename T, typename... Ts> inline GOptRunArgsP gout(optional<T> &arg, optional<Ts> &...args) {
...
public:
     ...
     * source.
     */
    template <typename T, typename... Args> void setSource(Args &&...args) {
