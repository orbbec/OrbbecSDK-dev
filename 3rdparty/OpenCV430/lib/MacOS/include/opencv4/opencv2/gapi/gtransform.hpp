...
template <typename K, typename... Ins, typename Out> struct TransHelper<K, std::tuple<Ins...>, Out> {
    template <typename Callable, int... IIs, int... OIs> static GComputation invoke(Callable f, Seq<IIs...>, Seq<OIs...>) {
...
class GTransformImpl<K, std::function<R(Args...)>> : public cv::detail::TransHelper<K, std::tuple<Args...>, R>, public cv::detail::TransformTag {
public:
