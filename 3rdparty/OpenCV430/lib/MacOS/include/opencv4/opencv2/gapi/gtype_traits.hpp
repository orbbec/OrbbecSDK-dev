...
namespace cv {
namespace detail {
template <typename, typename = void> struct contains_shape_field : std::false_type {};
...
template <typename Type> struct has_gshape : contains_shape_field<Type> {};
...
template <> struct GTypeOf<cv::RMat> {
    using type = cv::GMat;
};
...
template <typename U> struct GTypeOf {
    using type = cv::GOpaque<U>;
};
template <> struct GTypeOf<cv::MediaFrame> {
...
template <typename T> struct WrapValue<T, typename std::enable_if<has_custom_wrap<T>::value>::type> {
    ...
    template <typename U> static auto wrap_in(const U &u) -> typename GTypeTraits<T>::strip_type {
        static_assert(!(cv::detail::has_gshape<GTypeTraits<U>>::value || cv::detail::contains<typename std::decay<U>::type, GAPI_OWN_TYPES_LIST>::value),
...
template <typename T> struct GObtainCtor<GArray<T>> {
    static HostCtor get() {
        return HostCtor{ ConstructVec{ &GArray<T>::VCtor } };
    };
};
template <typename T> struct GObtainCtor<GOpaque<T>> {
    static HostCtor get() {
        return HostCtor{ ConstructOpaque{ &GOpaque<T>::Ctor } };
    };
};
}  // namespace detail
}  // namespace cv
