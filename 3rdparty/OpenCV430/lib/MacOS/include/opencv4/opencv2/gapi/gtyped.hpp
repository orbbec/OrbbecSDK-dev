...
template <> struct ProtoToParam<cv::GArray<cv::GMat>> {
    using type = std::vector<cv::Mat>;
};
template <typename U> struct ProtoToParam<cv::GOpaque<U>> {
    using type = U;
};
...
template <typename U> struct ProtoToMeta<cv::GOpaque<U>> {
...
public:
    GComputationT(const Gen &generator)
        : m_capture(capture(generator, detail::make_default<Args>()...)), m_comp(cv::GProtoInputArgs(std::move(m_capture.second)), cv::GOut(m_capture.first)) {}
