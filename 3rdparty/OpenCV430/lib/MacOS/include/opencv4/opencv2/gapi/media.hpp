...
public:
    ...
    template <typename T> T *get() const {
        static_assert(std::is_base_of<IAdapter, T>::value, "T is not derived from cv::MediaFrame::IAdapter!");
...
template <class T, class... Args> inline cv::MediaFrame cv::MediaFrame::Create(Args &&...args) {
...
public:
    ...
    /// @private
    View(
        Ptrs &&ptrs, Strides &&strs, Callback &&cb = []() {});
