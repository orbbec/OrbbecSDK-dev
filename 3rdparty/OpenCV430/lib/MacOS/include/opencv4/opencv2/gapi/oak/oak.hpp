...
G_API_OP(GEncFrame, <GArray<uint8_t>(GFrame, EncoderConfig)>,
         "org.opencv.oak.enc_frame"){ static GArrayDesc outMeta(const GFrameDesc &, const EncoderConfig &){ return cv::empty_array_desc();
}  // namespace oak
};  // namespace gapi

G_API_OP(GSobelXY, <GFrame(GFrame, const cv::Mat &, const cv::Mat &)>,
         "org.opencv.oak.sobelxy"){ static GFrameDesc outMeta(const GFrameDesc &in, const cv::Mat &, const cv::Mat &){ return in;
}  // namespace cv
}
;

G_API_OP(GCopy, <GFrame(GFrame)>, "org.opencv.oak.copy"){ static GFrameDesc outMeta(const GFrameDesc &in){ return in;
}
}
;
...
GAPI_EXPORTS GFrame sobelXY(const GFrame &in, const cv::Mat &hk, const cv::Mat &vk);
...
template <> struct CompileArgTag<gapi::oak::ColorCameraParams> {
    static const char *tag() {
        return "gapi.oak.colorCameraParams";
    }
};

template <> struct CompileArgTag<gapi::oak::EncoderConfig> {
    static const char *tag() {
        return "gapi.oak.encoderConfig";
    }
};
}  // namespace detail
