...
template <typename DST, typename SRC,
          typename = cv::util::enable_if_t<!std::is_same<DST, SRC>::value && std::is_integral<DST>::value && std::is_integral<SRC>::value>>
...
template <typename T> static CV_ALWAYS_INLINE T saturate(T x) {
    return x;
}
...
template <typename DST, typename SRC, typename R, cv::util::enable_if_t<std::is_integral<DST>::value && std::is_integral<SRC>::value, bool> = true>
static CV_ALWAYS_INLINE DST saturate(SRC x, R) {
...
template <typename DST, typename SRC, typename R, cv::util::enable_if_t<std::is_integral<DST>::value && std::is_floating_point<SRC>::value, bool> = true>
