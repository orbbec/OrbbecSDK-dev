...
template <typename... Types> cv::GCompileArgs getCompileArgs(const std::vector<char> &bytes);

template <typename... AdapterType> cv::GRunArgs getRunArgsWithAdapters(const std::vector<char> &bytes);
}  // namespace detail
 ...
 * @private
 */
template <typename T> static inline T deserialize(const std::vector<char> &bytes);
 ...
 */
template <> inline cv::GComputation deserialize(const std::vector<char> &bytes) {
 ...
 */
template <> inline cv::GMetaArgs deserialize(const std::vector<char> &bytes) {
 ...
 */
template <> inline cv::GRunArgs deserialize(const std::vector<char> &bytes) {
 ...
 */
template <> inline std::vector<std::string> deserialize(const std::vector<char> &bytes) {
...
inline typename std::enable_if<std::is_same<T, GCompileArgs>::value, GCompileArgs>::type deserialize(const std::vector<char> &bytes) {
...
inline typename std::enable_if<std::is_same<T, GRunArgs>::value, GRunArgs>::type deserialize(const std::vector<char> &bytes) {
...
// Generic STL types ////////////////////////////////////////////////////////////////
template <typename K, typename V> IOStream &operator<<(IOStream &os, const std::map<K, V> &m) {
...
template <typename K, typename V> IIStream &operator>>(IIStream &is, std::map<K, V> &m) {
    m.clear();
    uint32_t sz = 0u;
    is >> sz;
...
template <typename K, typename V> IOStream &operator<<(IOStream &os, const std::unordered_map<K, V> &m) {
    const uint32_t sz = static_cast<uint32_t>(m.size());
    os << sz;
    for(auto &&it: m)
        os << it.first << it.second;
...
template <typename K, typename V> IIStream &operator>>(IIStream &is, std::unordered_map<K, V> &m) {
    m.clear();
    uint32_t sz = 0u;
    is >> sz;
    for(std::size_t i = 0; i < sz; ++i) {
...
template <typename T> IOStream &operator<<(IOStream &os, const std::vector<T> &ts) {
...
template <typename T> IIStream &operator>>(IIStream &is, std::vector<T> &ts) {
    uint32_t sz = 0u;
    is >> sz;
    if(sz == 0u) {
        ts.clear();
...
namespace detail {
template <typename V> IOStream &put_v(IOStream &, const V &, std::size_t) {
    GAPI_Error("variant>>: requested index is invalid");
};

template <typename V, typename X, typename... Xs> IOStream &put_v(IOStream &os, const V &v, std::size_t x) {
    return (x == 0u) ? os << cv::util::get<X>(v) : put_v<V, Xs...>(os, v, x - 1);
}

template <typename V> IIStream &get_v(IIStream &, V &, std::size_t, std::size_t) {
    GAPI_Error("variant<<: requested index is invalid");
}

template <typename V, typename X, typename... Xs> IIStream &get_v(IIStream &is, V &v, std::size_t i, std::size_t gi) {
    if(i == gi) {
        X x{};
        is >> x;
        v = V{ std::move(x) };
        ...
        return get_v<V, Xs...>(is, v, i + 1, gi);
}
}  // namespace detail

//! @overload
template <typename... Ts> IOStream &operator<<(IOStream &os, const cv::util::variant<Ts...> &v) {
    os << static_cast<uint32_t>(v.index());
    return detail::put_v<cv::util::variant<Ts...>, Ts...>(os, v, v.index());
}
//! @overload
template <typename... Ts> IIStream &operator>>(IIStream &is, cv::util::variant<Ts...> &v) {
    int idx = -1;
    is >> idx;
...
/// @private -- Exclude this function from OpenCV documentation
template <typename... Ts> void getRunArgByIdx(IIStream &is, cv::util::variant<Ts...> &v, uint32_t idx) {
...
template <typename T, typename... Types> struct try_deserialize_comparg<std::tuple<T, Types...>> {
    static cv::util::optional<GCompileArg> exec(const std::string &tag, cv::gapi::s11n::IIStream &is) {
        if(tag == cv::detail::CompileArgTag<T>::tag()) {
            static_assert(cv::gapi::s11n::detail::has_S11N_spec<T>::value, "cv::gapi::deserialize<GCompileArgs, Types...> expects Types to have S11N "
                                                                           "specializations with deserialization callbacks!");
            return cv::util::optional<GCompileArg>(GCompileArg{ cv::gapi::s11n::detail::S11N<T>::deserialize(is) });
        }
        return try_deserialize_comparg<std::tuple<Types...>>::exec(tag, is);
    }
};

template <typename... T> struct deserialize_arg_with_adapter;

template <typename RA, typename TA> struct deserialize_arg_with_adapter<RA, TA> {
...
template <typename RA> struct deserialize_arg_with_adapter<RA, void> {
...
template <typename... Types> struct deserialize_runarg {
...
template <typename... Types> inline cv::util::optional<GCompileArg> tryDeserializeCompArg(const std::string &tag, const std::vector<char> &sArg) {
...
template <typename... Types> cv::GCompileArgs getCompileArgs(const std::vector<char> &sArgs) {
    ...
    for(uint32_t i = 0; i < sz; ++i) {
        ...
        cv::util::optional<GCompileArg> dArg = cv::gapi::detail::tryDeserializeCompArg<Types...>(tag, sArg);
...
template <typename... AdapterTypes> cv::GRunArgs getRunArgsWithAdapters(const std::vector<char> &bytes) {
