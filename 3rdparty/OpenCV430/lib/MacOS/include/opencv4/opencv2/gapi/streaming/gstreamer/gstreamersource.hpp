...
public:
    ...
    GStreamerSource(std::shared_ptr<GStreamerPipelineFacade> pipeline, const std::string &appsinkName,
...
GAPI_EXPORTS_W cv::Ptr<IStreamSource> inline make_gst_src(const std::string                &pipeline,
                                                          const GStreamerSource::OutputType outputType = GStreamerSource::OutputType::MAT) {
