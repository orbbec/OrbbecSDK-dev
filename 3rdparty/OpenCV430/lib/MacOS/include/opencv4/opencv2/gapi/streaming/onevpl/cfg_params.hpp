 ...
 * Please get on
 * https://spec.oneapi.io/versions/latest/elements/oneVPL/source/API_ref/VPL_disp_api_func.html?highlight=mfxcreateconfig#mfxsetconfigfilterproperty for using
 * OneVPL configuration. In this schema `mfxU8 *name` represents @ref CfgParam::get_name() and `mfxVariant value` is @ref CfgParam::get_value()
 */
struct GAPI_EXPORTS CfgParam {
    ...
    static constexpr const char *acceleration_mode_name() {
        return "mfxImplDescription.AccelerationMode";
    }
    ...
    static constexpr const char *decoder_id_name() {
        return "mfxImplDescription.mfxDecoderDescription.decoder.CodecID";
    }
    ...
    static constexpr const char *implementation_name() {
        return "mfxImplDescription.Impl";
    }
     ...
     *@param is_major       TRUE if parameter MUST be provided by OneVPL inner implementation, FALSE for optional (for resolve multiple available
     *implementations).
     *
     */
    template <typename ValueType> static CfgParam create(const std::string &name, ValueType &&value, bool is_major = true) {
