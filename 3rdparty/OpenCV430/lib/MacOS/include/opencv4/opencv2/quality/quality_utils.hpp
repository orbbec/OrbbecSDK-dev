...
template <typename R> inline R extract_mat(InputArray in, const int type = -1) {
    R result = {};
    if(in.isMat())
...
template <typename R> inline R expand_mat(InputArray src, int TYPE_DEFAULT = EXPANDED_MAT_DEFAULT_TYPE) {
...
inline cv::Mat get_column_range(const cv::Mat &data) {
    ...
    auto row_min = result.row(0), row_max = result.row(1);
...
template <typename T> inline void scale(cv::Mat &mat, const cv::Mat &range, const T min, const T max) {
