...
*/
class CV_EXPORTS_W QualityGMSD : public QualityBase {
public:
    /**
    @brief Compute GMSD
    ...
    */
    ...
    CV_WRAP bool empty() const CV_OVERRIDE {
        return _refImgData.empty() && QualityBase::empty();
    }
...
protected:
    ...
    struct _mat_data {
        ...
        mat_type gradient_map, gradient_map_squared;
        ...
        bool empty() const {
            return this->gradient_map.empty() && this->gradient_map_squared.empty();
        }
    ...
    QualityGMSD(_mat_data refImgData) : _refImgData(std::move(refImgData)) {}
