...
*/
class CV_EXPORTS_W QualityPSNR : public QualityBase {
...
public:
    ...
    */
    CV_WRAP cv::Scalar compute(InputArray cmp) CV_OVERRIDE {
        ...
        return _mse_to_psnr(result, _maxPixelValue);
    }

    /** @brief Implements Algorithm::empty()  */
    CV_WRAP bool empty() const CV_OVERRIDE {
        return _qualityMSE->empty() && QualityBase::empty();
    }
    ...
    */
    CV_WRAP static cv::Scalar compute(InputArray ref, InputArray cmp, OutputArray qualityMap, double maxPixelValue = QualityPSNR::MAX_PIXEL_VALUE_DEFAULT) {
        return _mse_to_psnr(QualityMSE::compute(ref, cmp, qualityMap), maxPixelValue);
    }
...
protected:
    ...
    /** @brief Constructor */
    QualityPSNR(Ptr<QualityMSE> qualityMSE, double maxPixelValue) : _qualityMSE(std::move(qualityMSE)), _maxPixelValue(maxPixelValue) {}

    // convert mse to psnr
    static double _mse_to_psnr(double mse, double max_pixel_value) {
        return (mse == 0.) ? std::numeric_limits<double>::infinity() : 10. * std::log10((max_pixel_value * max_pixel_value) / mse);
    }
