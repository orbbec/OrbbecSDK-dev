...
inline bool isValidDepth(const unsigned short int &depth) {
    return (depth != std::numeric_limits<unsigned short int>::min()) && (depth != std::numeric_limits<unsigned short int>::max());
}
CV_EXPORTS
...
public:
    ...
    RgbdNormals() : rows_(0), cols_(0), depth_(0), K_(Mat()), window_size_(0), method_(RGBD_NORMALS_METHOD_FALS), rgbd_normals_impl_(0) {}
    ...
    CV_WRAP static Ptr<RgbdNormals> create(int rows, int cols, int depth, InputArray K, int window_size = 5,
     ...
     */
    CV_WRAP_AS(apply) void operator()(InputArray points, OutputArray normals) const;
...
public:
    ...
    DepthCleaner() : depth_(0), window_size_(0), method_(DEPTH_CLEANER_NIL), depth_cleaner_impl_(0) {}
     ...
     */
    CV_WRAP_AS(apply) void operator()(InputArray points, OutputArray depth) const;
 ...
 */
CV_EXPORTS_W
void registerDepth(InputArray unregisteredCameraMatrix, InputArray registeredCameraMatrix, InputArray registeredDistCoeffs, InputArray Rt,
                   InputArray unregisteredDepth, const Size &outputImagePlaneSize, OutputArray registeredDepth, bool depthDilation = false);
...
public:
    ...
    CV_WRAP static Ptr<RgbdPlane> create(int method, int block_size, int min_size, double threshold, double sensor_error_a = 0, double sensor_error_b = 0,
     ...
     */
    CV_WRAP_AS(apply) void operator()(InputArray points3d, InputArray normals, OutputArray mask, OutputArray plane_coefficients);
     ...
     */
    CV_WRAP_AS(apply) void operator()(InputArray points3d, OutputArray mask, OutputArray plane_coefficients);
...
struct CV_EXPORTS_W OdometryFrame : public RgbdFrame {
    ...
    CV_WRAP static Ptr<OdometryFrame> create(const Mat &image = Mat(), const Mat &depth = Mat(), const Mat &mask = Mat(), const Mat &normals = Mat(),
                                             int ID = -1);

    CV_WRAP virtual void release() CV_OVERRIDE;
...
public:
    ...
    CV_WRAP static inline float DEFAULT_MIN_DEPTH() {
        return 0.f;  // in meters
    }
    CV_WRAP static inline float DEFAULT_MAX_DEPTH() {
        return 4.f;  // in meters
    }
    CV_WRAP static inline float DEFAULT_MAX_DEPTH_DIFF() {
        return 0.07f;  // in meters
    }
    CV_WRAP static inline float DEFAULT_MAX_POINTS_PART() {
        return 0.07f;  // in [0, 1]
    }
    CV_WRAP static inline float DEFAULT_MAX_TRANSLATION() {
        return 0.15f;  // in meters
    }
    CV_WRAP static inline float DEFAULT_MAX_ROTATION() {
     ...
     */
    CV_WRAP bool compute(const Mat &srcImage, const Mat &srcDepth, const Mat &srcMask, const Mat &dstImage, const Mat &dstDepth, const Mat &dstMask,
                         OutputArray Rt, const Mat &initRt = Mat()) const;

    /** One more method to compute a transformation from the source frame to the destination one.
     * It is designed to save on computing the frame data (image pyramids, normals, etc.).
     */
    CV_WRAP_AS(compute2) bool compute(Ptr<OdometryFrame> &srcFrame, Ptr<OdometryFrame> &dstFrame, OutputArray Rt, const Mat &initRt = Mat()) const;
...
public:
    ...
    CV_WRAP static Ptr<RgbdOdometry> create(const Mat &cameraMatrix = Mat(), float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
                                            float maxDepth = Odometry::DEFAULT_MAX_DEPTH(), float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
                                            const std::vector<int>   &iterCounts            = std::vector<int>(),
                                            const std::vector<float> &minGradientMagnitudes = std::vector<float>(),
                                            float maxPointsPart = Odometry::DEFAULT_MAX_POINTS_PART(), int transformType = Odometry::RIGID_BODY_MOTION);
...
public:
    ...
    CV_WRAP static Ptr<ICPOdometry> create(const Mat &cameraMatrix = Mat(), float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
                                           float maxDepth = Odometry::DEFAULT_MAX_DEPTH(), float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
                                           float maxPointsPart = Odometry::DEFAULT_MAX_POINTS_PART(), const std::vector<int> &iterCounts = std::vector<int>(),
                                           int transformType = Odometry::RIGID_BODY_MOTION);
...
public:
    ...
    RgbdICPOdometry(const Mat &cameraMatrix, float minDepth = Odometry::DEFAULT_MIN_DEPTH(), float maxDepth = Odometry::DEFAULT_MAX_DEPTH(),
                    float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(), float maxPointsPart = Odometry::DEFAULT_MAX_POINTS_PART(),
                    const std::vector<int> &iterCounts = std::vector<int>(), const std::vector<float> &minGradientMagnitudes = std::vector<float>(),
                    int transformType = Odometry::RIGID_BODY_MOTION);

    CV_WRAP static Ptr<RgbdICPOdometry> create(const Mat &cameraMatrix = Mat(), float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
                                               float maxDepth = Odometry::DEFAULT_MAX_DEPTH(), float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
...
public:
     ...
     * @param iterCounts Count of iterations on each pyramid level
     */
    FastICPOdometry(const Mat &cameraMatrix, float maxDistDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(), float angleThreshold = (float)(30. * CV_PI / 180.),
                    float sigmaDepth = 0.04f, float sigmaSpatial = 4.5f, int kernelSize = 7, const std::vector<int> &iterCounts = std::vector<int>());

    CV_WRAP static Ptr<FastICPOdometry> create(const Mat &cameraMatrix, float maxDistDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
                                               float angleThreshold = (float)(30. * CV_PI / 180.), float sigmaDepth = 0.04f, float sigmaSpatial = 4.5f,
                                               int kernelSize = 7, const std::vector<int> &iterCounts = std::vector<int>());
 ...
 */
CV_EXPORTS_W
void warpFrame(const Mat &image, const Mat &depth, const Mat &mask, const Mat &Rt, const Mat &cameraMatrix, const Mat &distCoeff, OutputArray warpedImage,
               OutputArray warpedDepth = noArray(), OutputArray warpedMask = noArray());
