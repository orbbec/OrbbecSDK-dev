...
public:
    ...
    CV_WRAP static Ptr<DepthNormal> create(int distance_threshold, int difference_threshold, size_t num_features, int extract_threshold);
...
public:
    ...
    CV_WRAP void match(const std::vector<Mat> &sources, float threshold, CV_OUT std::vector<Match> &matches,
                       const std::vector<String> &class_ids = std::vector<String>(), OutputArrayOfArrays quantized_images = noArray(),
...
protected:
    ...
    void matchClass(const LinearMemoryPyramid &lm_pyramid, const std::vector<Size> &sizes, float threshold, std::vector<Match> &matches, const String &class_id,
