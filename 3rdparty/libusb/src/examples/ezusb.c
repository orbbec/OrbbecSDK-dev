...
static int ezusb_write(libusb_device_handle *device, const char *label, uint8_t opcode, uint32_t addr, const unsigned char *data, size_t len) {
    ...
    status = libusb_control_transfer(device, LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE, opcode, addr & 0xFFFF, addr >> 16,
...
static int ezusb_read(libusb_device_handle *device, const char *label, uint8_t opcode, uint32_t addr, const unsigned char *data, size_t len) {
    ...
    if(verbose > 1)
        logerror("%s, addr 0x%08x len %4u (0x%04x)\n", label, addr, (unsigned)len, (unsigned)len);
    status = libusb_control_transfer(device, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE, opcode, addr & 0xFFFF, addr >> 16,
                                     (unsigned char *)data, (uint16_t)len, 1000);
    if(status != (signed)len) {
        if(status < 0)
            logerror("%s: %s\n", label, libusb_error_name(status));
...
static bool ezusb_cpucs(libusb_device_handle *device, uint32_t addr, bool doRun) {
    ...
    status = libusb_control_transfer(device, LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE, RW_INTERNAL, addr & 0xFFFF, addr >> 16,
                                     &data, 1, 1000);
    if((status != 1) &&
 ...
 */
static int parse_ihex(FILE *image, void *context, bool (*is_external)(uint32_t addr, size_t len),
...
static int (*parse[IMG_TYPE_MAX])(FILE *image, void *context, bool (*is_external)(uint32_t addr, size_t len),
                                  int (*poke)(void *context, uint32_t addr, bool external, const unsigned char *data, size_t len)) = { parse_ihex, parse_iic,
                                                                                                                                       parse_bin };
...
static int ram_poke(void *context, uint32_t addr, bool external, const unsigned char *data, size_t len) {
     ...
     */
    while((rc = ezusb_write(ctx->device, external ? "write external" : "write on-chip", external ? RW_MEMORY : RW_INTERNAL, addr, data, len)) < 0
          && retry < RETRY_LIMIT) {
...
static int fx3_load_ram(libusb_device_handle *device, const char *path) {
    ...
    if((fread(&dExpectedCheckSum, sizeof(uint32_t), 1, image) != 1) || (dCheckSum != dExpectedCheckSum)) {
...
int ezusb_load_ram(libusb_device_handle *device, const char *path, int fx_type, int img_type, int stage) {
    ...
    if(img_type == IMG_TYPE_IIC) {
        if((fread(iic_header, 1, sizeof(iic_header), image) != sizeof(iic_header))
           || (((fx_type == FX_TYPE_FX2LP) || (fx_type == FX_TYPE_FX2)) && (iic_header[0] != 0xC2)) || ((fx_type == FX_TYPE_AN21) && (iic_header[0] != 0xB2))
