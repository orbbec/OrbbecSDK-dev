...
int usbi_sanitize_device(struct libusb_device *dev) {
    ...
    if(dev->device_descriptor.bLength != LIBUSB_DT_DEVICE_SIZE || dev->device_descriptor.bDescriptorType != LIBUSB_DT_DEVICE) {
...
/** \ingroup libusb_dev
 * Get the path of windows  that a device is connected to.
 *
 * \param dev a device
 * \returns the windows path (0 if not available)
 */
char *API_EXPORTED libusb_get_windows_path(libusb_device *dev) {
    return dev->dev_path;
}
 ...
 */
char *API_EXPORTED libusb_get_interface_path(libusb_device *dev, uint8_t interface_index) {
    return dev->interface_path[interface_index];
}
 ...
 * \param dev a device
 * \param interface_index index of interface
 * \returns the sub composite device id
 */
char *LIBUSB_CALL libusb_get_sub_composite_device_id(libusb_device *dev, uint8_t index) {
    return dev->sub_composite_device_id[index];
}
 ...
 */
int API_EXPORTED libusb_get_port_numbers(libusb_device *dev, uint8_t *port_numbers, int port_numbers_len) {
 ...
 */
int API_EXPORTED libusb_get_port_path(libusb_context *ctx, libusb_device *dev, uint8_t *port_numbers, uint8_t port_numbers_len) {
    UNUSED(ctx);
...
static const struct libusb_endpoint_descriptor *find_endpoint(struct libusb_config_descriptor *config, unsigned char endpoint) {
    int iface_idx;
...
static const struct libusb_endpoint_descriptor *find_alt_endpoint(struct libusb_config_descriptor *config, int iface_idx, int altsetting_idx,
...
static int get_endpoint_max_packet_size(libusb_device *dev, const struct libusb_endpoint_descriptor *ep) {
 ...
 */
int API_EXPORTED libusb_wrap_sys_device(libusb_context *ctx, intptr_t sys_dev, libusb_device_handle **dev_handle) {
...
static void do_close(struct libusb_context *ctx, struct libusb_device_handle *dev_handle) {
    ...
    for_each_transfer_safe(ctx, itransfer, tmp) {
         ...
         */
        usbi_dbg(ctx, "Removed transfer %p from the in-flight list because device handle %p closed", (void *)transfer, (void *)dev_handle);
    }
...
int API_EXPORTED libusb_get_configuration(libusb_device_handle *dev_handle, int *config) {
    ...
    if(r == LIBUSB_ERROR_NOT_SUPPORTED) {
        ...
        r = libusb_control_transfer(dev_handle, LIBUSB_ENDPOINT_IN, LIBUSB_REQUEST_GET_CONFIGURATION, 0, 0, &tmp, 1, 1000);
        if(r == 1) {
            r = 0;
        }
        else if(r == 0) {
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int API_EXPORTED libusb_set_interface_alt_setting(libusb_device_handle *dev_handle, int interface_number, int alternate_setting) {
    usbi_dbg(HANDLE_CTX(dev_handle), "interface %d altsetting %d", interface_number, alternate_setting);
    if(interface_number < 0 || interface_number >= USB_MAXINTERFACES)
        return LIBUSB_ERROR_INVALID_PARAM;
    ...
    return usbi_backend.set_interface_altsetting(dev_handle, (uint8_t)interface_number, (uint8_t)alternate_setting);
}
 ...
 */
int API_EXPORTED libusb_alloc_streams(libusb_device_handle *dev_handle, uint32_t num_streams, unsigned char *endpoints, int num_endpoints) {
 ...
 */
int API_EXPORTED libusb_free_streams(libusb_device_handle *dev_handle, unsigned char *endpoints, int num_endpoints) {
 ...
 * \returns \ref LIBUSB_SUCCESS, or a LIBUSB_ERROR code on failure
 */
int API_EXPORTED libusb_dev_mem_free(libusb_device_handle *dev_handle, unsigned char *buffer, size_t length) {
...
int API_EXPORTED libusb_init_context(libusb_context **ctx, const struct libusb_init_option options[], int num_options) {
    ...
    usbi_dbg(_ctx, "libusb v%u.%u.%u.%u%s", libusb_version_internal.major, libusb_version_internal.minor, libusb_version_internal.micro,
             libusb_version_internal.nano, libusb_version_internal.rc);
...
static void log_v(struct libusb_context *ctx, enum libusb_log_level level, const char *function, const char *format, va_list args) {
    const char *prefix;
    ...
    if(global_debug) {
        ...
        header_len = snprintf(buf, sizeof(buf), "[%2ld.%06ld] [%08x] libusb: %s [%s] ", (long)timestamp.tv_sec, (long)(timestamp.tv_nsec / 1000L),
