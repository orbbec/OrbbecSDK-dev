...
static void calculate_timeout(struct usbi_transfer *itransfer) {
    unsigned int timeout = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer)->timeout;
...
struct libusb_transfer *LIBUSB_CALL libusb_alloc_transfer(int iso_packets) {
    ...
    alloc_size =
        priv_size + sizeof(struct usbi_transfer) + sizeof(struct libusb_transfer) + (sizeof(struct libusb_iso_packet_descriptor) * (size_t)iso_packets);
...
out:
    ...
    if(first && usbi_using_timer(ctx) && TIMESPEC_IS_SET(timeout)) {
        ...
        usbi_dbg(ctx, "arm timer for timeout in %ums (first in line)", USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer)->timeout);
...
static int remove_from_flying_list(struct usbi_transfer *itransfer) {
    ...
    rearm_timer = (TIMESPEC_IS_SET(&itransfer->timeout) && list_first_entry(&ctx->flying_transfers, struct usbi_transfer, list) == itransfer);
    list_del(&itransfer->list);
    if(rearm_timer)
 ...
 */
void API_EXPORTED libusb_transfer_set_stream_id(struct libusb_transfer *transfer, uint32_t stream_id) {
    struct usbi_transfer *itransfer = LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
 ...
 */
uint32_t API_EXPORTED libusb_transfer_get_stream_id(struct libusb_transfer *transfer) {
    struct usbi_transfer *itransfer = LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
...
int usbi_handle_transfer_completion(struct usbi_transfer *itransfer, enum libusb_transfer_status status) {
    ...
    usbi_dbg(ctx, "transfer %p has callback %p", (void *)transfer, transfer->callback);
...
int API_EXPORTED libusb_wait_for_event(libusb_context *ctx, struct timeval *tv) {
    ...
    r = usbi_cond_timedwait(&ctx->event_waiters_cond, &ctx->event_waiters_lock, tv);
    if(r < 0)
...
static void handle_timeout(struct usbi_transfer *itransfer) {
    ...
    if(r == LIBUSB_SUCCESS)
        ...
        usbi_warn(TRANSFER_CTX(transfer), "async cancel failed %d", r);
}
 ...
 * \returns 0 on success, or a LIBUSB_ERROR code on failure
 */
int API_EXPORTED libusb_handle_events_timeout(libusb_context *ctx, struct timeval *tv) {
 ...
 * \ref libusb_mtasync
 */
int API_EXPORTED libusb_handle_events_locked(libusb_context *ctx, struct timeval *tv) {
    int            r;
    struct timeval poll_timeout;
 ...
 */
int API_EXPORTED libusb_get_next_timeout(libusb_context *ctx, struct timeval *tv) {
    struct usbi_transfer *itransfer;
    struct timespec       systime;
 ...
 */
void API_EXPORTED libusb_set_pollfd_notifiers(libusb_context *ctx, libusb_pollfd_added_cb added_cb, libusb_pollfd_removed_cb removed_cb, void *user_data) {
...
const struct libusb_pollfd **LIBUSB_CALL libusb_get_pollfds(libusb_context *ctx) {
    ...
    static_assert(sizeof(struct usbi_event_source_data) == sizeof(struct libusb_pollfd), "mismatch between usbi_event_source_data and libusb_pollfd sizes");
    ...
    i = 0;
    for_each_event_source(ctx, ievent_source) i++;
    ...
    i                                                  = 0;
    for_each_event_source(ctx, ievent_source) ret[i++] = (struct libusb_pollfd *)ievent_source;
...
void usbi_handle_disconnect(struct libusb_device_handle *dev_handle) {
    ...
    usbi_dbg(ctx, "device %d.%d", dev_handle->dev->bus_number, dev_handle->dev->device_address);
