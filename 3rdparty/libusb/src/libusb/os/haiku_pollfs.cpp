...
WatchedEntry::WatchedEntry(BMessenger *messenger, entry_ref *ref)
    : fMessenger(messenger), fIsDirectory(false), fDevice(NULL), fEntries(NULL), fLink(NULL), fInitCheck(false) {
    BEntry entry(ref);
        ...
        if(fDevice != NULL && fDevice->InitCheck() == true) {
            ...
            for_each_context(ctx) {
                ...
                *((USBDevice **)usbi_get_device_priv(dev)) = fDevice;
                ...
                static_assert(sizeof(dev->device_descriptor) == sizeof(usb_device_descriptor), "mismatch between libusb and OS device descriptor sizes");
...
bool WatchedEntry::InitCheck() {
    return fInitCheck;
}

RosterLooper::RosterLooper(USBRoster *roster) : BLooper("LibusbRoster Looper"), fRoster(roster), fRoot(NULL), fMessenger(NULL), fInitCheck(false) {
...
void RosterLooper::MessageReceived(BMessage *message) {
    ...
    case B_ENTRY_CREATED: {
        ...
        ino_t       directory;
        const char *name;
        if(message->FindInt32("device", &device) < B_OK || message->FindInt64("directory", &directory) < B_OK || message->FindString("name", &name) < B_OK)
            break;
...
bool RosterLooper::InitCheck() {
    return fInitCheck;
}

USBRoster::USBRoster() : fLooper(NULL) {}
