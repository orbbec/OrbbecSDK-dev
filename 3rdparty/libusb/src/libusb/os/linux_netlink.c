...
static int linux_netlink_parse(const char *buffer, size_t len, int *detached, const char **sys_name, uint8_t *busnum, uint8_t *devaddr) {
...
static int linux_netlink_read_message(void) {
    ...
    struct msghdr      msg = {
             .msg_iov = &iov, .msg_iovlen = 1, .msg_control = cred_buffer, .msg_controllen = sizeof(cred_buffer), .msg_name = &sa_nl, .msg_namelen = sizeof(sa_nl)
    };
    ...
    if(sa_nl.nl_groups != NL_GROUP_KERNEL || sa_nl.nl_pid != 0) {
        usbi_dbg(NULL, "ignoring netlink message from unknown group/PID (%u/%u)", (unsigned int)sa_nl.nl_groups, (unsigned int)sa_nl.nl_pid);
        return -1;
    }
    ...
    usbi_dbg(NULL, "netlink hotplug found device busnum: %hhu, devaddr: %hhu, sys_name: %s, removed: %s", busnum, devaddr, sys_name, detached ? "yes" : "no");
...
static void *linux_netlink_event_thread_main(void *arg) {
    struct pollfd fds[] = {
        { .fd = USBI_EVENT_OS_HANDLE(&netlink_control_event), .events = USBI_EVENT_POLL_EVENTS },
        { .fd = linux_netlink_socket, .events = POLLIN },
    };
    int r;
