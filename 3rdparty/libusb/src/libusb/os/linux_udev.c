...
static void *linux_udev_event_thread_main(void *arg) {
    struct pollfd fds[] = {
        { .fd = USBI_EVENT_OS_HANDLE(&udev_control_event), .events = USBI_EVENT_POLL_EVENTS },
...
static int udev_device_info(struct libusb_context *ctx, int detached, struct udev_device *udev_dev, uint8_t *busnum, uint8_t *devaddr, const char **sys_name) {
    ...
    *sys_name = udev_device_get_sysname(udev_dev);
    ...
    return linux_get_device_address(ctx, detached, busnum, devaddr, dev_node, *sys_name, -1);
}
