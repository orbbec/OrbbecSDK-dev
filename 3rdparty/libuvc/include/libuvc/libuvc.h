...
enum uvc_frame_format {
    ...
    UVC_FRAME_FORMAT_HEVC,  // h265
    UVC_FRAME_FORMAT_I420,  // i420
    UVC_FRAME_FORMAT_NV21,  // nv12

    // orbbec format
    UVC_FRAME_FORMAT_Y8,
    UVC_FRAME_FORMAT_Y10,
    UVC_FRAME_FORMAT_Y11,
    UVC_FRAME_FORMAT_Y12,
    UVC_FRAME_FORMAT_Y14,
    UVC_FRAME_FORMAT_RVL,
    // UVC_FRAME_FORMAT_Y16, // as UVC_FRAME_FORMAT_GRAY16
    UVC_FRAME_FORMAT_Z16,
    UVC_FRAME_FORMAT_YV12,
 ...
 * @ingroup device
 */
typedef void(uvc_status_callback_t)(enum uvc_status_class status_class, int event, int selector, enum uvc_status_attribute status_attribute, void *data,
...
typedef struct uvc_frame {
    ...
    /** Payload header for this frame (payload header of first payload for this frame)*/
    void *payload_header;
    /** Size of metadata buffer */
    size_t payload_header_bytes;

    uint32_t pts;  // pts of frame

    void  *headerdata;
    size_t headerdata_bytes;
} uvc_frame_t;
...
uvc_error_t uvc_open(uvc_device_t *dev, uint16_t mi, uvc_device_handle_t **devh, struct libusb_device_handle *usbh);
...
uvc_error_t uvc_get_digital_window(uvc_device_handle_t *devh, uint16_t *window_top, uint16_t *window_left, uint16_t *window_bottom, uint16_t *window_right,
                                   uint16_t *num_steps, uint16_t *num_steps_units, enum uvc_req_code req_code);
uvc_error_t uvc_set_digital_window(uvc_device_handle_t *devh, uint16_t window_top, uint16_t window_left, uint16_t window_bottom, uint16_t window_right,
