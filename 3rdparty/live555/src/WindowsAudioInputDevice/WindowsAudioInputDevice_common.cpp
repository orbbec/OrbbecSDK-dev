...
WindowsAudioInputDevice_common ::WindowsAudioInputDevice_common(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample,
                                                                unsigned char numChannels, unsigned samplingFrequency, unsigned granularityInMS)
    : AudioInputDevice(env, bitsPerSample, numChannels, samplingFrequency, granularityInMS), fCurPortIndex(-1), fHaveStarted(False) {
...
void WindowsAudioInputDevice_common::audioReadyPoller1() {
    ...
    if(readHead != NULL) {
        ...
        nextTask() = envir().taskScheduler().scheduleDelayedTask(uSecondsToDelay, (TaskFunc *)audioReadyPoller, this);
    }
}
...
Boolean WindowsAudioInputDevice_common::waveIn_open(unsigned uid, WAVEFORMATEX &wfx) {
    ...
    do {
        waveIn_reset();
        if(waveInOpen(&shWaveIn, uid, &wfx, (DWORD)waveInCallback, 0, CALLBACK_FUNCTION) != MMSYSERR_NOERROR)
            break;
        ...
        for(unsigned i = 0; i < numBlocks; ++i) {
            ...
            if(waveInPrepareHeader(shWaveIn, &readHdrs[i], sizeof(WAVEHDR)) != MMSYSERR_NOERROR)
                break;
            if(waveInAddBuffer(shWaveIn, &readHdrs[i], sizeof(WAVEHDR)) != MMSYSERR_NOERROR)
                break;
        }

        if(waveInStart(shWaveIn) != MMSYSERR_NOERROR)
            break;
