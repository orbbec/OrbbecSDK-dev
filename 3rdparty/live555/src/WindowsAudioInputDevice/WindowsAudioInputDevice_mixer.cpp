...
AudioInputDevice *AudioInputDevice::createNew(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample, unsigned char numChannels,
                                              unsigned samplingFrequency, unsigned granularityInMS) {
    Boolean                  success;
    WindowsAudioInputDevice *newSource =
        new WindowsAudioInputDevice(env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency, granularityInMS, success);
    if(!success) {
        delete newSource;
        newSource = NULL;
    }
...
WindowsAudioInputDevice ::WindowsAudioInputDevice(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample, unsigned char numChannels,
                                                  unsigned samplingFrequency, unsigned granularityInMS, Boolean &success)
    : WindowsAudioInputDevice_common(env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency, granularityInMS), fCurMixerId(-1) {
    success = initialSetInputPort(inputPortNumber);
}

WindowsAudioInputDevice::~WindowsAudioInputDevice() {
    if(fCurMixerId >= 0)
        ourMixers[fCurMixerId].close();
...
Boolean WindowsAudioInputDevice::setInputPort(int portIndex) {
    ...
    if(allowedDeviceNames != NULL) {
        ...
        for(i = 0; allowedDeviceNames[i] != NULL; ++i) {
            if(strncmp(ourMixers[newMixerId].name, allowedDeviceNames[i], strlen(allowedDeviceNames[i])) == 0) {
...
Mixer::Mixer() : hMixer(NULL), dwRecLineID(0), numPorts(0), ports(NULL) {}
...
void Mixer::open(unsigned numChannels, unsigned samplingFrequency, unsigned granularityInMS) {
    ...
    do {
        MIXERCAPS mc;
        if(mixerGetDevCaps(index, &mc, sizeof mc) != MMSYSERR_NOERROR)
            break;
        ...
        if(i >= nWavIn)
            break;  // error: we couldn't find the right line

        if(mixerOpen(&newHMixer, index, (unsigned long)NULL, (unsigned long)NULL, MIXER_OBJECTF_MIXER) != MMSYSERR_NOERROR)
            break;
        if(newHMixer == NULL)
            break;

        // Sanity check: re-call "mixerGetDevCaps()" using the mixer device handle:
        if(mixerGetDevCaps((UINT)newHMixer, &mc, sizeof mc) != MMSYSERR_NOERROR)
            break;
        if(mc.cDestinations < 1)
            break;  // error: this mixer has no destinations

        if(!WindowsAudioInputDevice_common::openWavInPort(uWavIn, numChannels, samplingFrequency, granularityInMS))
            break;
...
void Mixer::getPortsInfo() {
    ...
    for(i = 0; i < mc.cDestinations; ++i) {
        ...
        mlt.dwDestination = i;
        if(mixerGetLineInfo((HMIXEROBJ)hMixer, &mlt, MIXER_GETLINEINFOF_DESTINATION) != MMSYSERR_NOERROR)
            continue;
        if(mlt.dwLineID == dwRecLineID)
            break;  // this is the destination we're interested in
    }
    ...
    for(i = 1; i < numPorts; ++i) {
        ...
        if(_strnicmp("mic", ports[i].name, 3) == 0 || _strnicmp("mik", ports[i].name, 3) == 0) {
#else
