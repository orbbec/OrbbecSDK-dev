...
WindowsAudioInputDevice ::WindowsAudioInputDevice(UsageEnvironment &env, int inputPortNumber, unsigned char bitsPerSample, unsigned char numChannels,
    ...
    : WindowsAudioInputDevice_common(env, inputPortNumber, bitsPerSample, numChannels, samplingFrequency, granularityInMS) {
    success = initialSetInputPort(inputPortNumber);
}

WindowsAudioInputDevice::~WindowsAudioInputDevice() {
    if(fCurPortIndex >= 0)
        ourAudioInputPorts[fCurPortIndex].close();

    delete[] ourAudioInputPorts;
    ourAudioInputPorts = NULL;
    numAudioInputPorts = 0;
}

void WindowsAudioInputDevice::initializeIfNecessary() {
    if(ourAudioInputPorts != NULL)
...
Boolean WindowsAudioInputDevice::setInputPort(int portIndex) {
    ...
    if(allowedDeviceNames != NULL) {
        ...
        for(i = 0; allowedDeviceNames[i] != NULL; ++i) {
            if(strncmp(ourAudioInputPorts[portIndex].name, allowedDeviceNames[i], strlen(allowedDeviceNames[i])) == 0) {
...
void AudioInputPort::open(unsigned numChannels, unsigned samplingFrequency, unsigned granularityInMS) {
    do {
        ...
        if(!WindowsAudioInputDevice_common::openWavInPort(index, numChannels, samplingFrequency, granularityInMS))
            break;
