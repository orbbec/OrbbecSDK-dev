...
int setupDatagramSocket(UsageEnvironment &env, Port port, int domain) {
    ...
    if(setsockopt(newSocket, domain == AF_INET ? IPPROTO_IP : IPPROTO_IPV6, domain == AF_INET ? IP_MULTICAST_LOOP : IPV6_MULTICAST_LOOP, (const char *)&loop,
                  sizeof loop)
       < 0) {
    ...
    if(SendingInterfaceAddr != INADDR_ANY) {  // later, fix for IPv6
        ...
        if(setsockopt(newSocket, domain == AF_INET ? IPPROTO_IP : IPPROTO_IPV6, domain == AF_INET ? IP_MULTICAST_IF : IPV6_MULTICAST_IF, (const char *)&addr,
                      sizeof addr)
           < 0) {
...
Boolean writeSocket(UsageEnvironment &env, int socket, struct sockaddr_storage const &addressAndPort, u_int8_t ttlArg, unsigned char *buffer,
                    unsigned bufferSize) {
...
static void clearMulticastAllSocketOption(int socket, int domain) {
    ...
    int multicastAll = 0;
    (void)setsockopt(socket, domain == AF_INET ? IPPROTO_IP : IPPROTO_IPV6,
...
Boolean socketJoinGroupSSM(UsageEnvironment &env, int socket, struct sockaddr_storage const &groupAddress, struct sockaddr_storage const &sourceFilterAddr) {
    if(!IsMulticastAddress(groupAddress))
        return True;  // ignore this case
    if(groupAddress.ss_family != AF_INET)
        return False;  // later, support IPv6
...
Boolean socketLeaveGroupSSM(UsageEnvironment & /*env*/, int socket, struct sockaddr_storage const &groupAddress,
...
static void copyAddress(struct sockaddr_storage &to, struct sockaddr const *from) {
    ...
    case AF_INET: {
        ...
        if(from->sa_len < sizeof(struct sockaddr_in))
            return;  // sanity check
    ...
    case AF_INET6: {
#ifdef HAVE_SOCKADDR_LEN
        if(from->sa_len < sizeof(struct sockaddr_in6))
            return;  // sanity check
