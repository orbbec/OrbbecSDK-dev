...
Boolean addressIsNull(sockaddr_storage const &address) {
    ...
    case AF_INET6: {
        for(unsigned i = 0; i < 16; ++i) {
            if(((sockaddr_in6 const &)address).sin6_addr.s6_addr[i] != 0)
                return False;
        }
        return True;
    }
...
NetAddressList::NetAddressList(char const *hostname, int addressFamily) : fNumAddresses(0), fAddressArray(NULL) {
    if(hostname == NULL)
        return;
    ...
    while(p != NULL) {
        if(p->ai_family != AF_INET && p->ai_family != AF_INET6)
            continue;  // unsupported address
        if(addressFamily != AF_UNSPEC && p->ai_family != addressFamily)
            continue;  // unwanted
...
NetAddressList::Iterator::Iterator(NetAddressList const &addressList) : fAddressList(addressList), fNextIndex(0) {}
...
AddressPortLookupTable::AddressPortLookupTable() : fTable(HashTable::create(NUM_RECORDS_IN_KEY_TOTAL)) {}
...
static void setKey(int *key, struct sockaddr_storage const &address1, struct sockaddr_storage const &address2, Port port) {
...
AddressPortLookupTable::Iterator::Iterator(AddressPortLookupTable &table) : fIter(HashTable::Iterator::create(*(table.fTable))) {}
