...
int main(int argc, char **argv) {
    ...
    *env << "\t(LIVE555 Streaming Media version " << LIVEMEDIA_LIBRARY_VERSION_STRING << ")\n";
    ...
    while(argc > 1) {
        ...
        if(opt[0] != '-') {
            if(argc <= 3)
                break;  // only the URL + prefix is left
            usage();
        }
    ...
    if(createHandlerServerForREGISTERCommand) {
        ...
        handlerServerForREGISTERCommand = HandlerServerForREGISTERCommand::createNew(*env, continueAfterClientCreation0, handlerServerForREGISTERCommandPortNum,
                                                                                     authDBForREGISTER, RTSP_CLIENT_VERBOSITY_LEVEL, programName);
        ...
        *env << *rtspClient << "Set up the \"" << *subsession << "\" subsession\n";
        ...
        if(strcmp(subsession->mediumName(), "video") == 0) {
            ...
            if(strcmp(subsession->codecName(), "H264") == 0) {
                ...
                framer      = H264VideoStreamDiscreteFramer::createNew(*env, subsession->readSource(), True /*includeStartCodeInOutput*/,
                ...
                framer      = H265VideoStreamDiscreteFramer::createNew(*env, subsession->readSource(), True /*includeStartCodeInOutput*/,
                                                                  True /*insertAccessUnitDelimiters*/);
                ...
                delete[] sPropRecordsVPS;
                delete[] sPropRecordsSPS;
                delete[] sPropRecordsPPS;
            }
...
public:
    ...
    char const *fileName() const {
        return fSegmentFileName;
    }
    double duration() const {
        return fSegmentDuration;
    }
...
void segmentationCallback(void * /*clientData*/, char const *segmentFileName, double segmentDuration) {
    ...
    fprintf(ourM3U8Fid,
            ...
            OUR_HLS_SEGMENTATION_DURATION, firstSegmentCounter);
