...
ADTSAudioStreamDiscreteFramer *ADTSAudioStreamDiscreteFramer ::createNew(UsageEnvironment &env, FramedSource *inputSource, char const *configStr) {
    ...
    if(configStr != NULL && strlen(configStr) >= 4) {
        configValue = (hexToBinary(configStr[0]) << 12) | (hexToBinary(configStr[1]) << 8) | (hexToBinary(configStr[2]) << 4) | hexToBinary(configStr[3]);
    }
...
ADTSAudioStreamDiscreteFramer ::ADTSAudioStreamDiscreteFramer(UsageEnvironment &env, FramedSource *inputSource, u_int8_t profile,
...
void ADTSAudioStreamDiscreteFramer ::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                                       unsigned durationInMicroseconds) {
    ADTSAudioStreamDiscreteFramer *source = (ADTSAudioStreamDiscreteFramer *)clientData;
    source->afterGettingFrame1(frameSize, numTruncatedBytes, presentationTime, durationInMicroseconds);
}

void ADTSAudioStreamDiscreteFramer ::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                                        unsigned durationInMicroseconds) {
