...
void DVVideoRTPSink::doSpecialFrameHandling(unsigned       fragmentationOffset, unsigned char * /*frameStart*/, unsigned /*numBytesInFrame*/,
...
char const *DVVideoRTPSink::auxSDPLineFromFramer(DVVideoStreamFramer *framerSource) {
    ...
    unsigned          fmtpSDPFmtSize = strlen(fmtpSDPFmt) + 3  // max payload format code length
                              + strlen(profileName);
