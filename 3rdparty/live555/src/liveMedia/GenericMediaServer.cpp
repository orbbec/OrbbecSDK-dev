...
void GenericMediaServer ::lookupServerMediaSession(char const *streamName, lookupServerMediaSessionCompletionFunc *completionFunc, void *completionClientData,
...
GenericMediaServer ::GenericMediaServer(UsageEnvironment &env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort, unsigned reclamationSeconds)
    : Medium(env),
      fServerSocketIPv4(ourSocketIPv4),
      fServerSocketIPv6(ourSocketIPv6),
      fServerPort(ourPort),
      ...
      fTLSCertificateFileName(NULL),
      fTLSPrivateKeyFileName(NULL) {
...
GenericMediaServer::~GenericMediaServer() {
    ...
    delete[] fTLSCertificateFileName;
    delete[] fTLSPrivateKeyFileName;
}
...
int GenericMediaServer::setUpOurSocket(UsageEnvironment &env, Port &ourPort, int domain) {
    ...
    do {
        ...
        if(ourPort.num() == 0) {
            ...
            if(!getSourcePort(env, ourSocket, domain, ourPort))
                break;
        }
...
void GenericMediaServer ::setTLSFileNames(char const *certFileName, char const *privKeyFileName) {
    delete[] fTLSCertificateFileName;
    fTLSCertificateFileName = strDup(certFileName);
    delete[] fTLSPrivateKeyFileName;
    fTLSPrivateKeyFileName = strDup(privKeyFileName);
}
...
GenericMediaServer::ClientConnection ::ClientConnection(GenericMediaServer &ourServer, int clientSocket, struct sockaddr_storage const &clientAddr,
    ...
    if(useTLS) {
        ...
        fTLS.setCertificateAndPrivateKeyFileNames(ourServer.fTLSCertificateFileName, ourServer.fTLSPrivateKeyFileName);
        fTLS.isNeeded = True;
...
void GenericMediaServer::ClientConnection::incomingRequestHandler() {
    if(fTLS.tlsAcceptIsNeeded) {  // we need to successfully call fTLS.accept() first:
        if(fTLS.accept(fOurSocket) <= 0)
            return;  // either an error, or we need to try again later
...
GenericMediaServer::ClientSession *GenericMediaServer::lookupClientSession(char const *sessionIdStr) {
