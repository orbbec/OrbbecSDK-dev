...
public:
    H264or5VideoStreamParser(int hNumber, H264or5VideoStreamFramer *usingSource, FramedSource *inputSource, Boolean includeStartCodeInOutput);
...
H264or5VideoStreamFramer ::H264or5VideoStreamFramer(int hNumber, UsageEnvironment &env, FramedSource *inputSource, Boolean createParser,
    ...
    : MPEGVideoStreamFramer(env, inputSource),
      ...
      fLastSeenPPS(NULL),
      fLastSeenPPSSize(0) {
    fParser    = createParser ? new H264or5VideoStreamParser(hNumber, this, inputSource, includeStartCodeInOutput) : NULL;
...
H264or5VideoStreamParser ::H264or5VideoStreamParser(int hNumber, H264or5VideoStreamFramer *usingSource, FramedSource *inputSource,
...
Boolean H264or5VideoStreamParser::isSEI(u_int8_t nal_unit_type) {
    return fHNumber == 264 ? nal_unit_type == 6 : (nal_unit_type == PREFIX_SEI_NUT || nal_unit_type == SUFFIX_SEI_NUT);
}

Boolean H264or5VideoStreamParser::isEOF(u_int8_t nal_unit_type) {
    // "end of sequence" or "end of (bit)stream"
    return fHNumber == 264 ? (nal_unit_type == 10 || nal_unit_type == 11) : (nal_unit_type == 36 || nal_unit_type == 37);
}

Boolean H264or5VideoStreamParser::usuallyBeginsAccessUnit(u_int8_t nal_unit_type) {
    return fHNumber == 264 ? (nal_unit_type >= 6 && nal_unit_type <= 9) || (nal_unit_type >= 14 && nal_unit_type <= 18)
                           : (nal_unit_type >= 32 && nal_unit_type <= 35) || (nal_unit_type == 39) || (nal_unit_type >= 41 && nal_unit_type <= 44)
...
void H264or5VideoStreamParser ::analyze_vui_parameters(BitVector &bv, unsigned &num_units_in_tick, unsigned &time_scale) {
...
void H264or5VideoStreamParser ::analyze_seq_parameter_set_data(unsigned &num_units_in_tick, unsigned &time_scale) {
    ...
    if(fHNumber == 264) {
        ...
        if(profile_idc == 100 || profile_idc == 110 || profile_idc == 122 || profile_idc == 244 || profile_idc == 44 || profile_idc == 83 || profile_idc == 86
...
void H264or5VideoStreamParser::analyze_sei_data(u_int8_t nal_unit_type) {
    ...
    while(j < seiSize) {
        ...
        if(fHNumber == 264) {
            unsigned descriptionNum = payloadType <= MAX_SEI_PAYLOAD_TYPE_DESCRIPTION_H264 ? payloadType : MAX_SEI_PAYLOAD_TYPE_DESCRIPTION_H264;
            description             = sei_payloadType_description_h264[descriptionNum];
        }
        else {  // 265
            description = payloadType == 3                                          ? "filler_payload"
...
void H264or5VideoStreamParser ::analyze_sei_payload(unsigned payloadType, unsigned payloadSize, u_int8_t *payload) {
    if(payloadType == 1 /* pic_timing, for both H.264 and H.265 */) {
        ...
        if(pic_struct_present_flag) {
            ...
            if(fHNumber == 264) {
                DeltaTfiDivisor = pic_struct == 0   ? 2.0
                                  : pic_struct <= 2 ? 1.0
                                  : pic_struct <= 4 ? 2.0
                                  : pic_struct <= 6 ? 3.0
                                  : pic_struct == 7 ? 4.0
...
unsigned H264or5VideoStreamParser::parse() {
    try {
        ...
        if(haveSeenEOF()) {
            ...
            if(fHNumber == 264) {
                ...
                fprintf(stderr, "Parsed trailing %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d (\"%s\"))\n", trailingNALUnitSize, nal_ref_idc,
                        nal_unit_type, nal_unit_type_description_h264[nal_unit_type]);
            }
            else {  // 265
        ...
        if(haveSeenEOF() || isEOF(nal_unit_type)) {
            ...
            u_int8_t const &next_nal_unit_type = fHNumber == 264 ? (firstBytesOfNextNALUnit[0] & 0x1F) : ((firstBytesOfNextNALUnit[0] & 0x7E) >> 1);
