...
JPEG2000VideoRTPSource *JPEG2000VideoRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
                                                          unsigned rtpTimestampFrequency, char const *sampling) {
    return new JPEG2000VideoRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, sampling);
}

JPEG2000VideoRTPSource ::JPEG2000VideoRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency,
                                                char const *sampling)
...
Boolean JPEG2000VideoRTPSource ::processSpecialHeader(BufferedPacket *packet, unsigned &resultSpecialHeaderSize) {
    ...
    if(packetSize < JPEG2000_PAYLOAD_HEADER_SIZE)
        return False;
