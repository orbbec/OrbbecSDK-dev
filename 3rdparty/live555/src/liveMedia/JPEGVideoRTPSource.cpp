...
JPEGVideoRTPSource *JPEGVideoRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency,
...
JPEGVideoRTPSource::JPEGVideoRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency,
                                       unsigned defaultWidth, unsigned defaultHeight)
...
static void createHuffmanHeader(unsigned char *&p, unsigned char const *codelens, int ncodes, unsigned char const *symbols, int nsymbols, int tableNo,
                                int tableClass) {
    *p++ = 0xff;
    *p++ = MARKER_DHT;
    *p++ = 0;                     /* length msb */
...
static unsigned char const defaultQuantizers[128] = {
    // luma table:
    16, 11, 12, 14, 12, 10, 16, 14, 13, 14, 18, 17, 16, 19, 24, 40, 26, 24, 22, 22, 24, 49, 35, 37, 29, 40, 58, 51, 61, 60, 57, 51, 56, 55, 64, 72, 92, 78, 64,
    68, 87, 69, 55, 56, 80, 109, 81, 87, 95, 98, 103, 104, 103, 62, 77, 113, 121, 112, 100, 120, 92, 101, 103, 99,
    // chroma table:
    17, 18, 18, 24, 21, 24, 47, 26, 26, 47, 99, 66, 56, 66, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99,
...
Boolean JPEGVideoRTPSource ::processSpecialHeader(BufferedPacket *packet, unsigned &resultSpecialHeaderSize) {
  ...
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    */
    ...
    if(width == 0)
        width = 256 * 8;  // special case
    if(height == 0)
        height = 256 * 8;  // special case
...
void JPEGBufferedPacket::reset() {
    ...
    if(offset > fPacketSize)
        offset = fPacketSize;  // shouldn't happen
    fHead = fTail = offset;
}

unsigned JPEGBufferedPacket ::nextEnclosedFrameSize(unsigned char *&framePtr, unsigned dataSize) {
    // Normally, the enclosed frame size is just "dataSize".  If, however,
    // the frame does not end with the "EOI" marker, then add this now:
    if(completesFrame && dataSize >= 2 && !(framePtr[dataSize - 2] == 0xFF && framePtr[dataSize - 1] == MARKER_EOI)) {
