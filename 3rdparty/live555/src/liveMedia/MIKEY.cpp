...
MIKEYState::MIKEYState(u_int8_t const *messageToParse, unsigned messageSize, Boolean &parsedOK)
    ...
    if(!parseHDRPayload(ptr, endPtr, nextPayloadType))
        return;

    // Then parse each subsequent payload that we see:
    while(nextPayloadType != 0) {
        if(!parseNonHDRPayload(ptr, endPtr, nextPayloadType))
            return;
    }
...
Boolean MIKEYState ::parseNonHDRPayload(u_int8_t const *&ptr, u_int8_t const *endPtr, u_int8_t &nextPayloadType) {
    do {
        ...
        case SP: {  // RFC 3830, section 6.10
            ...
            if(protType != 0 /*SRTP*/)
                break;  // unsupported protocol type
            ...
            while(1) {
                ...
                testSize(ppLength);
                if(ptr + ppLength > payloadEndPtr)
                    break;  // bad "length"s - too big for the payload
                ...
                case 0: {  // Encryption algorithm: we handle only NULL and AES-CM
                    ...
                    if(value > 0)
                        fEncryptSRTP = fEncryptSRTCP = True;
        ...
        case KEMAC: {  // RFC 3830, section 6.2
            ...
            if(encrAlg != 0 /*NULL*/)
                break;  // unknown or unsupported key encryption
            ...
            // Parse the 'key data sub-payload':
            {
                ...
                if(4 + keyDataLen + 1 + SPILength > encrDataLen)
                    break;
            ...
            if(macAlg == 0 /*NULL*/)
                ...
                break;  // unknown or unsupported MAC algorithm => parse fails
...
MIKEYPayload::MIKEYPayload(MIKEYState &ourMIKEYState, u_int8_t payloadType) : fOurMIKEYState(ourMIKEYState), fPayloadType(payloadType), fNext(NULL) {
    switch(payloadType) {
    ...
    case SP: {  // RFC 3830, section 6.10
        ...
        *p++                     = 0;  // Protocol type: SRTP
        ...
        add1BytePolicyParam(p, 0 /*Encryption algorithm*/, (fOurMIKEYState.encryptSRTP() || fOurMIKEYState.encryptSRTCP()) ? 1 /*AES-CM*/ : 0 /*NULL*/);
        add1BytePolicyParam(p, 1 /*Session Encryption key length*/, 16);
        add1BytePolicyParam(p, 2 /*Authentication algorithm*/, fOurMIKEYState.useAuthentication() ? 1 /*HMAC-SHA-1*/ : 0 /*NULL*/);
