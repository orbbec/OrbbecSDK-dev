...
MP3ADURTPSource *MP3ADURTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency) {
    return new MP3ADURTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency);
}

MP3ADURTPSource::MP3ADURTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency)
    : MultiFramedRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, new ADUBufferedPacketFactory) {}
...
unsigned ADUBufferedPacket ::nextEnclosedFrameSize(unsigned char *&framePtr, unsigned dataSize) {
    ...
    unsigned       remainingFrameSize = ADUdescriptor::getRemainingFrameSize(frameDataPtr);
...
BufferedPacket *ADUBufferedPacketFactory ::createNewPacket(MultiFramedRTPSource * /*ourSource*/) {
