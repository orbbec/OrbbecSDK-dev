...
MPEG1or2VideoRTPSink::MPEG1or2VideoRTPSink(UsageEnvironment &env, Groupsock *RTPgs) : VideoRTPSink(env, RTPgs, 32, 90000, "MPV") {
...
Boolean MPEG1or2VideoRTPSink ::frameCanAppearAfterPacketStart(unsigned char const *frameStart, unsigned numBytesInFrame) const {
    ...
    return numBytesInFrame >= 4 && frameStart[0] == 0 && frameStart[1] == 0 && frameStart[2] == 1 && frameStart[3] >= 1 && frameStart[3] <= 0xAF;
}

#define VIDEO_SEQUENCE_HEADER_START_CODE 0x000001B3
#define PICTURE_START_CODE 0x00000100

void MPEG1or2VideoRTPSink ::doSpecialFrameHandling(unsigned fragmentationOffset, unsigned char *frameStart, unsigned numBytesInFrame,
    ...
    if(fragmentationOffset == 0) {
        ...
        if(startCode == VIDEO_SEQUENCE_HEADER_START_CODE) {
            ...
            envir() << "Warning: MPEG1or2VideoRTPSink::doSpecialFrameHandling saw strange first 4 bytes " << (void *)startCode
    ...
    unsigned videoSpecificHeader =
        ...
        // AN == N == 0
        (fSequenceHeaderPresent << 13) | (fPacketBeginsSlice << 12) | (fPacketEndsSlice << 11) | (fPictureState.picture_coding_type << 8)
