...
MPEG1or2VideoStreamDiscreteFramer *MPEG1or2VideoStreamDiscreteFramer::createNew(UsageEnvironment &env, FramedSource *inputSource, Boolean iFramesOnly,
                                                                                double vshPeriod, Boolean leavePresentationTimesUnmodified) {
...
MPEG1or2VideoStreamDiscreteFramer ::MPEG1or2VideoStreamDiscreteFramer(UsageEnvironment &env, FramedSource *inputSource, Boolean iFramesOnly, double vshPeriod,
...
void MPEG1or2VideoStreamDiscreteFramer ::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
...
void MPEG1or2VideoStreamDiscreteFramer ::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                                            unsigned durationInMicroseconds) {
    ...
    if(frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
        ...
        if(nextCode == 0xB3) {  // VIDEO_SEQUENCE_HEADER_START_CODE
            ...
            for(vshSize = 4; vshSize < frameSize - 3; ++vshSize) {
                if(fTo[vshSize] == 0 && fTo[vshSize + 1] == 0 && fTo[vshSize + 2] == 1 && (fTo[vshSize + 3] == 0xB8 || fTo[vshSize + 3] == 0x00))
                    break;
            }
            ...
            if(pts > fSavedVSHTimestamp + fVSHPeriod && fSavedVSHSize + frameSize <= fMaxSize) {
        ...
        unsigned i = 3;
        if(nextCode == 0xB3 /*VIDEO_SEQUENCE_HEADER_START_CODE*/ || nextCode == 0xB8 /*GROUP_START_CODE*/) {
