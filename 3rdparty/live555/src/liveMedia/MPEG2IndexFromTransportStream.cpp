...
UsageEnvironment &operator<<(UsageEnvironment &env, IndexRecord &r) {
    return env << "[" << ((r.recordType() & 0x80) != 0 ? "1" : "") << recordTypeStr[r.recordType() & 0x7F] << ":" << (unsigned)r.transportPacketNumber() << ":"
               << r.startOffset() << "(" << r.size() << ")@" << r.pcr() << "]";
}
#endif
...
void MPEG2IFrameIndexFromTransportStream ::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
...
void MPEG2IFrameIndexFromTransportStream ::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                                              unsigned durationInMicroseconds) {
    ...
    if(totalHeaderSize > 5 && (fInputBuffer[5] & 0x10) != 0) {
        ...
        if(!fHaveSeenFirstPCR) {
            ...
            envir() << "\nWarning: At about " << fLastPCR - fFirstPCR << " seconds into the file, the PCR timestamp decreased - from " << fLastPCR << " to "
                    << pcr << "\n";
    ...
    if(payload_unit_start_indicator && totalHeaderSize < TRANSPORT_PACKET_SIZE - 8 && fInputBuffer[totalHeaderSize] == 0x00
       && fInputBuffer[totalHeaderSize + 1] == 0x00 && fInputBuffer[totalHeaderSize + 2] == 0x01) {
    ...
    if(fIsH264) {
        ...
        default:                                    // picture
            ...
            while(1) {
                if(!parseToNextCode(nextCode))
                    return False;
                if(nextCode == VIDEO_SEQUENCE_START_CODE || nextCode == VISUAL_OBJECT_SEQUENCE_START_CODE || nextCode == GROUP_START_CODE
                   || nextCode == GROUP_VOP_START_CODE || nextCode == PICTURE_START_CODE || nextCode == VOP_START_CODE)
                    break;
                fParseBufferParseEnd += 4;  // skip over the code that we just saw
            }
            break;
