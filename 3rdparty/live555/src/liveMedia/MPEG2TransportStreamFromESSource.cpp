...
void MPEG2TransportStreamFromESSource::doStopGettingFrames() {
    ...
    for(InputESSourceRecord *sourceRec = fInputSources; sourceRec != NULL; sourceRec = sourceRec->next()) {
...
InputESSourceRecord ::InputESSourceRecord(MPEG2TransportStreamFromESSource &parent, FramedSource *inputSource, u_int8_t streamId, int mpegVersion,
...
void InputESSourceRecord::askForNewData() {
    ...
    if(fInputBufferBytesAvailable < TS_FROM_ES_LOW_WATER_MARK && !fInputSource->isCurrentlyAwaitingData()) {
        ...
        fInputSource->getNextFrame(&fInputBuffer[fInputBufferBytesAvailable], INPUT_BUFFER_SIZE - fInputBufferBytesAvailable, afterGettingFrame, this,
                                   FramedSource::handleClosure, &fParent);
    }
}

Boolean InputESSourceRecord::deliverBufferToClient() {
    if(fInputBufferInUse || fInputBufferBytesAvailable < TS_FROM_ES_LOW_WATER_MARK)
        return False;
...
void InputESSourceRecord ::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
                                             unsigned /*durationInMicroseconds*/) {
...
void InputESSourceRecord ::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime) {
    if(numTruncatedBytes > 0) {
        fParent.envir()
            << "MPEG2TransportStreamFromESSource: input buffer too small; increase \"MPEG2TransportStreamFromESSource::maxInputESFrameSize\" by at least "
    ...
    if(fInputBufferBytesAvailable == SIMPLE_PES_HEADER_SIZE) {
        ...
        fprintf(stderr, "PES header: stream_id 0x%02x, pts: %u.%06u => SCR 0x%x%08x:%03x\n", fStreamId, (unsigned)presentationTime.tv_sec,
                (unsigned)presentationTime.tv_usec, fSCR.highBit, fSCR.remainingBits, fSCR.extension);
#endif
    }
