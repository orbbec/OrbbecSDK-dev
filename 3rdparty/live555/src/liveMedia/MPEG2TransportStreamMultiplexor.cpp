...
void MPEG2TransportStreamMultiplexor ::setTimedSegmentation(unsigned segmentationDuration, onEndOfSegmentFunc *onEndOfSegmentFunc,
...
void MPEG2TransportStreamMultiplexor::doGetNextFrame() {
    ...
    do {
        ...
        if((segmentationIsTimed() && fSegmentationIndication == 1) || (!segmentationIsTimed() && fOutgoingPacketCounter % PAT_PERIOD_IF_UNTIMED == 0)) {
        ...
        if(programMapHasChanged || (segmentationIsTimed() && fSegmentationIndication == 2)
...
void MPEG2TransportStreamMultiplexor::setProgramStreamMap(unsigned frameSize) {
    ...
    if(frameSize > 0xFF)
        return;  // program_stream_map is too large
