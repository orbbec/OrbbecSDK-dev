...
void MPEG2TransportStreamParser::continueParsing() {
    if(fAmCurrentlyParsing)
        return;  // don't allow recursive calls to parse()
    ...
    try {
        while(1) {
            ...
            if(adaptation_field_control == 2 && totalAdaptationFieldSize != 1 + 183) {
                fprintf(stderr,
                        "MPEG2TransportStreamParser::parse() Warning: Got an inconsistent \"totalAdaptationFieldSize\" %d for adaptation_field_control == 2\n",
...
PIDState::PIDState(MPEG2TransportStreamParser &parser, u_int16_t pid, PIDType pidType) : ourParser(parser), PID(pid), type(pidType) {}
