    ...
    do {
...
#endif
        if(section_length < 9 /*too small for remaining fields + CRC*/ || section_length > 1021 /*as per specification*/) {
#ifdef DEBUG_ERRORS
        ...
        if(endPos - startPos > numDataBytes) {
            ...
            fprintf(stderr, "MPEG2TransportStreamParser::parsePAT(%d, %d): section_length %d gives us a total size %d that's too large!\n", pusi, numDataBytes,
                    section_length, endPos - startPos);
#endif
            break;
        }
        ...
        while(curOffset() <= endPos - 4 /*for CRC*/ - 4 /*for a program_number+PID*/) {
            ...
            if(program_number != 0x0000) {
                if(fPIDState[pid] == NULL)
                    fPIDState[pid] = new PIDState_PMT(*this, pid, program_number);
            }
        }
    } while(0);
...
PIDState_PAT::PIDState_PAT(MPEG2TransportStreamParser &parser, u_int16_t pid) : PIDState(parser, pid, PAT) {}
