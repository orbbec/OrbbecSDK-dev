    ...
    do {
...
#endif
        if(section_length < 13 /*too small for remaining fields + CRC*/ || section_length > 1021 /*as per specification*/) {
#ifdef DEBUG_ERRORS
            fprintf(stderr, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): Bad section_length: %d\n", pidState->PID, pusi, numDataBytes,
                    section_length);
#endif
            break;
        }
...
#ifdef DEBUG_ERRORS
            fprintf(stderr,
                    "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): program_number %d does not match the value %d that was given to us in the PAT!\n",
        ...
        while(curOffset() <= endPos - 4 /*for CRC*/ - 5 /*for mapping fields*/) {
            ...
            fprintf(stderr, "\t\tstream_type: 0x%02x (%s); elementary_PID: 0x%04x; ES_info_length: %d\n", stream_type,
                    streamTypeDesc == NULL ? "???" : streamTypeDesc, elementary_PID, ES_info_length);
#endif
...
#define pDesc(str)                                                                                                               \
    do {                                                                                                                         \
        fprintf(stderr, "\t\t\tdescriptor_tag: 0x%02x (%s); descriptor_length: %d\n", descriptor_tag, (str), descriptor_length); \
    } while(0)
#else
#define pDesc(str)
#endif
...
void MPEG2TransportStreamParser::parseStreamDescriptors(unsigned numDescriptorBytes) {
    while(numDescriptorBytes >= 2 /* enough for "descriptor_tag" and "descriptor_length" */) {
        ...
        case 0x05: {
            ...
            fprintf(stderr, "\t\t\t\tformat_identifier: 0x%08x (%c%c%c%c)\n", format_identifier, format_identifier >> 24, format_identifier >> 16,
                    format_identifier >> 8, format_identifier);
            if(descriptor_length > 4) {
...
#endif
            if(descriptor_length < number_of_services * 6)
                break;
#ifdef DEBUG_CONTENTS
