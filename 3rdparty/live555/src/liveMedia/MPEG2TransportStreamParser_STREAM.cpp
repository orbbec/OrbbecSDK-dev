...
Boolean MPEG2TransportStreamParser ::processStreamPacket(PIDState_STREAM *pidState, Boolean pusi, unsigned numDataBytes) {
    ...
    do {
        ...
        if(pusi && pidState->stream_type != 0x05 /*these special private streams don't have PES hdrs*/) {
            ...
            if(pesHeaderSize == 0)
                break;  // PES header parsing failed
        }
...
unsigned MPEG2TransportStreamParser ::parsePESHeader(PIDState_STREAM *pidState, unsigned numDataBytes) {
    ...
    do {
        ...
        if(!isSpecialStreamId[stream_id]) {
            ...
            if(PTS_DTS_flags == 2 || PTS_DTS_flags == 3) {
                ...
                if((first8PTSBits & 0xF1) != ((PTS_DTS_flags << 4) | 0x01) || (last32PTSBits & 0x00010001) != 0x00010001) {
#ifdef DEBUG_ERRORS
                    fprintf(stderr, "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): Bad PTS bits: 0x%02x,0x%08x\n", pidState->PID, numDataBytes,
                            first8PTSBits, last32PTSBits);
#endif
                    break;
                }
            ...
            if(PTS_DTS_flags == 3) {
                ...
                if((first8DTSBits & 0x11) != 0x11 || (last32DTSBits & 0x00010001) != 0x00010001) {
#ifdef DEBUG_ERRORS
...
#ifdef DEBUG_CONTENTS
                fprintf(stderr,
                        "\t\t\tPES_extension: flags: 0x%02x (PES_private_data:%d; pack_header_field:%d; program_packet_sequence_counter:%d; P_STD_buffer:%d; "
                        "PES_extension_2:%d\n",
...
#ifdef DEBUG_ERRORS
                fprintf(stderr,
                        "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): Error: Parsed %d PES header bytes; expected %d (based on "
                        "\"PES_header_data_length\": %d)\n",
...
#ifdef DEBUG_ERRORS
            fprintf(stderr,
                    "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): Error: PES header size %d is larger than the number of bytes available (%d)\n",
    ...
    if(numBytesLeft > 0)
        skipBytes((unsigned)numBytesLeft);
    return 0;
}
