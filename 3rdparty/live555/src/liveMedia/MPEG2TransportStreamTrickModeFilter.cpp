...
MPEG2TransportStreamTrickModeFilter *MPEG2TransportStreamTrickModeFilter ::createNew(UsageEnvironment &env, FramedSource *inputSource,
...
MPEG2TransportStreamTrickModeFilter ::MPEG2TransportStreamTrickModeFilter(UsageEnvironment &env, FramedSource *inputSource,
                                                                          MPEG2TransportStreamIndexFile *indexFile, int scale)
...
#define isIFrameStart(type) \
    ((type) == 0x81 /*actually, a VSH*/ || (type) == 0x85 /*actually, a SPS, for H.264*/ || (type) == 0x8B /*actually, a VPS, for H.265*/)
...
void MPEG2TransportStreamTrickModeFilter::doGetNextFrame() {
    ...
    while(1) {
        ...
        //    fprintf(stderr, "#####read index record %ld: ts %ld: %c, PCR %f\n", fNextIndexRecordNum, fDesiredTSPacketNum, isIFrameStart(recordType) ? 'I' :
        //    isNonIFrameStart(recordType) ? 'j' : 'x', recordPCR);
        ...
        case DELIVERING_SAVED_FRAME: {
            ...
            if(endOfIndexFile || (isIFrameStart(recordType) && fNextIndexRecordNum - 1 != fSavedFrameIndexRecordStart) || isNonIFrameStart(recordType)) {
