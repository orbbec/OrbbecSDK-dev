...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
    ...
    if(!isVideoObjectLayerStartCode(next4Bytes)) {
        usingSource()->envir()
            << "MPEG4VideoStreamParser::parseVideoObjectLayer(): This appears to be a 'short video header', which we currently don't support\n";
    }
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
    ...
    default: {
        if(isVideoObjectStartCode(next4Bytes)) {
            ...
            usingSource()->envir() << "MPEG4VideoStreamParser::parseVideoObjectPlane(): Saw unexpected code " << (void *)next4Bytes << "\n";
