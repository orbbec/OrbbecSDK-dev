...
void MatroskaFile ::createNew(UsageEnvironment &env, char const *fileName, onCreationFunc *onCreation, void *onCreationClientData,
...
MatroskaDemux *MatroskaFile ::newDemux(MatroskaDemuxOnDeletionFunc *onDeletionFunc, void *objectToNotify) {
...
void MatroskaFile::getH264ConfigData(MatroskaTrack const *track, u_int8_t *&sps, unsigned &spsSize, u_int8_t *&pps, unsigned &ppsSize) {
    ...
    do {
        ...
        for(i = 0; i < numSPSs; ++i) {
            ...
            if(spsSize1 > NUM_BYTES_REMAINING)
                break;
        ...
        for(i = 0; i < numPPSs; ++i) {
            ...
            if(ppsSize1 > NUM_BYTES_REMAINING)
                break;
            u_int8_t nal_unit_type = ptr[0] & 0x1F;
...
void MatroskaFile::getH265ConfigData(MatroskaTrack const *track, u_int8_t *&vps, unsigned &vpsSize, u_int8_t *&sps, unsigned &spsSize, u_int8_t *&pps,
                                     unsigned &ppsSize) {
    vps = sps = pps = NULL;
...
void MatroskaFile ::getVorbisOrTheoraConfigData(MatroskaTrack const *track, u_int8_t *&identificationHeader, unsigned &identificationHeaderSize,
    ...
    do {
        ...
        if(n == 0 || p == NULL)
            break;  // we have no 'Codec Private' data
...
FramedSource *MatroskaFile ::createSourceForStreaming(FramedSource *baseSource, unsigned trackNumber, unsigned &estBitrate,
...
RTPSink *MatroskaFile ::createRTPSinkForTrackNumber(unsigned trackNumber, Groupsock *rtpGroupsock, unsigned char rtpPayloadTypeIfDynamic) {
    ...
    do {
        ...
        if(strcmp(track->mimeType, "audio/L16") == 0) {
            ...
            result = MPEG4GenericRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->samplingFrequency, "audio", "AAC-hbr", configStr,
                                                    track->numChannels);
            delete[] configStr;
        }
        else if(strcmp(track->mimeType, "audio/AC3") == 0) {
            ...
            result = SimpleRTPSink ::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, 48000, "audio", "OPUS", 2,
            ...
            if(strcmp(track->mimeType, "video/THEORA") == 0) {
                result = TheoraVideoRTPSink ::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, identificationHeader, identificationHeaderSize,
                                                        commentHeader, commentHeaderSize, setupHeader, setupHeaderSize);
            }
            else {  // Vorbis
                result = VorbisAudioRTPSink ::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->samplingFrequency, track->numChannels,
                                                        identificationHeader, identificationHeaderSize, commentHeader, commentHeaderSize, setupHeader,
                                                        setupHeaderSize);
            }
            delete[] identificationHeader;
            delete[] commentHeader;
            ...
            result = RawVideoRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->pixelWidth, track->pixelHeight, track->bitDepth,
    ...
    do {
        ...
        if(strcmp(track->mimeType, "video/H264") == 0) {
            ...
            char *sPropParameterSetsStr =
                new char[(sps_base64 == NULL ? 0 : strlen(sps_base64)) + (pps_base64 == NULL ? 0 : strlen(pps_base64)) + 10 /*more than enough space*/];
            sprintf(sPropParameterSetsStr, "%s,%s", sps_base64, pps_base64);
            delete[] sps_base64;
            delete[] pps_base64;

            result = H264VideoFileSink::createNew(envir(), fileName, sPropParameterSetsStr,
            ...
            result = H265VideoFileSink::createNew(envir(), fileName, vps_base64, sps_base64, pps_base64,
                                                  MAX_KEY_FRAME_SIZE);  // extra large buffer size for large key frames
            delete[] vps_base64;
            delete[] sps_base64;
            delete[] pps_base64;
        ...
        if(createOggFileSink) {
            ...
            if(strcmp(track->mimeType, "audio/VORBIS") == 0 || strcmp(track->mimeType, "video/THEORA") == 0) {
                ...
                u_int8_t *setupHeader;
                unsigned  setupHeaderSize;
                getVorbisOrTheoraConfigData(track, identificationHeader, identificationHeaderSize, commentHeader, commentHeaderSize, setupHeader,
                                            setupHeaderSize);
                u_int32_t identField = 0xFACADE;  // Can we get a real value from the file somehow?
                configStr = generateVorbisOrTheoraConfigStr(identificationHeader, identificationHeaderSize, commentHeader, commentHeaderSize, setupHeader,
...
MatroskaTrackTable::MatroskaTrackTable() : fTable(HashTable::create(ONE_WORD_HASH_KEYS)) {}
