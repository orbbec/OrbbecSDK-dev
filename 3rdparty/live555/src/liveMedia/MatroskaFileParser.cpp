...
MatroskaFileParser::MatroskaFileParser(MatroskaFile &ourFile, FramedSource *inputSource, FramedSource::onCloseFunc *onEndFunc, void *onEndClientData,
    ...
    while(fCurOffsetInFile < fLimitOffsetInFile) {
        ...
        case MATROSKA_ID_TRACK_ENTRY: {  // 'Track Entry' header: enter this
            ...
            if(track != NULL && track->trackNumber == 0)
                delete track;  // We had a previous "MatroskaTrack" object that was never used
        ...
        case MATROSKA_ID_TRACK_TYPE: {
            ...
            if(parseEBMLVal_unsigned(size, trackType) && track != NULL) {
                ...
                track->trackType = trackType == 1      ? MATROSKA_TRACK_TYPE_VIDEO
                                   ...
                                   : trackType == 0x11 ? MATROSKA_TRACK_TYPE_SUBTITLE
                ...
                if(track != NULL) {
                    ...
                    if(track->codecID != NULL) {
                        if(strcmp(track->codecID, "V_MPEG4/ISO/AVC") == 0) {  // H.264
                            ...
                            if(codecPrivateSize < 23 || (codecPrivate[13] & 0xF0) != 0xF0 || (codecPrivate[15] & 0xFC) != 0xFC
                               || (codecPrivate[16] & 0xFC) != 0xFC || (codecPrivate[17] & 0xF8) != 0xF8 || (codecPrivate[18] & 0xF8) != 0xF8) {
...
void MatroskaFileParser::parseBlock() {
    ...
    do {
        ...
        fprintf(stderr, "\ttrack number %d, timecode %d (=> %f seconds), %s lacing\n", fBlockTrackNumber, fBlockTimecode,
                (fClusterTimecode + fBlockTimecode) * (fOurFile.fTimecodeScale / 1000000000.0),
                (lacingType == NoLacing)          ? "no"
                : (lacingType == XiphLacing)      ? "Xiph"
                : (lacingType == FixedSizeLacing) ? "fixed-size"
                                                  : "EBML");
#endif
    ...
    do {
        ...
        if(track->defaultDuration == 0) {
            ...
            if(demuxedTrack->durationImbalance() > 0) {
                ...
                adjustment =
                    (unsigned)(-demuxedTrack->durationImbalance()) < durationInMicroseconds ? demuxedTrack->durationImbalance() : -(int)durationInMicroseconds;
            }
