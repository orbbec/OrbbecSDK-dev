...
OggDemux *OggFile ::newDemux(OggDemuxOnDeletionFunc *onDeletionFunc, void *objectToNotify) {
...
RTPSink *OggFile ::createRTPSinkForTrackNumber(u_int32_t trackNumber, Groupsock *rtpGroupsock, unsigned char rtpPayloadTypeIfDynamic) {
    ...
    if(strcmp(track->mimeType, "audio/VORBIS") == 0) {
        ...
        result = VorbisAudioRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->samplingFrequency, track->numChannels,
                                               track->vtoHdrs.header[0], track->vtoHdrs.headerSize[0], track->vtoHdrs.header[1], track->vtoHdrs.headerSize[1],
        ...
        // that we read when we initially read the headers at the start of the file:
        result = TheoraVideoRTPSink::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->vtoHdrs.header[0], track->vtoHdrs.headerSize[0],
                                               track->vtoHdrs.header[1], track->vtoHdrs.headerSize[1], track->vtoHdrs.header[2], track->vtoHdrs.headerSize[2]);
    }
...
OggTrackTable::OggTrackTable() : fTable(HashTable::create(ONE_WORD_HASH_KEYS)) {}
...
OggTrack::OggTrack() : trackNumber(0), mimeType(NULL), samplingFrequency(48000), numChannels(2), estBitrate(100) {  // default settings
