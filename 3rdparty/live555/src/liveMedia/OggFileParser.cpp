...
u_int8_t OggFileParser::parseInitialPage() {
    ...
    if(track != NULL) {  // sanity check
        ...
        if(track->mimeType != NULL
           && (strcmp(track->mimeType, "audio/VORBIS") == 0 || strcmp(track->mimeType, "video/THEORA") == 0 || strcmp(track->mimeType, "audio/OPUS") == 0)) {
...
class LEBitVector {
public:
    LEBitVector(u_int8_t const *p, unsigned numBytes) : fPtr(p), fEnd(&p[numBytes]), fNumBitsRemainingInCurrentByte(8) {}
...
static Boolean validateCommentHeader(u_int8_t const *p, unsigned headerSize, unsigned isOpus = 0) {
    ...
    if(strcmp(track->mimeType, "audio/VORBIS") == 0) {
        ...
        if(firstByte == 1) {  // "identification" header
            ...
            fprintf(stderr, "\t%u Hz, %u-channel, %u kbps (est), block sizes: %u,%u (%u,%u us)\n", track->samplingFrequency, track->numChannels,
                    track->estBitrate, blocksize_0, blocksize_1, track->vtoHdrs.uSecsPerPacket[0], track->vtoHdrs.uSecsPerPacket[1]);
#endif
...
Boolean OggFileParser::deliverPacketWithinPage() {
    ...
    if(packetNum < fPacketSizeTable->numCompletedPackets - 1 || fPacketSizeTable->lastPacketIsIncomplete) {
