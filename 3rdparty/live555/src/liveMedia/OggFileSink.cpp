...
OggFileSink *OggFileSink ::createNew(UsageEnvironment &env, char const *fileName, unsigned samplingFrequency, char const *configStr, unsigned bufferSize,
...
OggFileSink::OggFileSink(UsageEnvironment &env, FILE *fid, unsigned samplingFrequency, char const *configStr, unsigned bufferSize,
                         char const *perFrameFileNamePrefix)
...
void OggFileSink::addData(unsigned char const *data, unsigned dataSize, struct timeval presentationTime) {
    ...
    for(unsigned i = 0; i < numPagesToWrite; ++i) {
        ...
        if(i > 0)
            header_type_flag |= 0x01;  // 'continuation'
        ...
        if(i < numPagesToWrite - 1) {
            ...
            fPageHeaderBytes[6] = fPageHeaderBytes[7] = fPageHeaderBytes[8] = fPageHeaderBytes[9] = fPageHeaderBytes[10] = fPageHeaderBytes[11] =
                fPageHeaderBytes[12] = fPageHeaderBytes[13] = 0xFF;
        }
        else {
...
void OggFileSink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime) {
    if(!fHaveWrittenFirstFrame) {
        ...
        if(fConfigStr != NULL && fConfigStr[0] != '\0') {
            ...
            if(identificationHdrSize >= 42 && strncmp((const char *)&identificationHdr[1], "theora", 6) == 0) {
