...
OnDemandServerMediaSubsession ::OnDemandServerMediaSubsession(UsageEnvironment &env, Boolean reuseFirstSource, portNumBits initialPortNum,
                                                              Boolean multiplexRTCPWithRTP)
    : ServerMediaSubsession(env),
      fSDPLines(NULL),
      fMIKEYStateMessage(NULL),
      fMIKEYStateMessageSize(0),
...
char const *OnDemandServerMediaSubsession::sdpLines(int addressFamily) {
    if(fSDPLines == NULL) {
        ...
        if(dummyRTPSink != NULL) {
            if(fParentSession->streamingUsesSRTP) {
                fMIKEYStateMessage = dummyRTPSink->setupForSRTP(fParentSession->streamingIsEncrypted, fMIKEYStateMessageSize);
            }

            if(dummyRTPSink->estimatedBitrate() > 0)
...
void OnDemandServerMediaSubsession ::getStreamParameters(unsigned clientSessionId, struct sockaddr_storage const &clientAddress, Port const &clientRTPPort,
    ...
    if(fLastStreamToken != NULL && fReuseFirstSource) {
        ...
        if(clientRTPPort.num() != 0 || tcpSocketNum >= 0) {  // Normal case: Create destinations
            ...
            if(clientRTCPPort.num() == 0) {
                ...
                unsigned char rtpPayloadType = 96 + trackNumber() - 1;  // if dynamic
                rtpSink                      = mediaSource == NULL ? NULL : createNewRTPSink(rtpGroupsock, rtpPayloadType, mediaSource);
                if(rtpSink != NULL) {
                    if(fParentSession->streamingUsesSRTP) {
...
void OnDemandServerMediaSubsession::startStream(unsigned clientSessionId, void *streamToken, TaskFunc *rtcpRRHandler, void *rtcpRRHandlerClientData,
    ...
    if(streamState != NULL) {
        streamState->startPlaying(destinations, clientSessionId, rtcpRRHandler, rtcpRRHandlerClientData, serverRequestAlternativeByteHandler,
...
void OnDemandServerMediaSubsession::seekStream(unsigned /*clientSessionId*/, void *streamToken, double &seekNPT, double streamDuration, u_int64_t &numBytes) {
...
void OnDemandServerMediaSubsession::seekStream(unsigned /*clientSessionId*/, void *streamToken, char *&absStart, char *&absEnd) {
    // Seeking isn't allowed if multiple clients are receiving data from the same source:
    if(fReuseFirstSource)
        return;
...
void OnDemandServerMediaSubsession::nullSeekStream(unsigned /*clientSessionId*/, void *streamToken, double streamEndTime, u_int64_t &numBytes) {
    numBytes = 0;  // by default: unknown
...
void OnDemandServerMediaSubsession ::getRTPSinkandRTCP(void *streamToken, RTPSink const *&rtpSink, RTCPInstance const *&rtcp) {
    if(streamToken == NULL) {
        rtpSink = NULL;
        rtcp    = NULL;
        return;
...
void OnDemandServerMediaSubsession::seekStreamSource(FramedSource * /*inputSource*/, double & /*seekNPT*/, double /*streamDuration*/, u_int64_t &numBytes) {
...
Groupsock *OnDemandServerMediaSubsession ::createGroupsock(struct sockaddr_storage const &addr, Port port) {
...
void OnDemandServerMediaSubsession ::sendRTCPAppPacket(u_int8_t subtype, char const *name, u_int8_t *appDependentData, unsigned appDependentDataSize) {
...
void OnDemandServerMediaSubsession ::setSDPLinesFromRTPSink(RTPSink *rtpSink, FramedSource *inputSource, unsigned estBitrate) {
    ...
    char const *const sdpFmt = "m=%s %u RTP/%sAVP %d\r\n"
                               "c=IN %s %s\r\n"
                               "b=AS:%u\r\n"
                               "%s"
                               "%s"
                               ...
                               "%s"
                               "a=control:%s\r\n";
    unsigned sdpFmtSize = strlen(sdpFmt) + strlen(mediaType) + 5 /* max short len */ + 1 + 3 /* max char len */
                          + 3 /*IP4 or IP6*/ + strlen(ipAddressStr.val()) + 20               /* max int len */
                          + strlen(rtpmapLine) + strlen(keyMgmtLine) + strlen(rtcpmuxLine) + strlen(rangeLine) + strlen(auxSDPLine) + strlen(trackId());
...
StreamState::StreamState(OnDemandServerMediaSubsession &master, Port const &serverRTPPort, Port const &serverRTCPPort, RTPSink *rtpSink, BasicUDPSink *udpSink,
                         unsigned totalBW, FramedSource *mediaSource, Groupsock *rtpGS, Groupsock *rtcpGS)
    : fMaster(master),
...
void StreamState ::startPlaying(Destinations *dests, unsigned clientSessionId, TaskFunc *rtcpRRHandler, void *rtcpRRHandlerClientData,
    ...
    if(dests->isTCP) {
        ...
        if(fRTPSink != NULL) {
            ...
            RTPInterface ::setServerRequestAlternativeByteHandler(fRTPSink->envir(), dests->tcpSocketNum, serverRequestAlternativeByteHandler,
                                                                  serverRequestAlternativeByteHandlerClientData);
        ...
        if(fRTCPInstance != NULL) {
            ...
            fRTCPInstance->setSpecificRRHandler(tcpSocketNumAsAddress, dests->rtcpChannelId, rtcpRRHandler, rtcpRRHandlerClientData);
        }
    }
    else {
        ...
        if(fRTCPInstance != NULL) {
            fRTCPInstance->setSpecificRRHandler(dests->addr, dests->rtcpPort, rtcpRRHandler, rtcpRRHandlerClientData);
        }
    }
...
void StreamState::pause() {
    ...
    if(fMediaSource != NULL)
        fMediaSource->stopGettingFrames();
    fAreCurrentlyPlaying = False;
}
...
void StreamState::sendRTCPAppPacket(u_int8_t subtype, char const *name, u_int8_t *appDependentData, unsigned appDependentDataSize) {
    if(fRTCPInstance != NULL) {
