...
public:
    RTCPSourceRecord(struct sockaddr_storage const &addr, Port const &port) : addr(addr), port(port) {}
...
char const *PassiveServerMediaSubsession::sdpLines(int /*addressFamily*/) {
    if(fSDPLines == NULL) {
        ...
        char const *const sdpFmt = "m=%s %d RTP/%sAVP %d\r\n"
                                   "c=IN %s %s/%d\r\n"
                                   "b=AS:%u\r\n"
                                   "%s"
                                   "%s"
...
void PassiveServerMediaSubsession ::getStreamParameters(unsigned clientSessionId, struct sockaddr_storage const &clientAddress, Port const & /*clientRTPPort*/,
                                                        Port const &clientRTCPPort, int /*tcpSocketNum*/, unsigned char /*rtpChannelId*/,
                                                        unsigned char /*rtcpChannelId*/, TLSState * /*tlsState*/, struct sockaddr_storage &destinationAddress,
                                                        u_int8_t &destinationTTL, Boolean &isMulticast, Port &serverRTPPort, Port &serverRTCPPort,
                                                        void *&streamToken) {
    isMulticast   = True;
...
void PassiveServerMediaSubsession::startStream(unsigned clientSessionId, void * /*streamToken*/, TaskFunc *rtcpRRHandler, void *rtcpRRHandlerClientData,
