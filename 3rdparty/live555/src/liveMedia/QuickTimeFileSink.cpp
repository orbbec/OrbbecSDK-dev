...
public:
    ...
    ChunkDescriptor *extendChunk(int64_t newOffsetInFile, unsigned newSize, unsigned newFrameSize, unsigned newFrameDuration,
...
QuickTimeFileSink::QuickTimeFileSink(UsageEnvironment &env, MediaSession &inputSession, char const *outputFileName, unsigned bufferSize,
                                     unsigned short movieWidth, unsigned short movieHeight, unsigned movieFPS, Boolean packetLossCompensate,
                                     Boolean syncStreams, Boolean generateHintTracks, Boolean generateMP4Format)
    : Medium(env),
    ...
    while((subsession = iter.next()) != NULL) {
        ...
        if(generateHintTracks) {
            ...
            SubsessionIOState *hintTrack = new SubsessionIOState(*this, *subsession);
...
QuickTimeFileSink *QuickTimeFileSink::createNew(UsageEnvironment &env, MediaSession &inputSession, char const *outputFileName, unsigned bufferSize,
                                                unsigned short movieWidth, unsigned short movieHeight, unsigned movieFPS, Boolean packetLossCompensate,
                                                Boolean syncStreams, Boolean generateHintTracks, Boolean generateMP4Format) {
    QuickTimeFileSink *newSink = new QuickTimeFileSink(env, inputSession, outputFileName, bufferSize, movieWidth, movieHeight, movieFPS, packetLossCompensate,
                                                       syncStreams, generateHintTracks, generateMP4Format);
...
void QuickTimeFileSink ::noteRecordedFrame(MediaSubsession & /*inputSubsession*/, unsigned /*packetDataSize*/, struct timeval const & /*presentationTime*/) {
    // Default implementation: Do nothing
}

Boolean QuickTimeFileSink::startPlaying(afterPlayingFunc *afterFunc, void *afterClientData) {
...
void QuickTimeFileSink ::afterGettingFrame(void *clientData, unsigned packetDataSize, unsigned numTruncatedBytes, struct timeval presentationTime,
    ...
    if(numTruncatedBytes > 0) {
        ioState->envir() << "QuickTimeFileSink::afterGettingFrame(): The input frame data was too large for our buffer.  " << numTruncatedBytes
...
void QuickTimeFileSink::onRTCPBye(void *clientData) {
    ...
    ioState->envir() << "Received RTCP \"BYE\" on \"" << subsession.mediumName() << "/" << subsession.codecName() << "\" subsession (after " << secsDiff
                     << " seconds)\n";
...
static Boolean timevalGE(struct timeval const &tv1, struct timeval const &tv2) {
    return (unsigned)tv1.tv_sec > (unsigned)tv2.tv_sec || (tv1.tv_sec == tv2.tv_sec && (unsigned)tv1.tv_usec >= (unsigned)tv2.tv_usec);
}
...
void QuickTimeFileSink::completeOutputFile() {
    ...
    while((subsession = iter.next()) != NULL) {
        ...
        if(headChunk != NULL && timevalGE(fFirstDataTime, headChunk->fPresentationTime)) {
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink &sink, MediaSubsession &subsession)
...
Boolean SubsessionIOState::setQTstate() {
    ...
    char const *noCodecWarning3 = "\" track, so we'll insert a dummy \"????\" Media Data Atom instead.  A separate, codec-specific editing pass will be needed "
    ...
    do {
        ...
        if(isHintTrack()) {
            ...
            fQTMediaDataAtomCreator        = &QuickTimeFileSink::addAtom_soundMediaGeneral;  // by default
            fQTSoundSampleVersion          = 0;                                              // by default

            // Make sure that our subsession's codec is one that we can handle:
            if(strcmp(fOurSubsession.codecName(), "X-QT") == 0 || strcmp(fOurSubsession.codecName(), "X-QUICKTIME") == 0) {
            ...
            else if(strcmp(fOurSubsession.codecName(), "MPEG4-GENERIC") == 0 || strcmp(fOurSubsession.codecName(), "MP4A-LATM") == 0) {
                ...
                unsigned frequencyFromConfig = samplingFrequencyFromAudioSpecificConfig(fOurSubsession.fmtp_config());
                if(frequencyFromConfig != 0)
                    fQTTimeScale = frequencyFromConfig;
            }
            else {
                envir() << noCodecWarning1 << "Audio" << noCodecWarning2 << fOurSubsession.codecName() << noCodecWarning3;
            ...
            if(strcmp(fOurSubsession.codecName(), "X-QT") == 0 || strcmp(fOurSubsession.codecName(), "X-QUICKTIME") == 0) {
                ...
                envir() << noCodecWarning1 << "Video" << noCodecWarning2 << fOurSubsession.codecName() << noCodecWarning3;
                fQTMediaDataAtomCreator = &QuickTimeFileSink::addAtom_dummy;
                fQTEnableTrack          = False;  // disable this track in the movie
            }
        }
        else {
            envir() << "Warning: We don't implement a QuickTime Media Handler for media type \"" << fOurSubsession.mediumName() << "\"";
            break;
        }

#ifdef QT_SUPPORT_PARTIALLY_ONLY
        envir()
            << "Warning: We don't have sufficient codec-specific information (e.g., sample sizes) to fully generate the \"" << fOurSubsession.mediumName()
    ...
    envir() << ", so a track for the \"" << fOurSubsession.mediumName() << "/" << fOurSubsession.codecName()
...
void SubsessionIOState::afterGettingFrame(unsigned packetDataSize, struct timeval presentationTime) {
    ...
    if(fOurSubsession.rtpSource() != NULL  // we have a RTP stream
       && fQTMediaDataAtomCreator == &QuickTimeFileSink::addAtom_genericMedia) {
        QuickTimeGenericRTPSource *         rtpSource = (QuickTimeGenericRTPSource *)fOurSubsession.rtpSource();
...
void SubsessionIOState::useFrame(SubsessionBuffer &buffer) {
    ...
    if(!fOurSink.fSyncStreams || fQTcomponentSubtype != fourChar('v', 'i', 'd', 'e')) {
        ...
        if(ppt.tv_sec != 0 || ppt.tv_usec != 0) {
            ...
            double duration = (presentationTime.tv_sec - ppt.tv_sec) + (presentationTime.tv_usec - ppt.tv_usec) / 1000000.0;
            if(duration < 0.0)
                duration = 0.0;
    ...
    if(hasHintTrack() && fOurSubsession.rtpSource() != NULL) {
        ...
        if(fHaveBeenSynced) {
            fHintTrackForUs->useFrameForHinting(frameSize, presentationTime, sampleNumberOfFrameStart);
        }
    }
}

void SubsessionIOState::useFrameForHinting(unsigned frameSize, struct timeval presentationTime, unsigned startSampleNumber) {
    ...
    Boolean hackm4a_generic    = strcmp(fOurSubsession.mediumName(), "audio") == 0 && strcmp(fOurSubsession.codecName(), "MPEG4-GENERIC") == 0;
    Boolean hackm4a_latm       = strcmp(fOurSubsession.mediumName(), "audio") == 0 && strcmp(fOurSubsession.codecName(), "MP4A-LATM") == 0;
    ...
    if(ppt.tv_sec != 0 || ppt.tv_usec != 0) {
        ...
        unsigned short numPTEntries                = (fPrevFrameState.frameSize + (maxPacketSize - 1)) / maxPacketSize;  // normal case
        ...
        for(unsigned i = 0; i < numPTEntries; ++i) {
            ...
            unsigned dataFrameSize = (i + 1 < numPTEntries) ? maxPacketSize : fPrevFrameState.frameSize - i * maxPacketSize;  // normal case
            ...
            unsigned short const bytesPerCompressionBlock   = fTrackHintedByUs->fQTBytesPerFrame;
            unsigned short const samplesPerCompressionBlock = fTrackHintedByUs->fQTSamplesPerFrame;
        ...
        fQTTotNumSamples += useFrame1(hintSampleSize, ppt, hintSampleDuration, hintSampleDestFileOffset);
    }
    ...
    fPrevFrameState.rtpHeader         = rs->curPacketMarkerBit() << 23 | (rs->rtpPayloadFormat() & 0x7F) << 16;
    if(hack263) {
        ...
        if(sizeLength + indexLength != 16) {
            envir() << "Warning: unexpected 'sizeLength' " << sizeLength << " and 'indexLength' " << indexLength << "seen when creating hint track\n";
        }
...
unsigned SubsessionIOState::useFrame1(unsigned sourceDataSize, struct timeval presentationTime, unsigned frameDuration, int64_t destFileOffset) {
    ...
    if(fTailChunk == NULL) {
        newTailChunk = fHeadChunk = new ChunkDescriptor(destFileOffset, sourceDataSize, frameSize, frameDuration, presentationTime);
    }
    else {
        newTailChunk = fTailChunk->extendChunk(destFileOffset, sourceDataSize, frameSize, frameDuration, presentationTime);
    }
...
void SubsessionIOState::setHintTrack(SubsessionIOState *hintedTrack, SubsessionIOState *hintTrack) {
    if(hintedTrack != NULL)
...
SyncFrame::SyncFrame(unsigned frameNum) : nextSyncFrame(NULL), sfFrameNum(frameNum) {}
...
ChunkDescriptor ::ChunkDescriptor(int64_t offsetInFile, unsigned size, unsigned frameSize, unsigned frameDuration, struct timeval presentationTime)
    : fNextChunk(NULL),
...
ChunkDescriptor *ChunkDescriptor ::extendChunk(int64_t newOffsetInFile, unsigned newSize, unsigned newFrameSize, unsigned newFrameDuration,
                                               struct timeval newPresentationTime) {
    ...
    ChunkDescriptor *newDescriptor = new ChunkDescriptor(newOffsetInFile, newSize, newFrameSize, newFrameDuration, newPresentationTime);
...
unsigned QuickTimeFileSink::addArbitraryString(char const *str, Boolean oneByteLength) {
    unsigned size = 0;
    if(oneByteLength) {
        // Begin with a byte containing the string length:
        unsigned strLength = strlen(str);
        if(strLength >= 256) {
            envir() << "QuickTimeFileSink::addArbitraryString(\"" << str << "\") saw string longer than we know how to handle (" << strLength << "\n";
        }
...
void QuickTimeFileSink::setWord64(int64_t filePosn, u_int64_t size) {
    ...
    // One of the SeekFile64()s failed, probable because we're not a seekable file
    envir() << "QuickTimeFileSink::setWord64(): SeekFile64 failed (err " << envir().getErrno() << ")\n";
}
...
#define addEdit1(duration, trackPosition)                                               \
    do {                                                                                \
        unsigned trackDuration = (unsigned)((2 * (duration)*movieTimeScale() + 1) / 2); \
        /* in movie time units */                                                       \
...
while(chunk != NULL) {
    ...
    double                movieDurationOfEdit = (chunkStartTime.tv_sec - editStartTime.tv_sec) + (chunkStartTime.tv_usec - editStartTime.tv_usec) / 1000000.0;
    trackDurationOfEdit                       = (currentTrackPosition - editTrackPosition) / (double)(fCurrentIOState->fQTTimeScale);
...
unsigned QuickTimeFileSink::addAtom_hdlr2() {
    ...
    size += addWord(0x00000000);  // Version+Flags
    size += addWord(0x00000001);  // Number of entries
    SubsessionIOState::atomCreationFunc mediaDataAtomCreator = fCurrentIOState->fQTMediaDataAtomCreator;
    ...
    unsigned QuickTimeFileSink::addAtom_genericMedia() {
        ...
        QuickTimeGenericRTPSource *rtpSource = (QuickTimeGenericRTPSource *)fCurrentIOState->fOurSubsession.rtpSource();
        unsigned                   size      = 0;
        if(rtpSource != NULL) {
            QuickTimeGenericRTPSource::QTState &qtState = rtpSource->qtState;
        ...
        unsigned QuickTimeFileSink::addAtom_soundMediaGeneral() {
            ...
            unsigned short numChannels = (unsigned short)(fCurrentIOState->fOurSubsession.numChannels());
                ...
                unsigned QuickTimeFileSink::addAtom_mp4a() {
                    ...
                    unsigned QuickTimeFileSink::addAtom_rtp() {
                        ...
                        if(fCurrentIOState->fHeadSyncFrame != NULL) {
                            ...
                            for(ChunkDescriptor *chunk = fCurrentIOState->fHeadChunk; chunk != NULL; chunk = chunk->fNextChunk)
                                totNumFrames += chunk->fNumFrames;

                            while(currentSyncFrame != NULL) {
                                if(currentSyncFrame->sfFrameNum >= totNumFrames)
                                    break;  // sanity check
                        ...
                        ChunkDescriptor *chunk                = fCurrentIOState->fHeadChunk;
                        while(chunk != NULL) {
                            double bps = (double)(chunk->fFrameSize) / (fCurrentIOState->fQTSamplesPerFrame);
                            if(bps < 1.0) {
                        ...
                        if(haveSingleEntryTable) {
                            if(fCurrentIOState->isHintTrack() && fCurrentIOState->fHeadChunk != NULL) {
                                sampleSize = fCurrentIOState->fHeadChunk->fFrameSize / fCurrentIOState->fQTSamplesPerFrame;
                            }
                            else {
                        ...
                        if(!haveSingleEntryTable) {
                            ...
                            ChunkDescriptor *chunk = fCurrentIOState->fHeadChunk;
                            while(chunk != NULL) {
                                unsigned numSamples = chunk->fNumFrames * (fCurrentIOState->fQTSamplesPerFrame);
                                unsigned sampleSize = chunk->fFrameSize / (fCurrentIOState->fQTSamplesPerFrame);
                        ...
                        sprintf(description, "Hinted %s track", fCurrentIOState->fOurSubsession.mediumName());
                        ...
                        unsigned QuickTimeFileSink::addAtom_sdp() {
                            ...
                            if(rtpSource != NULL) {
                                ...
                                sprintf(rtpmapString, "%s/%d", ourSubsession.codecName(), rtpSource->timestampFrequency());
