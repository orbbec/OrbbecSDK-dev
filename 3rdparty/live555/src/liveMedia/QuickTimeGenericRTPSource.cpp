...
QuickTimeGenericRTPSource *QuickTimeGenericRTPSource::createNew(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat,
...
QuickTimeGenericRTPSource ::QuickTimeGenericRTPSource(UsageEnvironment &env, Groupsock *RTPgs, unsigned char rtpPayloadFormat, unsigned rtpTimestampFrequency,
...
Boolean QuickTimeGenericRTPSource ::processSpecialHeader(BufferedPacket *packet, unsigned &resultSpecialHeaderSize) {
    ...
    if(Q) {  // A "QuickTime Payload Description" follows
        ...
        fprintf(stderr, "\tmediaType: '%c%c%c%c', timescale: %d, %d bytes of TLVs left\n", mediaType >> 24, (mediaType & 0xFF0000) >> 16,
                (mediaType & 0xFF00) >> 8, mediaType & 0xFF, qtState.timescale, payloadDescriptionLength);
#endif
        ...
        if(payloadDescriptionLength > 0)
            return False;  // malformed TLV data
        headerStart += padding;
    }
...
QTGenericBufferedPacket ::QTGenericBufferedPacket(QuickTimeGenericRTPSource &ourSource) : fOurSource(ourSource) {}
