...
public:
    RTCPMemberDatabase(RTCPInstance &ourRTCPInstance)
        : fOurRTCPInstance(ourRTCPInstance), fNumMembers(1 /*ourself*/), fTable(HashTable::create(ONE_WORD_HASH_KEYS)) {}
...
RTCPInstance::RTCPInstance(UsageEnvironment &env, Groupsock *RTCPgs, unsigned totSessionBW, unsigned char const *cname, RTPSink *sink, RTPSource *source,
                           Boolean isSSMTransmitter, SRTPCryptographicContext *crypto)
    : Medium(env),
      fRTCPInterface(this, RTCPgs),
      fTotSessionBW(totSessionBW),
      fSink(sink),
      fSource(source),
      fIsSSMTransmitter(isSSMTransmitter),
      fCrypto(crypto),
    ...
    if(isSSMTransmitter)
        RTCPgs->multicastSendOnly();  // don't receive multicast
...
void RTCPInstance::noteArrivingRR(struct sockaddr_storage const &fromAddressAndPort, int tcpSocketNum, unsigned char tcpStreamChannelId) {
    ...
    if(fSpecificRRHandlerTable != NULL) {
        ...
        RRHandlerRecord *rrHandler = (RRHandlerRecord *)(fSpecificRRHandlerTable->Lookup(fromAddress, fromPort));
        if(rrHandler != NULL) {
...
RTCPInstance *RTCPInstance::createNew(UsageEnvironment &env, Groupsock *RTCPgs, unsigned totSessionBW, unsigned char const *cname, RTPSink *sink,
                                      RTPSource *source, Boolean isSSMTransmitter, SRTPCryptographicContext *crypt) {
    return new RTCPInstance(env, RTCPgs, totSessionBW, cname, sink, source, isSSMTransmitter, crypt);
}
...
void RTCPInstance::setByeHandler(TaskFunc *handlerTask, void *clientData, Boolean handleActiveParticipantsOnly) {
...
void RTCPInstance::setByeWithReasonHandler(ByeWithReasonHandlerFunc *handlerTask, void *clientData, Boolean handleActiveParticipantsOnly) {
    fByeHandlerTask                  = NULL;
...
void RTCPInstance ::setSpecificRRHandler(struct sockaddr_storage const &fromAddress, Port fromPort, TaskFunc *handlerTask, void *clientData) {
    ...
    RRHandlerRecord *existingRecord = (RRHandlerRecord *)fSpecificRRHandlerTable->Add(fromAddress, fromPort, rrHandler);
    delete existingRecord;  // if any
}

void RTCPInstance ::unsetSpecificRRHandler(struct sockaddr_storage const &fromAddress, Port fromPort) {
    if(fSpecificRRHandlerTable == NULL)
        return;
...
void RTCPInstance::sendAppPacket(u_int8_t subtype, char const *name, u_int8_t *appDependentData, unsigned appDependentDataSize) {
...
void RTCPInstance ::injectReport(u_int8_t const *packet, unsigned packetSize, struct sockaddr_storage const &fromAddress) {
...
void RTCPInstance::incomingReportHandler1() {
    do {
        if(fNumBytesAlreadyRead >= maxRTCPPacketSize) {
            envir() << "RTCPInstance error: Hit limit when reading incoming packet over TCP. (fNumBytesAlreadyRead (" << fNumBytesAlreadyRead
                    << ") >= maxRTCPPacketSize (" << maxRTCPPacketSize
        ...
        Boolean readResult = fRTCPInterface.handleRead(&fInBuf[fNumBytesAlreadyRead], maxRTCPPacketSize - fNumBytesAlreadyRead, numBytesRead, fromAddress,
...
void RTCPInstance ::processIncomingReport(unsigned packetSize, struct sockaddr_storage const &fromAddressAndPort, int tcpSocketNum,
                                          unsigned char tcpStreamChannelId) {
    do {
        if(fCrypto != NULL) {  // The packet is assumed to be SRTCP.  Verify/decrypt it first:
            unsigned newPacketSize;
            if(!fCrypto->processIncomingSRTCPPacket(fInBuf, packetSize, newPacketSize))
                break;
        ...
        if((rtcpHdr & 0xE0FE0000) != (0x80000000 | (RTCP_PT_SR << 16)) && (rtcpHdr & 0xE0FF0000) != (0x80000000 | (RTCP_PT_APP << 16))) {
#ifdef DEBUG
        ...
        while(1) {
                ...
                if(fSink != NULL) {
                    ...
                    for(unsigned i = 0; i < rc; ++i) {
                        ...
                        if(senderSSRC == fSink->SSRC()) {
                            ...
                            ADVANCE(4);
                            transmissionStats.noteIncomingRR(reportSenderSSRC, fromAddressAndPort, lossStats, highestReceived, jitter, timeLastSR,
                                                             timeSinceLastSR);
                        }
                        else {
                            ADVANCE(4 * 5);
            ...
            case RTCP_PT_BYE: {
                ...
                if((fByeHandlerTask != NULL || fByeWithReasonHandlerTask != NULL)
                   && (!fByeHandleActiveParticipantsOnly || (fSource != NULL && fSource->receptionStatsDB().lookup(reportSenderSSRC) != NULL)
                       || (fSink != NULL && fSink->transmissionStatsDB().lookup(reportSenderSSRC) != NULL))) {
                    callByeHandler = True;
                }
            ...
            case RTCP_PT_SDES: {
                ...
                ADVANCE(-4);
                length += 4;          // hack so that we see the first SSRC/CSRC again
                ...
                if(!chunkOK || length > 0)
                    break;  // bad chunk, or not enough bytes for the last chunk
#endif
                subPacketOK = True;
                break;
            }
...
void RTCPInstance::sendBuiltPacket() {
    ...
    if(fCrypto != NULL) {  // Encrypt/tag the data before sending it:
        ...
        if(!fCrypto->processOutgoingSRTCPPacket(fOutBuf->packet(), reportSize, newReportSize))
            return;
...
void RTCPInstance::addSR() {
    ...
    enqueueCommonReportPrefix(RTCP_PT_SR, fSink->SSRC(), 5 /* extra words in a SR */);
...
void RTCPInstance::schedule(double nextTime) {
    ...
    nextTask()     = envir().taskScheduler().scheduleDelayedTask(usToGo, (TaskFunc *)RTCPInstance::onExpire, this);
}
