...
public:
    tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId, TLSState *tlsState, tcpStreamRecord *next);
...
static SocketDescriptor *lookupSocketDescriptor(UsageEnvironment &env, int sockNum, TLSState *tlsState = NULL, Boolean createIfNotFound = True) {
...
void RTPInterface::setStreamSocket(int sockNum, unsigned char streamChannelId, TLSState *tlsState) {
...
void RTPInterface::addStreamSocket(int sockNum, unsigned char streamChannelId, TLSState *tlsState) {
    if(sockNum < 0)
        return;
...
void RTPInterface::setServerRequestAlternativeByteHandler(UsageEnvironment &env, int socketNum, ServerRequestAlternativeByteHandler *handler,
                                                          void *clientData) {
...
Boolean RTPInterface::sendPacket(unsigned char *packet, unsigned packetSize) {
    ...
    for(tcpStreamRecord *stream = fTCPStreams; stream != NULL; stream = nextStream) {
        ...
        if(!sendRTPorRTCPPacketOverTCP(packet, packetSize, stream->fStreamSocketNum, stream->fStreamChannelId, stream->fTLSState)) {
            success = False;
        }
    }
...
Boolean RTPInterface::handleRead(unsigned char *buffer, unsigned bufferMaxSize, unsigned &bytesRead, struct sockaddr_storage &fromAddress, int &tcpSocketNum,
    ...
    if(fNextTCPReadStreamSocketNum < 0) {
        ...
        while((curBytesRead = (fNextTCPReadTLSState != NULL && fNextTCPReadTLSState->isNeeded)
                                  ...
                                  : readSocket(envir(), fNextTCPReadStreamSocketNum, &buffer[bytesRead], curBytesToRead, fromAddress))
              > 0) {
...
Boolean RTPInterface::sendRTPorRTCPPacketOverTCP(u_int8_t *packet, unsigned packetSize, int socketNum, unsigned char streamChannelId, TLSState *tlsState) {
#ifdef DEBUG_SEND
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, TLSState *tlsState, u_int8_t const *data, unsigned dataSize, Boolean forceSendToSucceed) {
    int sendResult =
        (tlsState != NULL && tlsState->isNeeded) ? tlsState->write((char const *)data, dataSize) : send(socketNum, (char const *)data, dataSize, 0 /*flags*/);
        ...
        if(numBytesSentSoFar > 0 || (forceSendToSucceed && envir().getErrno() == EAGAIN)) {
            ...
            if((unsigned)sendResult != numBytesRemainingToSend) {
                ...
                fprintf(stderr, "sendDataOverTCP: blocking send() failed (delivering %d bytes out of %d); closing socket %d\n", sendResult,
                        numBytesRemainingToSend, socketNum);
                fflush(stderr);
#endif
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
    ...
    if(fTCPReadingState != AWAITING_PACKET_DATA) {
        int result = (fTLSState != NULL && fTLSState->isNeeded) ? fTLSState->read(&c, 1) : readSocket(fEnv, fOurSocketNum, &c, 1, dummy);
...
tcpStreamRecord ::tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId, TLSState *tlsState, tcpStreamRecord *next)
    : fNext(next), fStreamSocketNum(streamSocketNum), fStreamChannelId(streamChannelId), fTLSState(tlsState) {}
