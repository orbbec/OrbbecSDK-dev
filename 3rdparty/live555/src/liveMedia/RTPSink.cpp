...
void RTPSink::getTotalBitrate(unsigned &outNumBytes, double &outElapsedTime) {
    ...
    outElapsedTime = (double)(timeNow.tv_sec - fTotalOctetCountStartTime.tv_sec) + (timeNow.tv_usec - fTotalOctetCountStartTime.tv_usec) / 1000000.0;
...
char *RTPSink::rtpmapLine() const {
    if(rtpPayloadType() >= 96) {  // the payload format type is dynamic
        ...
        unsigned rtpmapLineSize = strlen(rtpmapFmt) + 3 /* max char len */ + strlen(rtpPayloadFormatName()) + 20 /* max int len */ + strlen(encodingParamsPart);
...
char *RTPSink::keyMgmtLine() {
    ...
    if(fMIKEYState != NULL && (mikeyMessage = fMIKEYState->generateMessage(mikeyMessageSize)) != NULL) {
...
RTPTransmissionStatsDB::RTPTransmissionStatsDB(RTPSink &rtpSink) : fOurRTPSink(rtpSink), fTable(HashTable::create(ONE_WORD_HASH_KEYS)) {
    fNumReceivers = 0;
}
...
void RTPTransmissionStatsDB ::noteIncomingRR(u_int32_t SSRC, struct sockaddr_storage const &lastFromAddress, unsigned lossStats, unsigned lastPacketNumReceived,
...
RTPTransmissionStatsDB::Iterator ::Iterator(RTPTransmissionStatsDB &receptionStatsDB) : fIter(HashTable::Iterator::create(*(receptionStatsDB.fTable))) {}
...
void RTPTransmissionStats ::noteIncomingRR(struct sockaddr_storage const &lastFromAddress, unsigned lossStats, unsigned lastPacketNumReceived, unsigned jitter,
