...
RTSPClient *RTSPClient::createNew(UsageEnvironment &env, char const *rtspURL, int verbosityLevel, char const *applicationName,
                                  portNumBits tunnelOverHTTPPortNum, int socketNumToServer) {
    return new RTSPClient(env, rtspURL, verbosityLevel, applicationName, tunnelOverHTTPPortNum, socketNumToServer);
}
...
unsigned RTSPClient::sendSetupCommand(MediaSubsession &subsession, responseHandler *responseHandler, Boolean streamOutgoing, Boolean streamUsingTCP,
                                      Boolean forceMulticastOnUnspecified, Authenticator *authenticator) {
...
unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler, double start, double end, float scale,
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession &subsession, responseHandler *responseHandler, double start, double end, float scale,
                                     Authenticator *authenticator) {
    if(fCurrentAuthenticator < authenticator)
        fCurrentAuthenticator = *authenticator;
...
unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler, char const *absStartTime, char const *absEndTime, float scale,
                                     Authenticator *authenticator) {
    if(fCurrentAuthenticator < authenticator)
        fCurrentAuthenticator = *authenticator;
    sendDummyUDPPackets(session);  // hack to improve NAT traversal
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession &subsession, responseHandler *responseHandler, char const *absStartTime, char const *absEndTime,
                                     float scale, Authenticator *authenticator) {
    if(fCurrentAuthenticator < authenticator)
        fCurrentAuthenticator = *authenticator;
    sendDummyUDPPackets(subsession);  // hack to improve NAT traversal
...
unsigned RTSPClient::sendSetParameterCommand(MediaSession &session, responseHandler *responseHandler, char const *parameterName, char const *parameterValue,
                                             Authenticator *authenticator) {
    if(fCurrentAuthenticator < authenticator)
        fCurrentAuthenticator = *authenticator;
...
unsigned RTSPClient::sendGetParameterCommand(MediaSession &session, responseHandler *responseHandler, char const *parameterName, Authenticator *authenticator) {
    if(fCurrentAuthenticator < authenticator)
        fCurrentAuthenticator = *authenticator;
...
Boolean RTSPClient::changeResponseHandler(unsigned cseq, responseHandler *newResponseHandler) {
    ...
    if((request = fRequestsAwaitingConnection.findByCSeq(cseq)) != NULL || (request = fRequestsAwaitingHTTPTunneling.findByCSeq(cseq)) != NULL
...
Boolean RTSPClient::parseRTSPURL(char const *url, char *&username, char *&password, NetAddress &address, portNumBits &portNum, char const **urlSuffix) {
    do {
        ...
        if(addresses.numAddresses() == 0) {
            envir().setResultMsg("Failed to find network address for \"", parseBuffer, "\"");
            break;
        }
...
RTSPClient::RTSPClient(UsageEnvironment &env, char const *rtspURL, int verbosityLevel, char const *applicationName, portNumBits tunnelOverHTTPPortNum,
    ...
    unsigned userAgentNameSize = strlen(applicationName) + strlen(libPrefix) + strlen(libName) + strlen(libVersionStr) + strlen(libSuffix) + 1;
...
unsigned RTSPClient::sendRequest(RequestRecord *request) {
    ...
    do {
        ...
        if(!setRequestFields(request, cmdURL, cmdURLWasAllocated, protocolStr, extraHeaders, extraHeadersWereAllocated)) {
            break;
        }
        ...
        if(contentStrLen > 0) {
            ...
            unsigned    contentLengthHeaderSize = strlen(contentLengthHeaderFmt) + 20 /* max int len */;
        ...
        char const *const cmdFmt  = "%s %s %s\r\n"
                                    ...
                                    "\r\n"
                                    "%s";
        unsigned          cmdSize = strlen(cmdFmt) + strlen(request->commandName()) + strlen(cmdURL) + strlen(protocolStr) + 20 /* max int len */
                           + strlen(authenticatorStr) + fUserAgentHeaderStrLen + strlen(fRequireStr) + strlen(extraHeaders) + strlen(contentLengthHeader)
                           + contentStrLen;
        cmd = new char[cmdSize];
        sprintf(cmd, cmdFmt, request->commandName(), cmdURL, protocolStr, request->cseq(), authenticatorStr, fUserAgentHeaderStr, fRequireStr, extraHeaders,
...
Boolean RTSPClient::setRequestFields(RequestRecord *request, char *&cmdURL, Boolean &cmdURLWasAllocated, char const *&protocolStr, char *&extraHeaders,
    ...
    if(strcmp(request->commandName(), "DESCRIBE") == 0) {
        ...
        if(streamUsingTCP) {  // streaming over the RTSP connection
            ...
            Boolean requestMulticastStreaming = IsMulticastAddress(connectionAddress) || (forceMulticastOnUnspecified && addressIsNull(connectionAddress));
        ...
        if(!parseRTSPURL(fBaseURL, username, password, destAddress, urlPortNum, (char const **)&cmdURL))
            return False;
        if(cmdURL[0] == '\0')
            cmdURL = (char *)"/";
        delete[] username;
        ...
        if(strcmp(request->commandName(), "GET") == 0) {
            ...
            char const *const extraHeadersFmt  = "Host: %s\r\n"
                                                 "x-sessioncookie: %s\r\n"
            ...
            extraHeadersWereAllocated          = True;
            sprintf(extraHeaders, extraHeadersFmt, serverAddressString.val(), fSessionCookie);
        }
        else {  // "POST"
            char const *const extraHeadersFmt  = "Host: %s\r\n"
                                                 "x-sessioncookie: %s\r\n"
    ...
    do {
        ...
        if(connectResult < 0)
            ...
            if(fTLS.isNeeded) {
                ...
                if(connectResult < 0)
                    break;
                if(connectResult > 0 && fVerbosityLevel >= 1)
                    envir() << "...TLS connection completed\n";
            }
...
char *RTSPClient::createAuthenticatorString(char const *cmd, char const *url) {
    ...
    if(auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) {
        ...
        if(auth.nonce() != NULL) {  // Digest authentication
            ...
            sprintf(authenticatorStr, authFmt, auth.username(), auth.realm(), auth.nonce(), url, response);
...
char *RTSPClient::createKeyMgmtString(char const *url, MediaSubsession const &subsession) {
    ...
    if((mikeyState = subsession.getMIKEYState()) == NULL || (mikeyMessage = mikeyState->generateMessage(mikeyMessageSize)) == NULL) {
        keyMgmtStr = strDup("");
    }
    else {
        ...
        sprintf(keyMgmtStr, keyMgmtFmt, url, base64EncodedData);
...
void RTSPClient::handleIncomingRequest() {
    ...
    Boolean  urlIsRTSPS;
    if(!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen, cmdName, sizeof cmdName, urlPreSuffix, sizeof urlPreSuffix, urlSuffix,
                               sizeof urlSuffix, cseq, sizeof cseq, sessionId, sizeof sessionId, contentLength, urlIsRTSPS)) {
        return;
    }
    else {
        if(fVerbosityLevel >= 1) {
        ...
        snprintf(tmpBuf, sizeof tmpBuf, "RTSP/1.0 405 Method Not Allowed\r\nCSeq: %s\r\n\r\n", cseq);
...
Boolean RTSPClient::parseTransportParams(char const *paramsStr, char *&serverAddressStr, portNumBits &serverPortNum, unsigned char &rtpChannelId,
...
Boolean RTSPClient::handlePLAYResponse(MediaSession *session, MediaSubsession *subsession, char const *scaleParamsStr, char const *speedParamsStr,
    ...
    do {
        if(session != NULL) {
            ...
            if(scaleParamsStr != NULL && !parseScaleParam(scaleParamsStr, session->scale()))
                break;
            scaleOK = True;
            if(speedParamsStr != NULL && !parseSpeedParam(speedParamsStr, session->speed()))
                break;
            speedOK = True;
            Boolean startTimeIsNow;
            if(rangeParamsStr != NULL
               && !parseRangeParam(rangeParamsStr, session->playStartTime(), session->playEndTime(), session->_absStartTime(), session->_absEndTime(),
                                   startTimeIsNow))
                break;
            rangeOK = True;
            ...
            Boolean startTimeIsNow;
            if(rangeParamsStr != NULL
               && !parseRangeParam(rangeParamsStr, subsession->_playStartTime(), subsession->_playEndTime(), subsession->_absStartTime(),
        ...
        if(parameterName != NULL && parameterName[0] != '\0') {
            ...
            if(parameterNameLen > 0 && _strncasecmp(resultValueString, parameterName, parameterNameLen) == 0) {
                ...
                if(resultValueString[0] == ':')
                    ++resultValueString;
                while(resultValueString < resultValueStringEnd && (resultValueString[0] == ' ' || resultValueString[0] == '\t')) {
                    ++resultValueString;
                }
            }
        }
    ...
    do {
        ...
        if(fTLS.isNeeded) {
            ...
            if(tlsConnectResult < 0)
                break;  // error in TLS connection
            if(tlsConnectResult > 0 && fVerbosityLevel >= 1)
                envir() << "...TLS connection completed\n";
            if(tlsConnectResult == 0) {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
    ...
    // envir() << "fResponseBuffer:  " << fResponseBuffer; /*<< " new bytes of response data.\n";*/
    ...
    do {
        ...
        if(!endOfHeaders)
            return;  // subsequent reads will be needed to get the complete response
            ...
            while(1) {
                ...
                if(checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
                    ...
                    while((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
                        if(request->cseq() < cseq) {  // assumes that the CSeq counter will never wrap around
                            ...
                            if(fVerbosityLevel >= 1 && strcmp(request->commandName(), "POST") != 0) {
                                envir() << "WARNING: The server did not respond to our \"" << request->commandName() << "\" request (CSeq: " << request->cseq()
                    ...
                    if(oldBaseURLTail != NULL) {
                        unsigned newBaseURLLen = (oldBaseURLTail - fBaseURL) + 8 /* for "/stream=" */ + strlen(headerParamsStr);
                        char    *newBaseURL    = new char[newBaseURLLen + 1];
                        // Note: We couldn't use "asprintf()", because some compilers don't support it
                        sprintf(newBaseURL, "%.*s/stream=%s", (int)(oldBaseURLTail - fBaseURL), fBaseURL, headerParamsStr);
            ...
            if(contentLength > numBodyBytes) {
                ...
                if(fVerbosityLevel >= 1) {
                    envir() << "Have received " << fResponseBytesAlreadySeen << " total bytes of a "
                            << (foundRequest != NULL ? foundRequest->commandName() : "(unknown)") << " RTSP response; awaiting " << numExtraBytesNeeded
                            << " bytes more.\n";
                }
            ...
            if(foundRequest != NULL) {
                ...
                if(responseCode == 200) {
                    ...
                    if(strcmp(foundRequest->commandName(), "SETUP") == 0) {
                        if(!handleSETUPResponse(*foundRequest->subsession(), sessionParamsStr, transportParamsStr, foundRequest->booleanFlags() & 0x1))
                            break;
                    }
                    else if(strcmp(foundRequest->commandName(), "PLAY") == 0) {
                        if(!handlePLAYResponse(foundRequest->session(), foundRequest->subsession(), scaleParamsStr, speedParamsStr, rangeParamsStr,
                                               rtpInfoParamsStr))
                            break;
                    }
                        ...
                        if(!handleGET_PARAMETERResponse(foundRequest->contentStr(), bodyStart, responseEnd))
                            break;
                    }
                }
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const *commandName, responseHandler *handler, MediaSession *session, MediaSubsession *subsession,
                                         u_int32_t booleanFlags, double start, double end, float scale, char const *contentStr)
    : fNext(NULL),
      fCSeq(cseq),
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, responseHandler *handler, char const *absStartTime, char const *absEndTime, float scale,
...
RTSPClient::RequestQueue::RequestQueue(RequestQueue &origQueue) : fHead(NULL), fTail(NULL) {
    RequestRecord *request;
...
HandlerServerForREGISTERCommand *HandlerServerForREGISTERCommand ::createNew(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc, Port ourPort,
...
HandlerServerForREGISTERCommand ::HandlerServerForREGISTERCommand(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc, int ourSocketIPv4,
                                                                  int ourSocketIPv6, Port ourPort, UserAuthenticationDatabase *authDatabase, int verbosityLevel,
                                                                  char const *applicationName)
...
RTSPClient *HandlerServerForREGISTERCommand ::createNewRTSPClient(char const *rtspURL, int verbosityLevel, char const *applicationName, int socketNumToServer) {
...
Boolean HandlerServerForREGISTERCommand ::weImplementREGISTER(char const *cmd /*"REGISTER" or "DEREGISTER"*/, char const * /*proxyURLSuffix*/,
                                                              char      *&responseStr) {
    responseStr = NULL;
...
void HandlerServerForREGISTERCommand ::implementCmd_REGISTER(char const *cmd /*"REGISTER" or "DEREGISTER"*/, char const *url, char const *urlSuffix,
                                                             int socketToRemoteServer, Boolean deliverViaTCP, char const * /*proxyURLSuffix*/) {
