...
RTSPServer *RTSPServer::createNew(UsageEnvironment &env, Port ourPort, UserAuthenticationDatabase *authDatabase, unsigned reclamationSeconds) {
...
Boolean RTSPServer::lookupByName(UsageEnvironment &env, char const *name, RTSPServer *&resultServer) {
...
char *RTSPServer ::rtspURL(ServerMediaSession const *serverMediaSession, int clientSocket, Boolean useIPv6) const {
...
char *RTSPServer::rtspURLPrefix(int clientSocket, Boolean useIPv6) const {
    ...
    if(portNumHostOrder == defaultPortNum) {
        sprintf(urlBuffer, "rtsp%s://%s%s%s/", fWeServeSRTP ? "s" : "", addressPrefixInURL, AddressString(ourAddress).val(), addressSuffixInURL);
    }
    else {
        sprintf(urlBuffer, "rtsp%s://%s%s%s:%hu/", fWeServeSRTP ? "s" : "", addressPrefixInURL, AddressString(ourAddress).val(), addressSuffixInURL,
                portNumHostOrder);
    }
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
    ...
    if(fHTTPServerSocketIPv4 >= 0 || fHTTPServerSocketIPv6 >= 0) {
        ...
        envir().taskScheduler().turnOnBackgroundReadHandling(fHTTPServerSocketIPv4, incomingConnectionHandlerHTTPIPv4, this);
        envir().taskScheduler().turnOnBackgroundReadHandling(fHTTPServerSocketIPv6, incomingConnectionHandlerHTTPIPv6, this);
        return True;
    }
...
Boolean RTSPServer::specialClientUserAccessCheck(int /*clientSocket*/, struct sockaddr_storage const & /*clientAddr*/, char const * /*urlSuffix*/,
                                                 char const * /*username*/) {
    // default implementation; no further access restrictions:
    return True;
}

RTSPServer::RTSPServer(UsageEnvironment &env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort, UserAuthenticationDatabase *authDatabase,
...
public:
    streamingOverTCPRecord(u_int32_t sessionId, unsigned trackNum, streamingOverTCPRecord *next) : fNext(next), fSessionId(sessionId), fTrackNum(trackNum) {}
...
RTSPServer::RTSPClientConnection ::RTSPClientConnection(RTSPServer &ourServer, int clientSocket, struct sockaddr_storage const &clientAddr, Boolean useTLS)
...
void RTSPServer::RTSPClientConnection ::DESCRIBELookupCompletionFunction(void *clientData, ServerMediaSession *sessionLookedUp) {
...
void RTSPServer::RTSPClientConnection::handleCmd_redirect(char const *urlSuffix) {
    snprintf((char *)fResponseBuffer, sizeof fResponseBuffer,
             ...
             "%s"
             "Location: %s%s\r\n\r\n",
             fCurrentCSeq, dateHeader(), fOurRTSPServer.rtspURLPrefix(fClientInputSocket), urlSuffix);
}
...
Boolean RTSPServer::RTSPClientConnection::parseHTTPRequestString(char *resultCmdName, unsigned resultCmdNameMaxSize, char *urlSuffix, unsigned urlSuffixMaxSize,
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_notFound() {
    snprintf((char *)fResponseBuffer, sizeof fResponseBuffer, "HTTP/1.0 404 Not Found\r\n%s\r\n\r\n", dateHeader());
}
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
    ...
    do {
        ...
        Boolean parseSucceeded =
            parseRTSPRequestString((char *)fRequestBuffer, fLastCRLF + 2 - fRequestBuffer, cmdName, sizeof cmdName, urlPreSuffix, sizeof urlPreSuffix,
                                   urlSuffix, sizeof urlSuffix, cseq, sizeof cseq, sessionIdStr, sizeof sessionIdStr, contentLength, urlIsRTSPS);
        ...
        if(parseSucceeded) {
#ifdef DEBUG
            fprintf(stderr,
                    "parseRTSPRequestString() succeeded, returning cmdName \"%s\", urlPreSuffix \"%s\", urlSuffix \"%s\", CSeq \"%s\", Content-Length %u, with "
                    "%d bytes following the message.\n",
            ...
            else if(strcmp(cmdName, "TEARDOWN") == 0 || strcmp(cmdName, "PLAY") == 0 || strcmp(cmdName, "PAUSE") == 0 || strcmp(cmdName, "GET_PARAMETER") == 0
            ...
            *fLastCRLF     = '\0';  // temporarily, for parsing
            parseSucceeded = parseHTTPRequestString(cmdName, sizeof cmdName, urlSuffix, sizeof urlPreSuffix, sessionCookie, sizeof sessionCookie, acceptStr,
                                                    sizeof acceptStr);
            *fLastCRLF     = '\r';
            if(parseSucceeded) {
#ifdef DEBUG
...
static Boolean parseAuthorizationHeader(char const *buf, char const *&username, char const *&realm, char const *&nonce, char const *&uri,
                                        char const *&response) {
...
Boolean RTSPServer::RTSPClientConnection ::authenticationOK(char const *cmdName, char const *urlSuffix, char const *fullRequestStr) {
    ...
    do {
        ...
        // and response string:
        if(!parseAuthorizationHeader(fullRequestStr, username, realm, nonce, uri, response) || username == NULL || realm == NULL
           || strcmp(realm, fCurrentAuthenticator.realm()) != 0 || nonce == NULL || strcmp(nonce, fCurrentAuthenticator.nonce()) != 0 || uri == NULL
           || response == NULL) {
            break;
        }
    ...
    snprintf((char *)fResponseBuffer, sizeof fResponseBuffer,
             ...
             fCurrentCSeq, dateHeader(), fCurrentAuthenticator.realm(), fCurrentAuthenticator.nonce());
    return False;
}
...
static void parseTransportHeader(char const *buf, StreamingMode &streamingMode, char *&streamingModeString, char *&destinationAddressStr,
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP(RTSPServer::RTSPClientConnection *ourClientConnection, char const *urlPreSuffix, char const *urlSuffix,
    ...
    fURLPreSuffix        = urlPreSuffix;
    fURLSuffix           = urlSuffix;
    fFullRequestStr      = fullRequestStr;
    fTrackId             = urlSuffix;  // in the normal case

    // Begin by checking whether the specified stream name exists:
    char const *streamName = urlPreSuffix;  // in the normal case
    fOurServer.lookupServerMediaSession(streamName, SETUPLookupCompletionFunction1, this, fOurServerMediaSession == NULL);
}

void RTSPServer::RTSPClientSession ::SETUPLookupCompletionFunction1(void *clientData, ServerMediaSession *sessionLookedUp) {
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup1(ServerMediaSession *sms) {
    if(sms != NULL) {
    ...
    // Check again:
    fOurServer.lookupServerMediaSession(streamName, SETUPLookupCompletionFunction2, this, fOurServerMediaSession == NULL);
    delete[] concatenatedStreamName;
}

void RTSPServer::RTSPClientSession ::SETUPLookupCompletionFunction2(void *clientData, ServerMediaSession *sessionLookedUp) {
    RTSPServer::RTSPClientSession *session = (RTSPServer::RTSPClientSession *)clientData;
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP_afterLookup2(ServerMediaSession *sms) {
    do {
        ...
        parseTransportHeader(fFullRequestStr, streamingMode, streamingModeString, clientsDestinationAddressStr, clientsDestinationTTL, clientRTPPortNum,
        ...
        subsession->getStreamParameters(fOurSessionId, fOurClientConnection->fClientAddr, clientRTPPort, clientRTCPPort, fStreamStates[trackNum].tcpSocketNum,
                                        ...
                                        serverRTPPort, serverRTCPPort, fStreamStates[trackNum].streamToken);
        ...
        if(fIsMulticast) {
            ...
            case RTP_UDP: {
                snprintf((char *)fOurClientConnection->fResponseBuffer, sizeof fOurClientConnection->fResponseBuffer,
                         ...
                         "Session: %08X%s\r\n\r\n",
                         fOurClientConnection->fCurrentCSeq, dateHeader(), fOurRTSPServer.fWeServeSRTP ? "SAVP" : "AVP", destAddrStr.val(), sourceAddrStr.val(),
                         ntohs(serverRTPPort.num()), ntohs(serverRTCPPort.num()), destinationTTL, fOurSessionId, timeoutParameterString);
                break;
            }
            case RTP_TCP: {
            ...
            case RAW_UDP: {
                snprintf((char *)fOurClientConnection->fResponseBuffer, sizeof fOurClientConnection->fResponseBuffer,
                         ...
                         "Session: %08X%s\r\n\r\n",
                         fOurClientConnection->fCurrentCSeq, dateHeader(), streamingModeString, destAddrStr.val(), sourceAddrStr.val(),
                         ntohs(serverRTPPort.num()), destinationTTL, fOurSessionId, timeoutParameterString);
                break;
            }
            }
        }
            ...
            case RTP_UDP: {
                snprintf((char *)fOurClientConnection->fResponseBuffer, sizeof fOurClientConnection->fResponseBuffer,
                         ...
                         "Session: %08X%s\r\n\r\n",
                         fOurClientConnection->fCurrentCSeq, dateHeader(), fOurRTSPServer.fWeServeSRTP ? "SAVP" : "AVP", destAddrStr.val(), sourceAddrStr.val(),
                         ntohs(clientRTPPort.num()), ntohs(clientRTCPPort.num()), ntohs(serverRTPPort.num()), ntohs(serverRTCPPort.num()), fOurSessionId,
            ...
            case RTP_TCP: {
                if(!fOurRTSPServer.fAllowStreamingRTPOverTCP) {
                    ...
                    snprintf((char *)fOurClientConnection->fResponseBuffer, sizeof fOurClientConnection->fResponseBuffer,
                             ...
                             "Session: %08X%s\r\n\r\n",
                             fOurClientConnection->fCurrentCSeq, dateHeader(), destAddrStr.val(), sourceAddrStr.val(), rtpChannelId, rtcpChannelId,
            ...
            case RAW_UDP: {
                snprintf((char *)fOurClientConnection->fResponseBuffer, sizeof fOurClientConnection->fResponseBuffer,
                         ...
                         "Session: %08X%s\r\n\r\n",
                         fOurClientConnection->fCurrentCSeq, dateHeader(), streamingModeString, destAddrStr.val(), sourceAddrStr.val(),
                         ntohs(clientRTPPort.num()), ntohs(serverRTPPort.num()), fOurSessionId, timeoutParameterString);
                break;
            }
            }
        }
...
void RTSPServer::RTSPClientSession ::handleCmd_withinSession(RTSPServer::RTSPClientConnection *ourClientConnection, char const *cmdName,
    ...
    if(fOurServerMediaSession == NULL) {  // There wasn't a previous SETUP!
        ...
        if(strncmp(fOurServerMediaSession->streamName(), urlPreSuffix, urlPreSuffixLen) == 0 && fOurServerMediaSession->streamName()[urlPreSuffixLen] == '/'
...
void RTSPServer::RTSPClientSession ::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection *ourClientConnection, ServerMediaSubsession *subsession) {
    unsigned i;
...
void RTSPServer::RTSPClientSession ::handleCmd_PLAY(RTSPServer::RTSPClientConnection *ourClientConnection, ServerMediaSubsession *subsession,
                                                    char const *fullRequestStr) {
            ...
            if(fStreamStates[i].subsession != NULL) {
                ...
                if(absStart != NULL) {
                    ...
                    if(!sawRangeHeader || startTimeIsNow) {
                        ...
                        fStreamStates[i].subsession->nullSeekStream(fOurSessionId, fStreamStates[i].streamToken, rangeEnd, numBytes);
                    }
                    else {
                        // We do a real 'seek':
    ...
    for(i = 0; i < fNumStreamStates; ++i) {
        if(subsession == NULL /* means: aggregated operation */
           || subsession == fStreamStates[i].subsession) {
            ...
            if(fStreamStates[i].subsession == NULL)
                continue;
            fStreamStates[i].subsession->startStream(fOurSessionId, fStreamStates[i].streamToken, (TaskFunc *)noteClientLiveness, this, rtpSeqNum, rtpTimestamp,
            ...
            sprintf(rtpInfo, rtpInfoFmt, prevRTPInfo, numRTPInfoItems++ == 0 ? "" : ",", rtspURL, urlSuffix, rtpSeqNum, rtpTimestamp);
            delete[] prevRTPInfo;
        }
    }
    ...
    snprintf((char *)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
             ...
             "Session: %08X\r\n"
             "%s\r\n",
             ourClientConnection->fCurrentCSeq, dateHeader(), scaleHeader, rangeHeader, fOurSessionId, rtpInfo);
    delete[] rtpInfo;
    delete[] rangeHeader;
    delete[] scaleHeader;
    delete[] rtspURL;
...
void RTSPServer::RTSPClientSession ::handleCmd_PAUSE(RTSPServer::RTSPClientConnection *ourClientConnection, ServerMediaSubsession *subsession) {
    for(unsigned i = 0; i < fNumStreamStates; ++i) {
        if(subsession == NULL /* means: aggregated operation */
           || subsession == fStreamStates[i].subsession) {
            if(fStreamStates[i].subsession != NULL) {
...
void RTSPServer::RTSPClientSession ::handleCmd_GET_PARAMETER(RTSPServer::RTSPClientConnection *ourClientConnection, ServerMediaSubsession * /*subsession*/,
                                                             char const * /*fullRequestStr*/) {
...
void RTSPServer::RTSPClientSession ::handleCmd_SET_PARAMETER(RTSPServer::RTSPClientConnection *ourClientConnection, ServerMediaSubsession * /*subsession*/,
                                                             char const * /*fullRequestStr*/) {
...
GenericMediaServer::ClientConnection *RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_storage const &clientAddr) {
