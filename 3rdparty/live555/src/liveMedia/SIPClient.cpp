...
SIPClient *SIPClient ::createNew(UsageEnvironment &env, unsigned char desiredAudioRTPPayloadFormat, char const *mimeSubtype, int verbosityLevel,
...
SIPClient::SIPClient(UsageEnvironment &env, unsigned char desiredAudioRTPPayloadFormat, char const *mimeSubtype, int verbosityLevel,
                     char const *applicationName)
    : Medium(env),
      fT1(500000 /* 500 ms */),
...
char *SIPClient::invite1(Authenticator *authenticator) {
    do {
        ...
        if(fMIMESubtypeSize > 0) {
            ...
            unsigned          rtpmapFmtSize = strlen(rtpmapFmt) + 3 /* max char len */ + fMIMESubtypeSize;
        ...
        char const *const inviteSDPFmt = "v=0\r\n"
                                         ...
                                         "m=audio %u RTP/AVP %u\r\n"
                                         "%s";
        unsigned inviteSDPFmtSize = strlen(inviteSDPFmt) + 20 /* max int len */ + 20 + fOurAddressStrSize + fApplicationNameSize + fOurAddressStrSize
                                    + 5 /* max short len */ + 3 /* max char len */
                                    + rtpmapLineSize;
        delete[] fInviteSDPDescription;
        fInviteSDPDescription = new char[inviteSDPFmtSize];
        sprintf(fInviteSDPDescription, inviteSDPFmt, fCallId, fCSeq, fOurAddressStr, fApplicationName, fOurAddressStr, fClientStartPortNum,
        ...
        char const *const cmdFmt = "INVITE %s SIP/2.0\r\n"
                                   ...
                                   "Content-Length: %d\r\n\r\n"
                                   "%s";
        unsigned inviteCmdSize = strlen(cmdFmt) + fURLSize + 2 * fUserNameSize + fOurAddressStrSize + 20 /* max int len */
                                 + fOurAddressStrSize + 5                                                /* max port len */
                                 + fURLSize + fUserNameSize + fOurAddressStrSize + 5 + 20 + fOurAddressStrSize + 20 + strlen(authenticatorStr)
                                 + fUserAgentHeaderStrLen + 20 + inviteSDPSize;
        delete[] fInviteCmd;
        fInviteCmd = new char[inviteCmdSize];
        sprintf(fInviteCmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr, fFromTag, fOurAddressStr, fOurPortNum, fURL, fUserName, fOurAddressStr,
                fOurPortNum, fCallId, fOurAddressStr, ++fCSeq, authenticatorStr, fUserAgentHeaderStr, inviteSDPSize, fInviteSDPDescription);
...
void SIPClient::timerAHandler(void *clientData) {
    ...
    if(client->fVerbosityLevel >= 1) {
        client->envir() << "RETRANSMISSION " << ++client->fTimerACount << ", after " << client->fTimerALen / 1000000.0 << " additional seconds\n";
    }
...
unsigned SIPClient::getResponseCode() {
    ...
    do {
        ...
        if(responseCode != 200) {
            if(responseCode >= 400 && responseCode <= 499 && fWorkingAuthenticator != NULL) {
                ...
                while(1) {
                    ...
                    if(
                        ...
                        // Cisco ATA #####
                        sscanf(lineStart, "Proxy-Authenticate: Digest algorithm=MD5,domain=\"%*[^\"]\",nonce=\"%[^\"]\", realm=\"%[^\"]\"", nonce, realm)
                            == 2) {
            ...
            if(contentLength > (int)numBodyBytes) {
                ...
                if(numExtraBytesNeeded > remainingBufferSize) {
                    ...
                    sprintf(tmpBuf, "Read buffer size (%d) is too small for \"Content-Length:\" %d (need a buffer size of >= %d bytes\n", readBufSize,
                            contentLength, readBufSize + numExtraBytesNeeded - remainingBufferSize);
                ...
                while(numExtraBytesNeeded > 0) {
                    ...
                    Boolean                 readSuccess = fOurSocket->handleRead((unsigned char *)ptr, numExtraBytesNeeded, bytesRead2, dummy);
                    if(!readSuccess)
                        break;
                    ptr[bytesRead2] = '\0';
                    if(fVerbosityLevel >= 1) {
...
Boolean SIPClient::sendACK() {
    ...
    do {
        char const *const cmdFmt = "ACK %s SIP/2.0\r\n"
                                   ...
                                   "CSeq: %d ACK\r\n"
                                   "Content-Length: 0\r\n\r\n";
        unsigned cmdSize = strlen(cmdFmt) + fURLSize + 2 * fUserNameSize + fOurAddressStrSize + 20 /* max int len */
                           + fOurAddressStrSize + 5                                                /* max port len */
                           + fURLSize + fToTagStrSize + 20 + fOurAddressStrSize + 20;
        cmd = new char[cmdSize];
        sprintf(cmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr, fFromTag, fOurAddressStr, fOurPortNum, fURL, fToTagStr, fCallId, fOurAddressStr,
...
char *SIPClient::createAuthenticatorString(Authenticator const *authenticator, char const *cmd, char const *url) {
    if(authenticator != NULL && authenticator->realm() != NULL && authenticator->nonce() != NULL && authenticator->username() != NULL
       && authenticator->password() != NULL) {
        ...
        unsigned          authBufSize = strlen(authFmt) + strlen(authenticator->username()) + strlen(authenticator->realm()) + strlen(authenticator->nonce())
...
unsigned SIPClient::getResponse(char *&responseBuffer, unsigned responseBufferSize) {
    ...
    while(bytesRead < (int)responseBufferSize) {
        ...
        Boolean                 readSuccess = fOurSocket->handleRead(toPosn, responseBufferSize - bytesRead, bytesReadNow, dummy);
