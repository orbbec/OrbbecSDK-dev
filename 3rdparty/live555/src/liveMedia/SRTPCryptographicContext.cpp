...
#ifndef NO_OPENSSL
    : fMIKEYState(mikeyState), fHaveReceivedSRTPPackets(False), fHaveSentSRTPPackets(False), fSRTCPIndex(0) {
...
#ifdef DEBUG
                    fprintf(stderr,
                            "SRTPCryptographicContext::processIncomingSRTPPacket(): Error: Packet size %d is shorter than the minimum specified RTP header "
                            "size %d!\n",
...
#ifdef DEBUG
            fprintf(stderr,
                    "SRTPCryptographicContext::processIncomingSRTCPPacket(): Error: Packet size %d is too short for encrpytion and/or authentication!\n",
                    inPacketSize);
#endif
            break;
        }
...
Boolean SRTPCryptographicContext ::processOutgoingSRTPPacket(u_int8_t *buffer, unsigned inPacketSize, unsigned &outPacketSize) {
#ifndef NO_OPENSSL
    do {
        ...
        if(weEncryptSRTP()) {
...
#ifdef DEBUG
                    fprintf(stderr,
                            "SRTPCryptographicContext::processOutgoingSRTPPacket(): Error: Packet size %d is shorter than the minimum specified RTP header "
                            "size %d!\n",
                            inPacketSize, rtpHeaderSize + 4);
#endif
                    break;
            ...
            if(!fHaveSentSRTPPackets) {
                ...
                if(rtpSeqNum == 0)
                    ++fSendingROC;  // increment the ROC when the RTP seq num rolls over
            }
        ...
        if(weAuthenticate()) {
            ...
            outPacketSize += generateSRTPAuthenticationTag(buffer, outPacketSize, &buffer[outPacketSize]);
        }
...
#ifndef NO_OPENSSL
unsigned SRTPCryptographicContext ::generateSRTPAuthenticationTag(u_int8_t const *dataToAuthenticate, unsigned numBytesToAuthenticate,
                                                                  u_int8_t *resultAuthenticationTag) {
    return generateAuthenticationTag(fDerivedKeys.srtp, dataToAuthenticate, numBytesToAuthenticate, resultAuthenticationTag);
}

unsigned SRTPCryptographicContext ::generateSRTCPAuthenticationTag(u_int8_t const *dataToAuthenticate, unsigned numBytesToAuthenticate,
                                                                   u_int8_t *resultAuthenticationTag) {
    return generateAuthenticationTag(fDerivedKeys.srtcp, dataToAuthenticate, numBytesToAuthenticate, resultAuthenticationTag);
}

Boolean SRTPCryptographicContext ::verifySRTPAuthenticationTag(u_int8_t *dataToAuthenticate, unsigned numBytesToAuthenticate, u_int32_t roc,
...
Boolean SRTPCryptographicContext ::verifySRTCPAuthenticationTag(u_int8_t const *dataToAuthenticate, unsigned numBytesToAuthenticate,
                                                                u_int8_t const *authenticationTag) {
...
void SRTPCryptographicContext ::encryptSRTPPacket(u_int64_t index, u_int32_t ssrc, u_int8_t *data, unsigned numDataBytes) {
    cryptData(fDerivedKeys.srtp, index, ssrc, data, numDataBytes);
}
...
unsigned SRTPCryptographicContext ::generateAuthenticationTag(derivedKeys &keysToUse, u_int8_t const *dataToAuthenticate, unsigned numBytesToAuthenticate,
                                                              u_int8_t *resultAuthenticationTag) {
...
Boolean SRTPCryptographicContext ::verifyAuthenticationTag(derivedKeys &keysToUse, u_int8_t const *dataToAuthenticate, unsigned numBytesToAuthenticate,
                                                           u_int8_t const *authenticationTag) {
    u_int8_t computedAuthTag[SHA1_DIGEST_LEN];
    HMAC_SHA1(keysToUse.authKey, sizeof keysToUse.authKey, dataToAuthenticate, numBytesToAuthenticate, computedAuthTag);
    ...
    for(unsigned i = 0; i < SRTP_AUTH_TAG_LENGTH; ++i) {
        if(computedAuthTag[i] != authenticationTag[i])
            return False;
    }
    return True;
}
...
void SRTPCryptographicContext::cryptData(derivedKeys &keys, u_int64_t index, u_int32_t ssrc, u_int8_t *data, unsigned numDataBytes) {
    ...
    iv[sizeof iv - 12] ^= ssrc >> 24;
    iv[sizeof iv - 11] ^= ssrc >> 16;
    iv[sizeof iv - 10] ^= ssrc >> 8;
    iv[sizeof iv - 9] ^= ssrc;

    iv[sizeof iv - 8] ^= index >> 40;
    iv[sizeof iv - 7] ^= index >> 32;
    iv[sizeof iv - 6] ^= index >> 24;
    iv[sizeof iv - 5] ^= index >> 16;
    iv[sizeof iv - 4] ^= index >> 8;
    iv[sizeof iv - 3] ^= index;
...
void SRTPCryptographicContext ::deriveSingleKey(u_int8_t const *masterKey, u_int8_t const *salt, SRTPKeyDerivationLabel label, unsigned resultKeyLength,
                                                u_int8_t *resultKey) {
