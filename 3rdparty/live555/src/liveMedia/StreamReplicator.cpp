...
void StreamReplicator::deactivateStreamReplica(StreamReplica *replicaBeingDeactivated) {
    if(replicaBeingDeactivated->fFrameIndex == -1)
        return;  // this replica has already been deactivated (or was never activated at all)
    ...
    if(replicaBeingDeactivated->fFrameIndex != fFrameIndex && fNumDeliveriesMadeSoFar > 0)
    ...
    if(replicaBeingDeactivated == fPrimaryReplica) {
        ...
        // the primary replica:
        if(fPrimaryReplica != NULL && fInputSource != NULL && !fInputSource->isCurrentlyAwaitingData())
            deliverReceivedFrame();
    }
...
void StreamReplicator::afterGettingFrame(void *clientData, unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime,
