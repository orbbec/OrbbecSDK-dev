...
int ClientTLSState::connect(int socketNum) {
    ...
    else if(sslConnectResult < 0 && (sslGetErrorResult == SSL_ERROR_WANT_READ || sslGetErrorResult == SSL_ERROR_WANT_WRITE)) {
...
ServerTLSState::ServerTLSState(UsageEnvironment &env)
    : tlsAcceptIsNeeded(False)
      ...
      ,
      fEnv(env),
      fCertificateFileName(NULL),
      fPrivateKeyFileName(NULL)
#endif
{
}
...
void ServerTLSState ::setCertificateAndPrivateKeyFileNames(char const *certFileName, char const *privKeyFileName) {
#ifndef NO_OPENSSL
...
Boolean ServerTLSState::setup(int socketNum) {
    do {
        ...
        if(SSL_CTX_use_certificate_file(fCtx, fCertificateFileName, SSL_FILETYPE_PEM) != 1)
            break;
