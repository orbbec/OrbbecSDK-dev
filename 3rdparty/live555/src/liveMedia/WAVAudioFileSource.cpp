...
static Boolean get4Bytes(FILE *fid, u_int32_t &result) {  // little-endian
    int c0, c1, c2, c3;
    if((c0 = nextc) == EOF || (c1 = nextc) == EOF || (c2 = nextc) == EOF || (c3 = nextc) == EOF)
        return False;
...
WAVAudioFileSource::WAVAudioFileSource(UsageEnvironment &env, FILE *fid)
    ...
    do {
        ...
        while(tmp != 0x20746d66 /*'fmt ', little-endian*/) {
            ...
            if(!get4Bytes(fid, chunkLength))
                break;
            if(!skipBytes(fid, chunkLength))
                break;
            if(!get4Bytes(fid, tmp))
                break;
        }
        ...
        // EYRE chunk (optional):
        if(c == 'e') {
            if(nextc != 'y' || nextc != 'r' || nextc != 'e')
                break;
            unsigned eyreLength;
            if(!get4Bytes(fid, eyreLength))
                break;
            if(!skipBytes(fid, eyreLength))
                break;
            c = nextc;
        }
