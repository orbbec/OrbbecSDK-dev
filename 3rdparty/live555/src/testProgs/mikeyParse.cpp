...
static Boolean parseKeyDataSubPayload(u_int8_t const *&ptr, u_int8_t const *endPtr, u_int8_t &nextPayloadType) {
    ...
    for(unsigned i = 0; i < keyDataLen; ++i)
        fprintf(stderr, ":%02x", getByte(ptr));
    fprintf(stderr, "\n");
    ...
    if(hasKV) {
        ...
        if(KV == 1) {  // SPI/MKI
            ...
            testSize(SPILength);
            fprintf(stderr, "\t\t\tSPI: ");
            for(unsigned i = 0; i < SPILength; ++i)
                fprintf(stderr, ":%02x", getByte(ptr));
            fprintf(stderr, "\n");
        }
...
Boolean parseMikeyKEMAC(u_int8_t const *&ptr, u_int8_t const *endPtr, u_int8_t &nextPayloadType) {
    ...
    fprintf(stderr, "\tEncr alg: %d (%s)\n", encrAlg, encrAlg == 0 ? "NULL" : encrAlg == 1 ? "AES-CM-128" : encrAlg == 2 ? "AES-KW-128" : "unknown");
    ...
    while(ptr < endOfKeyData) {
        if(!parseKeyDataSubPayload(ptr, endOfKeyData, nextPayloadType))
            return False;
    }

    u_int8_t macAlg = getByte(ptr);
    fprintf(stderr, "\tMAC alg: %d (%s)\n", macAlg, macAlg == 0 ? "NULL" : macAlg == 1 ? "HMAC-SHA-1-160" : "unknown");
    if(macAlg > 1)
        return False;
...
Boolean parseMikeyT(u_int8_t const *&ptr, u_int8_t const *endPtr, u_int8_t &nextPayloadType) {
    ...
    for(unsigned i = 0; i < TS_value_len; ++i)
        fprintf(stderr, ":%02x", getByte(ptr));
    fprintf(stderr, "\n");
...
static char const *SRTPPolicyParamTypeExplanation[] = {
    "Encryption algorithm",        "Session Encryption key length", "Authentication algorithm", "Session Authentication key length", "Session Salt key length",
    "SRTP Pseudo Random Function", "Key derivation rate",           "SRTP encryption off/on",   "SRTCP encryption off/on",           "Sender's FEC order",
    "SRTP authentication off/on",  "Authentication tag length",     "SRTP prefix length",
};
...
static Boolean parseSRTPPolicyParam(u_int8_t const *&ptr, u_int8_t const *endPtr) {
    ...
    while(ptr < endPtr) {
        ...
        fprintf(stderr, "\t\ttype: %d (%s); ", ppType, ppType > MAX_SRTP_POLICY_PARAM_TYPE ? "unknown" : SRTPPolicyParamTypeExplanation[ppType]);
        ...
        if(ppLen == 1) {
            ...
            for(unsigned j = 0; j < ppLen; ++j)
                fprintf(stderr, ":%02x", getByte(ptr));
        }

        switch(ppType) {
        case 0: {  // Encryption algorithm
            fprintf(stderr, " (%s)", ppVal == 0 ? "NULL" : ppVal == 1 ? "AES-CM" : ppVal == 2 ? "AES-F8" : "unknown");
            break;
        }
        case 2: {  // Authentication algorithm
            fprintf(stderr, " (%s)", ppVal == 0 ? "NULL" : ppVal == 1 ? "HMAC-SHA-1" : "unknown");
            break;
        }
        case 5: {  // SRTP Pseudo Random Function
            fprintf(stderr, " (%s)", ppVal == 0 ? "AES-CM" : "unknown");
            break;
        }
        case 9: {  // sender's FEC order
            fprintf(stderr, " (%s)", ppVal == 0 ? "First FEC, then SRTP" : "unknown");
            break;
        }
        }
        fprintf(stderr, "\n");
...
int main(int argc, char **argv) {
    ...
    for(unsigned i = 0; i < mikeyDataSize; ++i)
        fprintf(stderr, ":%02x", mikeyData[i]);
    fprintf(stderr, "\n");
    ...
    do {
        ...
        fprintf(stderr, "HDR:\n");
        if(!parseMikeyHDR(ptr, endPtr, nextPayloadType))
            break;

        // Then parse each successive payload:
        while(nextPayloadType != 0 /* Last payload */) {
            fprintf(stderr, "%s:\n", payloadTypeName[nextPayloadType]);
            if(!(*payloadParser[nextPayloadType])(ptr, endPtr, nextPayloadType))
                break;
        }
    } while(0);
