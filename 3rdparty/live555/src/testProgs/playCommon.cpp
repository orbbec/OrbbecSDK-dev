...
void usage() {
    *env << "Usage: " << progName
         ...
         << (controlConnectionUsesTCP ? " [-t|-T <http-port>]" : "") << " [-u <username> <password>"
         ...
         << " [-w <width> -h <height>] [-f <frames-per-second>] [-y] [-H] [-Q [<measurement-interval>]] [-F <filename-prefix>] [-b <file-sink-buffer-size>] "
            "[-B <input-socket-buffer-size>] [-I <input-interface-ip-address>] [-m] [<url>|-R [<port-num>]] (or "
         << progName << " -o [-V] <url>)\n";
    shutdown();
}
...
int main(int argc, char **argv) {
    ...
    while(argc > 1) {
        ...
        case 'p': {  // specify start port number
            ...
            if(portArg <= 0 || portArg >= 65536 || portArg & 1) {
                *env << "bad port number: " << portArg << " (must be even, and in the range (0,65536))\n";
                usage();
            }
    ...
    if(createHandlerServerForREGISTERCommand) {
        handlerServerForREGISTERCommand = HandlerServerForREGISTERCommand::createNew(*env, continueAfterClientCreation0, handlerServerForREGISTERCommandPortNum,
                                                                                     authDBForREGISTER, verbosityLevel, progName);
    ...
    while((subsession = iter.next()) != NULL) {
        ...
        if(singleMediumToTest != NULL) {
            if(strcmp(subsession->mediumName(), singleMediumToTest) != 0) {
                *env << "Ignoring \"" << subsession->mediumName() << "/" << subsession->codecName() << "\" subsession, because we've asked to receive a single "
        ...
        if(createReceivers) {
                ...
                if(subsession->rtpSource() != NULL) {
                    ...
                    if(socketInputBufferSize > 0 || fileSinkBufferSize > curBufferSize) {
                        ...
                        if(socketInputBufferSize > 0) {  // The user explicitly asked for the new socket buffer size; announce it:
                            *env << "Changed socket receive buffer size for the \"" << subsession->mediumName() << "/" << subsession->codecName()
                                 << "\" subsession from " << curBufferSize << " to " << newBufferSize << " bytes\n";
                        }
                    }
                }
            }
            ...
            if(subsession->clientPortNum() == 0) {
                *env << "No client port was specified for the \"" << subsession->mediumName() << "/" << subsession->codecName()
...
void createOutputFiles(char const *periodicFilenameSuffix) {
    ...
    if(outputQuickTimeFile || outputAVIFile) {
        ...
        if(outputQuickTimeFile) {
            qtOut = QuickTimeFileSink::createNew(*env, *session, outFileName, fileSinkBufferSize, movieWidth, movieHeight, movieFPS, packetLossCompensate,
        ...
        while((subsession = iter.next()) != NULL) {
            ...
            if(singleMedium == NULL || periodicFilenameSuffix[0] != '\0') {
                ...
                snprintf(outFileName, sizeof outFileName, "%s%s-%s-%d%s", fileNamePrefix, subsession->mediumName(), subsession->codecName(), ++streamCounter,
            ...
            if(strcmp(subsession->mediumName(), "video") == 0) {
                if(strcmp(subsession->codecName(), "H264") == 0) {
                    ...
                    fileSink = H265VideoFileSink::createNew(*env, outFileName, subsession->fmtp_spropvps(), subsession->fmtp_spropsps(),
...
void continueAfterPLAY(RTSPClient *, int resultCode, char *resultString) {
    ...
    if(duration > 0) {
        ...
        if(duration + rangeAdjustment > 0.0)
            duration += rangeAdjustment;
...
void checkForPacketArrival(void * /*clientData*/) {
    ...
    if(!syncStreams) {
        ...
        notifyTheUser = numSubsessionsWithReceivedData >= numSubsessionsToCheck && numSubsessionsThatHaveBeenSynced == numSubsessionsChecked;
...
void checkSessionTimeoutBrokenServer(void * /*clientData*/) {
    if(!sendKeepAlivesToBrokenServers)
        return;  // we're not checking
    ...
    sessionTimeoutBrokenServerTask =
        env->taskScheduler().scheduleDelayedTask(secondsUntilNextKeepAlive * 1000000, (TaskFunc *)checkSessionTimeoutBrokenServer, NULL);
}
