...
int main(int argc, char const **argv) {
    ...
    float startTime;
    if(sscanf(argv[2], "%f", &startTime) != 1 || startTime < 0.0f)
        usage();

    int scale;
    if(sscanf(argv[3], "%d", &scale) != 1 || scale == 0)
        usage();
    ...
    if(startTime > 0.0f) {
        ...
        if(!trickModeFilter->seekTo(tsRecordNumber, indexRecordNumber)) {  // TARFU!
            *env << "Failed to seek trick mode filter to ts #" << (unsigned)tsRecordNumber << ", ix #" << (unsigned)indexRecordNumber << "(for time "
                 << startTime << ")\n";
            exit(1);
        }
    }
