    ...
    for(unsigned i = 0; i < numTracks; ++i) {
        ...
        if(trackState[i].source != NULL) {
            ...
            if(trackState[i].sink != NULL) {
                ...
                trackState[i].rtcp = RTCPInstance::createNew(*env, rtcpGroupsock, estBitrate, CNAME, trackState[i].sink, NULL /* we're a server */,
...
void afterPlaying(void * /*clientData*/) {
    *env << "...done reading from file\n";
    ...
    for(i = 0; i < numTracks; ++i) {
        if(trackState[i].sink != NULL)
            trackState[i].sink->stopPlaying();
        Medium::close(trackState[i].source);
        trackState[i].source = NULL;
    }
