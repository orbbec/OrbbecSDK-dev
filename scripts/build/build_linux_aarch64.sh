...
echo "Building openorbbecsdk for linux_aarch64"

# Detect the latest docker image with prefix "openorbbecsdk-env:aarch64_"
DOCKER_IMAGE=$(docker images --format "{{.Repository}}:{{.Tag}}" | grep "openorbbecsdk-env:aarch64_" | sort -r | head -n 1)
if [ -z "$DOCKER_IMAGE" ]; then
    echo "Docker image openorbbecsdk-env:aarch64 with latest timestamp not found"
    exit 1
fi

# Variables for version, timestamp, and commit hash
...
# Define the installation directory name with version, timestamp, commit hash, and platform
...
docker run --rm -u $USER_ID:$GROUP_ID -v "$PROJECT_ROOT":/workspace -w /workspace/build-release-$PLATFORM --entrypoint /bin/bash $DOCKER_IMAGE -c "
    cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR -DBUILD_SHARED_LIBS=ON -DOB_BUILD_LINUX_ARM64=ON || { echo 'Failed to run cmake'; exit 1; }
