...
G2DepthAlgModeManager::G2DepthAlgModeManager(IDevice *owner) : DeviceComponentBase(owner) {
...
std::vector<OBDepthAlgModeChecksum> G2DepthAlgModeManager::getDepthAlgModeList() const {
    return depthAlgModeChecksumList_;
}

const OBDepthAlgModeChecksum &G2DepthAlgModeManager::getCurrentDepthAlgModeChecksum() const {
    return currentAlgMode_;
}

void G2DepthAlgModeManager::switchDepthAlgMode(const std::string &modeName) {
...
void G2DepthAlgModeManager::switchDepthAlgMode(const OBDepthAlgModeChecksum &targetDepthMode) {
    auto owner      = getOwner();
