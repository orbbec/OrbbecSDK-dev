...
G330SensorStartStrategy::G330SensorStartStrategy(std::shared_ptr<G330DepthAlgModeManager> depthAlgModeManager) {
    initializeValidateSensor();
    depthAlgModeManager_ = depthAlgModeManager;
...
bool G330SensorStartStrategy::validatePresetProfile(OBSensorType sensorType, std::shared_ptr<const StreamProfile> streamProfile) {
    bool isValid = true;
    if(depthAlgModeManager_ == nullptr) {
        return isValid;
    }
    auto        currentDepthMode = depthAlgModeManager_->getCurrentDepthAlgModeChecksum();
