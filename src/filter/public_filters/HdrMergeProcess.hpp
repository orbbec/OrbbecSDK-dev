...
private:
    ...
    void                   discardDepthMergedFrameIfNeeded(const std::shared_ptr<Frame> frame);
    std::shared_ptr<Frame> mergingAlgorithm(const std::shared_ptr<Frame> first_f, const std::shared_ptr<Frame> second_f, const bool use_ir);
