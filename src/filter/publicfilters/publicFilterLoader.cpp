...
#define ADD_FILTER_CREATOR(name) { #name, std::make_shared<publicFilterCreator>([]() { return std::make_shared<name>(#name); }) }
...
std::map<std::string, std::shared_ptr<IFilterCreator>> getCreators() {
    static std::map<std::string, std::shared_ptr<IFilterCreator>> filterCreators = {
        ...
        ADD_FILTER_CREATOR(IMUCorrector),
        ADD_FILTER_CREATOR(Align),
    };
