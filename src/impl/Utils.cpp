...
bool ob_calibration_3d_to_3d(const ob_calibration_param calibration_param, const ob_point3f source_point3f,
    ...
    memcpy(&extrinsic, &calibration_param.extrinsics[OB_SENSOR_DEPTH][OB_SENSOR_COLOR], sizeof(OBExtrinsic));
...
ob_frame *transformation_depth_frame_to_color_camera(ob_device *device, ob_frame *depth_frame, uint32_t target_color_camera_width,
    ...
    if(device == nullptr || device->device == nullptr) {
        throw libobsensor::invalid_value_exception("Device is invalid!");
    }
    if(depth_frame == nullptr || depth_frame->frame == nullptr) {
        throw libobsensor::invalid_value_exception("Input depth frame is invalid!");
    }
    if(target_color_camera_width == 0 || target_color_camera_height == 0) {
        throw libobsensor::invalid_value_exception("Input color camera target resolution is invalid!");
    }
