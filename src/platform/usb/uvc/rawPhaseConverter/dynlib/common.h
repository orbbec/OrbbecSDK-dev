...
#define MAX_SERIAL_NUMBER_LENGTH (13 * 2)  // Current schema is for 12 digits plus NULL, the extra size is in case that grows in the future.
...
inline static bool k4a_convert_resolution_to_width_height(k4a_color_resolution_t resolution, uint32_t *width_out, uint32_t *height_out) {
    uint32_t width  = 0;
    uint32_t height = 0;
    switch(resolution) {
...
inline static bool k4a_convert_depth_mode_to_width_height(k4a_depth_mode_t mode, uint32_t *width_out, uint32_t *height_out) {
    uint32_t width  = 0;
    uint32_t height = 0;
    switch(mode) {
...
inline static bool k4a_is_version_greater_or_equal(k4a_version_t *fw_version_l, k4a_version_t *fw_version_r) {
    typedef enum { FW_OK, FW_TOO_LOW, FW_UNKNOWN } fw_check_state_t;
